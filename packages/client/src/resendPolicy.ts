...
export const VOLATILE_MESSAGES: ReadonlySet<string> = new Set([
  ...
  "ping", // 스스로 다시 온다
  // 「방금 멎어 있었다」는 **그 순간의 관찰**이다. 15초 뒤에 도착하면 그때는 이미
  // 다른 국면이고, 로그에 엉뚱한 시각으로 박혀 원인 추적을 오히려 흐린다.
  // 게다가 끊겼다 붙는 길목에서 큐에 담기면, 재접속마다 묵은 보고가 한 번씩 더 나간다.
  "clientStall",
  ...
  "adminPauseGame",
  // 방 공지·시간 연장도 같다 — 둘 다 «지금 이 탁자, 지금 이 순간»에 거는 것이다.
  // 15초 묵은 「5분 뒤 재개」나 「+30초」가 다음 판에 도착하면 그건 구조가 아니라 소음이다.
  "adminRoomNotice", "adminExtendTime",
  // 국 무효도 같다 — 되돌릴 수 없는 판정이고, 방 코드는 판이 끝나도 그 방에 남는다.
  // 15초 묵은 「이 국을 물려라」가 도착하면 관리자가 본 적 없는 국이 지워진다.
  "adminVoidRound",
  /*
   * 전역 공지도 «지금 이 순간»의 조작이다 (QA 2차 admin 확정 5).
   *
   * 이 줄이 여태 **없었다.** 정의가 protocol.ts 의 서버→클라이언트 구간에 잘못
   * 놓여 있어서(지금은 옮겼다) 「두 목록이 ClientMessage 전체를 덮는다」를 강제하는
   * 테스트가 이 타입을 아예 못 봤다. `isResendable` 기본값이 false 라 사실상
   * 볼라틸로 굴러가긴 했지만, 분류되지 않은 채로 굴러간 것과 분류된 것은 다르다 —
   * 이 자리 근처에 다음 메시지를 더하는 사람이 같은 구멍에 빠진다.
   *
   * 볼라틸인 이유 자체는 위 손잡이들과 같다: 끊긴 사이에 세운 「점검 5분 전」이
   * 15초 뒤에야 도착하면 그건 예고가 아니다. 다시 세우는 편이 낫다.
   */
  "adminSetNotice",
  // 점검 모드 켜기·끄기(2026-09-21)도 같다 — 끊긴 사이 누른 «켜라»가 15초 뒤 도착하면
  // 관리자가 이미 마음을 바꿨을 수 있고, 켜지는 순간 전원이 막히는 조작이라 늦게
  // 도착한 한 줄이 곧 사고다. 다시 누르는 편이 낫다.
  "adminSetMaintenance",
]);
...
export const RESENDABLE_MESSAGES: ReadonlySet<string> = new Set([
  ...
  "ready", "startGame", "setGameMode", "setRoomRules", "setRoomPace", "shuffleSeats", "moveSeat",

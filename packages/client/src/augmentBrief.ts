...
export const AUGMENT_BRIEF: Record<string, AugmentBrief> = {
  ...
  avenger: { use: "상시", text: "내 복수 상대에게는 후리텐이어도, 역이없어도(단, 머리1+몸통4의 형태) 론할 수 있다. 그 론은 +2판" },
  ...
  bottom_deal: { use: "2순에 1회", text: "패산 맨 밑 3장을 항상 볼 수 있으며, 사용시 다음 순의 쯔모를 패산 위가 아닌 패산 밑에서 뽑는다." },
  ...
  broken_wall: { use: "상시", text: "슌쯔가 9에서 1로 이어진다 - 1과 9사이의 벽이 사라져 8-9-1, 9-1-2 도 한 몸통으로 취급한다." },
  call_seal: { use: CD_2_3, text: "6순 동안 상대 셋이 치·퐁·대명깡을 하지 못한다." },
  cliff_bloom: {
    use: "상시",
    text: "뽑아올 영상패를 직접 고른다. 한 국에 깡을 두 번 하면 그 자리에서 반드시 화료할 수 있다.(만개는 동풍전 1회, 반장전 2회)",
  },
  conjure_draw: { use: "매 국 1회", text: "손패 1장을 지목하면 다음 쯔모에 반드시 그 패를 소환해온다." },
  counter: { use: "매 국 1회", text: "선제 리치한 상대에게 추격리치로 반격하여 먼저 화료하면 그 상대가 받아야했던 점수를 뱅크에서 추가로 받는다." },
  danger_sense: { use: "매 국 1회", text: "지금 버리면 방총이 되는 패가 무엇인지 나에게만 알려 준다." },
  dead_wall_master: {
    use: "2국에 1회",
    text: "왕패 14장이 다 보이고, 발동한 국 첫 순에 왕패와 손패를 2장까지 바꾼다.",
  },
  devils_advance: { use: "게임 1회", text: "10000점을 미리 받고, 만관 이상으로 화료시 내 빚을 상대가 대신 3000점씩 갚는다." },
  ...
  future_sight: { use: "3순에 1회", text: "손패 무작위 3장을 뽑아 패산 위 3장과 교환한다. 이때 선택한 패를 내 바닥에 버린다. 화료시 교환횟수마다 +1판." },
  ...
  karma: { use: MODE_1_2, text: "잃은 점수가 게이지로 쌓이고, 8000 이상이면 사용하여 상대 셋에게서 게이지의 1/3씩 빼앗아온다." },
  ...
  meld_dissolve: { use: "매 국 1회", text: "내 치, 퐁 하나를 후로하기전으로 되돌린다." },
  ...
  no_retreat: { use: "2국에 1회", text: "액티브 리치 사용시 리치,일발은 각각 2판, 뒷도라는 장당 2판이 된다. 또한 공탁금은 내지않는다." },
  no_ron_pact: { use: "상시", text: "매 국 첫 6순동안 론 당하지 않는다. 단, 리치, 후로(안깡 포함)시 조약이 사라진다." },
  ...
  peek_riichi_waits: { use: "매 국 1회", text: "리치한 상대의 오름패를 확인하고, 한번 더 사용시 내 손패 한 장을 간파해둔 오름패로 바꾼다." },
  polar_ends: { use: "상시", text: "1과 9의 구별이 사라진다 — 같은 무늬의 199·911도 한 커쯔." },
  pond_snatch: { use: MODE_3_5, text: "방금 쯔모한 패 대신 상대 셋의 최근 버림패 9장중 1장을 훔쳐온다. 단 후리텐이면 쯔모로 화료할 수 없다." },
  ...
  riichi_upgrade: { use: "상시", text: "내 리치는 언제나 더블리치로 취급된다. 또한 내 하가는 그 국에 리치를 봉인당한다. 만약 이미 더블리치라면 트리플리치로 취급된다." },
  ...
  siege_riichi: { use: "상시", text: "텐파이가 아니어도 가짜 리치를 걸 수 있다(그 상태로는 화료 불가)" },
  silent_pact: { use: "매 국 1회", text: "멘젠을 깨지않는 후로 1회를 할 수 있다. 이후 평범한 퐁,치,대명깡을 더 하면 멘젠은 깨진다." },
  silent_swap: { use: "매 국 1회", text: "아무도 리치를 걸지않은 국에 쯔모패 대신 상대 바닥에서 1장을 가져올 수 있다. 후리텐이더라도 가져온패로 쯔모가 가능하다." },
  ...
  table_flip: { use: "매 국 1회", text: "국 첫 순에 손패를 통째로 뒤엎는다. 반납한 손패는 전원에게 공개된다." },
  ...
  ura_peek: { use: "매 국 1회", text: "뒷도라 표시패를 나만 확인하고, 한 번은 내 손패 하나를 선택하여 바꿔치기한다." },
  void_kan: { use: "상시(리치 중 X)", text: "리치를 안 걸었고 텐파이면, 상대의 깡을 오름패와 상관없이 창깡으로 가로챈다." },
  wind_lineage: { use: "상시", text: "자패의 슌쯔가 허용된다. - 자풍,장풍이 포함된 슌쯔와 백발중 슌쯔는 각각 1판" },

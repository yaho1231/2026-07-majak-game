 ...
 *
 * (하한 MIN_SCALE에 걸리는 아주 작은 창·아주 납작한 창만 예외 — 그쪽은 styles.css 의
 *  `@container ui (max-height: …)` 안전망이 띠를 함께 줄여 받는다.)
 */
const REF_W = 1920;
const REF_H = 1080;
...
/**
 * 배율 하한. 더 줄이면 글자가 안 읽힌다(15px 본문 → 11.25px). 1440×810보다 작은 창과
 * 아주 납작한 창(예: 1920×600)이 여기 걸린다 — 그 구간은 배율로 다 못 풀므로
 * styles.css 의 `@container ui (max-height: …)` 가 띠를 함께 줄여 받는다.
 */

...
export const GLOSSARY: readonly GlossaryEntry[] = [
  ...
  },
  {
    ...
    group: "seat",
    label: "대면",
    short: "내 반대편(정면)에 위치한 사람. '대가'라고도 한다.",
  ...
  },
  {
    ...
    long:
      "아무도 이기지 못한 채 뽑을 패가 떨어져 국이 끝나는 것. 이때 완성 직전이던 사람과 아니던 사람 사이에 3,000점이 오간다 — 아무도 완성 직전이 아니거나 모두가 그렇다면 점수는 움직이지 않는다.",
    match: ["황패유국", "유국(?![만])"],
  },
  {
    key: "nagashi",
  ...
  // ── 리치 ──────────────────────────────────────────────────
  {
    ...
    long:
      "완성 직전에 1,000점을 판에 걸고 하는 선언이다. 선언한 뒤에는 뽑은 패를 그대로 버려야 하고 손을 바꿀 수 없다 — 그 대신 점수가 크게 붙고, 이겼을 때 뒷도라까지 열어 본다. 남이 보기에도 위험한 사람이 되므로 그때부터 아무도 위험한 패를 안 버린다.",
    match: ["리치(?![봉])"],
  },
  {
    key: "double_riichi",
  ...
  },
  {
    ...
    group: "riichi",
    label: "공탁",
    short: "리치할 때 판에 내놓는 1,000점. 다음에 이긴 사람이 통째로 가져간다.",
    match: ["공탁금", "공탁", "리치봉"],
  },
  ...
  },
  {
    ...
    group: "score",
    label: "유국역만",
    short: "요구패만 흘리고 아무도 울지 않은 채 유국하면 역만으로 값하는 것.",
   ...
   */
  {
    ...
    group: "score",
    label: "우마",
    short: "순위에 따라 주고받는 보너스. 1위 +15 · 2위 +5 · 3위 −5 · 4위 −15.",
    long:
      "종국 점수를 그대로 성적으로 쓰면 '4위지만 점수는 많다' 같은 어정쩡한 결과가 나온다. 그래서 순위에 따라 정해진 크기를 더하고 뺀다 — 이 게임은 1위 +15, 2위 +5, 3위 −5, 4위 −15다. 주고받는 총합이 0이라 아무 점수도 새로 생기지 않는다. 결과 화면의 성적은 (최종 점수 − 시작 점수 25,000)을 1,000으로 나눈 값에 이 우마를 더한 것이다. 그래서 25,000점으로 끝난 1위는 ±0이 아니라 +15가 된다.",
    hudOnly: true,
  },
  {
    key: "oka",
    group: "score",
    label: "오카",
    short: "1위가 혼자 가져가는 추가 보너스. 이 서버에서는 0이라 실제로는 오가지 않는다.",
    long:
      "다른 마작에서는 성적을 반환점(30,000)을 기준으로 세고, 그때 붕 뜨는 5,000점 × 4명 = 20,000점을 1위가 통째로 가져간다. 이것을 오카라고 한다. 이 게임은 그 방식을 쓰지 않는다 — 오카가 0이고, 성적의 기준선도 반환점이 아니라 시작 점수 25,000이다. 그래서 결과 화면에 오카 칩이 뜨지 않고, 1위와 2위의 간격도 우마 차이(+15 대 +5)만큼이다. 다른 마작을 하다 온 사람이 '1위 하면 20,000점을 더 받는다'로 계산하면 순위 다툼의 크기를 크게 잘못 잡게 된다.",
    hudOnly: true,
  },
  {
    key: "return_score",
    group: "score",
    label: "반환점",
    short: "서든데스 판정의 기준점(30,000). 성적은 시작 점수 25,000을 기준으로 센다.",
    long:
      "연장전(서든데스)에 들어갈지를 가르는 선이다. 마지막 국이 끝났을 때 아무도 30,000점에 닿지 못했으면 판이 이어진다. 성적 계산과는 관계가 없다 — 성적의 기준선은 시작 점수 25,000이다.",
    match: ["반환점", "원점"],
    hudOnly: true,
  },
  ...
  // 찍는 말이라, 증강 텍스트를 코퍼스로 보는 미사용 검사에 걸릴 수가 없다.
  {
    ...
    group: "system",
    label: "증강",
    short: "규칙 자체를 바꾸는 카드. 판마다 골라 쌓아 가는 이 게임의 정체다.",
    long:
      "평범한 마작 위에 얹는 규칙 변경 카드다. 게임 중 몇 번 선택 화면이 뜨고, 고른 것은 그 판이 끝날 때까지 계속 따라온다. 내 손을 강하게 만드는 것도 있고, 남의 손을 망가뜨리거나 점수 계산 자체를 비트는 것도 있다. 상대가 무엇을 골랐는지는 대개 보이지 않는다.",
    match: ["증강"],
    hudOnly: true,
  },
  {
    ...
    group: "system",
    label: "드래프트",
    short: "증강 세 장 중 하나를 고르는 선택 화면. 정해진 국에 판 전체가 멈추고 열린다.",
    long:
      "증강을 얻는 자리다. 정해진 국(동1국 시작, 동3국…)에 판 전체가 멈추고 네 사람이 동시에 세 장 중 하나를 고른다. 제한 시간이 지나면 서버가 무작위로 하나를 골라 진행하므로, 시간 안에 반드시 누르는 편이 좋다. 자리마다 한 번씩 '새로고침'으로 카드 한 장을 다른 것으로 바꿀 수 있다.",
    match: ["드래프트", "증강 선택"],
    hudOnly: true,
  },
  {
    ...
    long:
      "증강은 크게 둘이다. 대부분은 **상시형**이라 조건이 맞는 순간 저절로 작동하지만, 액티브는 내 차례에 직접 눌러야 한다 — 액션 메뉴에 그 이름이 뜬다. 언제 쓰느냐가 곧 실력이 되는 쪽이고, 안 쓰면 아무 일도 일어나지 않는다.",
    match: ["액티브", "상시형"],
    hudOnly: true,
  },
  {
  ...
  },
  {
    ...
    group: "system",
    label: "퀘스트",
    short: "적힌 조건을 채우면 그때부터 효과가 열리는 증강. 배지에 🎯로 뜬다.",
    long:
      "받자마자 일하지 않는 증강이다. '이 판에서 ○○을 몇 번 하라' 같은 조건이 붙어 있고, 그것을 채운 뒤부터 진짜 효과가 열린다. 조건을 못 채우면 그 판 내내 아무 일도 하지 않으므로, 고를 때 내 손이 그 조건으로 갈 수 있는지부터 본다.",
    hudOnly: true,
  },
  {
    key: "disarm",
  ...
  },
  {
    ...
    group: "system",
    label: "재장전",
    short: "다 쓴 증강을 다시 쓸 수 있게 되돌린다 (쿨다운·사용 횟수 회복).",
    hudOnly: true,
  },

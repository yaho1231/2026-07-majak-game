 ...
 *
 * ⚠ 이 문단은 오랫동안 **오카를 "안 쓰는 말"의 예로 들고 있었다.** 사실이 아니었다 —
 * 결과 화면이 "우마 +15 · 오카 +20"을 그대로 찍는다(감사 §3-10). 주석이 코드보다
 * 뒤처지면 그 주석을 믿은 사람이 같은 구멍을 다시 판다. 지금은 실려 있다.
 *
...
export const GLOSSARY: readonly GlossaryEntry[] = [
   ...
   * ("우마 +15 · 오카 +20"). 25000점으로 끝난 사람이 왜 +35가 되는지 물을 곳이
   * 없었다는 뜻이다. 이 파일 머리말은 오랫동안 "오카는 한 번도 안 쓰는 말이라
   * 뺐다"고 적고 있었는데 **주석이 낡았던 것**이다 — 결과 화면이 쓰고 있었다.
   */
  {
    key: "uma",
    group: "score",
    ...
    long:
      "종국 점수를 그대로 성적으로 쓰면 '4위지만 점수는 많다' 같은 어정쩡한 결과가 나온다. 그래서 순위에 따라 정해진 크기를 더하고 뺀다 — 이 게임은 1위 +15, 2위 +5, 3위 −5, 4위 −15다. 주고받는 총합이 0이라 아무 점수도 새로 생기지 않는다. 이 값이 있어서 마지막 국의 순위 다툼이 점수 다툼보다 중요해진다.",
    hudOnly: true,
  },
  {
    key: "oka",
    group: "score",
    label: "오카",
    short: "1위가 혼자 가져가는 추가 보너스. 반환점을 넘긴 몫을 1위에게 몰아 준다.",
    long:
      "모두 25000점으로 시작하지만 성적은 30000점(반환점)을 기준으로 센다. 그 차이 5000점 × 4명 = 20000점이 붕 뜨는데, 이것을 1위가 통째로 가져간다. 그래서 2위와 1위 사이의 간격이 다른 순위 사이보다 훨씬 크다 — 이 게임에서 '어떻게든 1위'가 되는 이유다.",
    hudOnly: true,
  },
  {
    key: "return_score",
    group: "score",
    label: "반환점",
    short: "성적을 셀 때 기준이 되는 점수(30000). 시작 점수 25000과 다르다.",
    match: ["반환점", "원점"],
    hudOnly: true,
  },

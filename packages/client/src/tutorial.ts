...
export interface CoachCtx {
   ...
   * 정산 화면 위에 "리치를 걸었습니다. 이제 기다립니다. 누군가 오름패를 버리면 «론»
...
/**
 * **리치 모드** — 리치 버튼을 누른 뒤, 이제 낼 패를 고르는 상태. 액션 바가 리치
 * 버튼 자리에 «취소»를 세우는 것이 그 표식이다(App.tsx `ActionBar`의 `riichiMode`).
 */
const RIICHI_MODE = ".action-bar .act-cancel";
/** 방금 가져온 그 한 장 — 손패 오른쪽 끝에 떨어져 있는 패 (App.tsx가 `hand-drawn`을 붙인다) */
const DRAWN_HAND_TILE = ".own-hand .hand-drawn";
...
export const LESSONS: readonly Lesson[] = [
  {
    ...
    urgent: true,
    chapter: "시작",
    title: "봇 3명과 함께 한 판 해 봅니다",
    body: "화면에서 무엇을 눌러야 하는지 순서대로 알려 드립니다. 규칙을 다 몰라도 됩니다. 이 게임은 튜토리얼이라서 시간 제한이 없으니 천천히 보세요.",
    when: (c) => c.view !== null,
  },
  ...
  // ─────────── 증강 고르기 — 창이 떠 있는 동안에만 할 수 있는 이야기 ───────────
  {
     ...
     */
    body: "국이 시작될 때마다 세 장 중 하나를 고릅니다. 증강은 이 게임의 규칙 자체를 바꾸는 능력입니다. 실제 대국에서는 세 장 중 아무거나 고를 수 있지만, 튜토리얼에서는 설명하기 좋은 연금술사 하나로 고정해 두었습니다.",
    anchor: ".draft-panel",
    when: (c) => c.draftOpen,
  },
  {
    ...
    urgent: true,
    chapter: "증강 고르기",
    title: "카드 설명을 더 펼쳐 볼 수 있습니다",
    body: "카드에 보이는 것은 한 줄 요약입니다. 조건과 예외까지 적힌 전체 설명은 따로 볼 수 있습니다.",
  ...
  },
  {
    ...
    urgent: true,
    chapter: "증강 고르기",
    title: "마음에 안 들면 한 번 바꿀 수 있습니다",
    body: "카드 아래 새로고침 버튼(↻)을 누르면 그 카드를 새로 뽑습니다. 카드마다 한 번만 할 수 있고, 되돌릴 수 없습니다.",
    anchor: ".draft-cards",
  ...
  },
  {
     ...
     */
    body: "가운데와 오른쪽 카드는 어둡게 잠겨 있습니다. 튜토리얼에서는 연금술사 하나로 고정해 두었기 때문에 눌러도 선택되지 않습니다. 제한 시간도 없으니 세 장을 다 읽어 보고 고르세요.",
    todo: "밝게 표시된 연금술사 카드를 눌러 고르세요.",
    anchor: ".draft-cards",
    when: (c) => c.draftOpen,
    done: (c) => !c.draftOpen,
  },
  ...
  // ─────────── 기본 조작 ───────────
  {
    ...
    /*
     * ⚠ "13장을 쥐고 시작합니다"만 적어 두면 **화면과 어긋난다**: 이 강의가 뜨는
     * 시점은 이미 내 차례라 오른쪽 끝에 한 장이 더 붙어 열네 장이다(2026-08-18 실측).
     * 세어 보는 사람에게 첫 문장부터 틀린 셈이라, 그 한 장이 무엇인지까지 말한다.
     */
    body: "처음에는 13장을 들고 시작하고, 내 차례가 되면 한 장을 더 가져와 14장이 됩니다. 오른쪽 끝에 떨어져 있는 패가 방금 가져온 패입니다. 지금은 자동정렬이 켜져 있어 만·통·삭·자패 순서로 저절로 정리됩니다. 순서를 직접 정하고 싶으면 왼쪽 아래 자동정렬을 끄세요. 그러면 패를 끌어서 순서를 바꿀 수 있습니다.",
    anchor: ".own-hand",
  ...
  },
  {
    ...
    id: "discard-script",
    chapter: "기본 조작",
    title: "내 차례입니다. 이번에는 9삭을 버립니다",
    body: "손패에서 쓸모없는 패는 1삭과 9삭입니다. 1삭은 2삭 두 장과 이어질 수 있지만, 9삭은 어떤 패와도 이어지지 않습니다. 그래서 9삭을 버립니다.",
    /*
     ...
     */
    todo: "빛나는 9삭(대나무 아홉 개)을 눌러서 버리거나, 바닥으로 끌어다 놓으세요. 폰·태블릿에서는 한 번 눌러 패를 들어 올린 뒤 한 번 더 눌러야 버려집니다.",
  ...
  },
  {
    ...
    id: "discard",
    chapter: "기본 조작",
    title: "내 차례입니다. 한 장을 버립니다",
    body: "오른쪽 끝에 떨어져 있는 패가 방금 가져온 패입니다. 한 장 가져오고 한 장 버리기를 반복하며 손패를 완성해 갑니다.",
   ...
   */
  {
    ...
    body: "기다리던 패를 상대가 버렸습니다. 론 버튼을 누르면 그 사람에게서 점수를 받고 국이 끝납니다. 누르지 않고 넘기면 후리텐이 되어 이 국에서는 다시 론을 할 수 없습니다.",
    todo: "빨간 론 버튼을 누르세요.",
    anchor: ".act-win",
  ...
  },
  {
    ...
    body: "손패가 완성됐습니다. 빨간 쯔모 버튼을 누르면 이 국이 끝나고 세 사람 모두에게서 점수를 받습니다.",
    todo: "쯔모 버튼을 누르세요.",
    anchor: ".act-win",
    /*
  ...
  },
  {
    ...
    urgent: true,
    chapter: "증강 읽기",
    title: "보라색 패는 증강이 만들어 낸 패입니다",
    body: "원래 패산에 있던 136장이 아니라 증강이 새로 만든 패입니다. 노란색으로 반짝이는 패는 도라, 보라색으로 반짝이는 패는 생성패입니다.",
  ...
  {
    /**
     * **대본 3 — 리치.** 텐파이가 서면 곧바로 여기로 온다. 두 마디로 끊는다.
     *
     * 한 마디였을 때는 딤이 **리치 버튼**만 비춘 채로 "누른 뒤 가져온 패를 버리세요"까지
     * 말했다. 그런데 튜토리얼에서는 밝은 구멍 밖은 눌리지 않으므로(App.tsx
     * `FirstRunCoach`의 `coach-block`), 버튼을 누른 뒤 정작 버려야 할 패가 어두운
     * 쪽에 잠겨 있게 된다. 누를 곳이 둘이면 마디도 둘이어야 한다(연금술사와 같다).
     */
    id: "riichi",
    urgent: true,
    chapter: "한 판 끝내기",
    title: "텐파이입니다. 리치를 걸어 보세요",
    body: "한 장만 더 맞으면 완성이라는 뜻입니다. 리치는 1,000점을 걸고 점수를 크게 올립니다. 대신 리치 뒤에는 손패를 바꿀 수 없어, 가져온 패를 그대로 버리게 됩니다.",
    todo: "빛나는 리치 버튼을 누르세요.",
    anchor: ".act-riichi",
    // 가져온 패 말고는 못 버리게 한다 — 옆의 4통·7통을 버리면 텐파이는 유지되지만
    // 그게 곧 내 오름패라 후리텐이 된다(`DRAWN_TILE` 주석).
    lock: { kind: DRAWN_TILE, how: "discard" },
    /*
     * **대본의 연금술 마디를 앞지르지 않는다.**
     *
     * 운 좋게 2삭을 뽑으면 연금술 없이도 텐파이가 서는데(실측), 그러면 리치가 먼저
     * 열려 코치가 그쪽을 집는다. 리치는 끼어드는 강의라 한 번 잡으면 연금술 마디가
     * 뒤로 밀리고, 리치를 걸고 나면 그 마디는 조건상 **영영 안 나온다**
     * (손이 잠긴 뒤라 증강 대상을 고를 수 없다).
     *
     * 그래서 "아직 가르칠 연금술이 남아 있으면" 리치는 한 박자 기다린다. 판이 막히는
     * 것은 아니다 — 리치 버튼은 그대로 눌린다. 코치가 말하는 순서만 바뀐다.
     */
    when: (c) =>
      c.optionTypes.has("riichi") &&
      (c.seen.has("aug-script") || !c.augmentReady || !c.handKinds.has(SCRIPT_ALCHEMY)),
    // 버튼을 눌러 리치 모드에 들어갔으면 다음 마디로 — 이미 걸었거나 기회가 지났어도 끝.
    done: (c) => c.riichiDeclared || c.hit(RIICHI_MODE) || !c.optionTypes.has("riichi"),
  },
  {
    /** **대본 3-2** — 리치 모드에 들어온 뒤. 이제 비출 곳은 방금 가져온 그 한 장이다. */
    id: "riichi-pick",
    urgent: true,
    chapter: "한 판 끝내기",
    title: "이제 방금 가져온 패를 버리면 리치가 걸립니다",
    body: "오른쪽 끝에 떨어져 있는 패가 방금 가져온 패입니다. 그 패를 버려도 텐파이가 그대로 유지됩니다. 옆의 다른 패를 버리면 내 오름패를 버리는 셈이라 론을 할 수 없게 되니, 지금은 이 패만 누를 수 있게 해 두었습니다.",
    todo: "오른쪽 끝의 빛나는 패를 눌러서 버리거나, 바닥으로 끌어다 놓으세요.",
    anchor: DRAWN_HAND_TILE,
    lock: { kind: DRAWN_TILE, how: "discard" },
    when: (c) => c.hit(RIICHI_MODE) && c.optionTypes.has("riichi") && !c.riichiDeclared,
    done: (c) => c.riichiDeclared || !c.hit(RIICHI_MODE),
  },
  {
    /**
    ...
    id: "riichi-wait",
    chapter: "한 판 끝내기",
    title: "리치를 걸었습니다. 이제 기다립니다",
    body: "리치 뒤에는 손패를 바꿀 수 없어 가져온 패를 그대로 버리게 됩니다. 이제 할 일은 하나뿐입니다. 상대 셋 중 누군가가 내 오름패를 버리면 빨간 론 버튼이 나타납니다. 그때 누르면 국이 끝납니다.",
  ...
  },
  {
    ...
    // 같은 종류의 후로는 버튼 하나로 접힌다 — 방법이 여럿이면 누른 뒤 손패에서 함께 쓸 패를 고른다
    // (2026-09-25, docs/59 U56). 버튼만 누르면 끝난다고 배운 사람이 멈춘 화면 앞에서 헤매지 않게 여기서 말한다.
    body: "치·퐁은 상대가 버린 패로 몸통(멘쯔)을 완성하는 것입니다. 대신 손패 일부가 공개되어 리치를 걸 수 없게 됩니다. 만드는 방법이 여럿이면 치·퐁을 누른 뒤 함께 쓸 손패를 고르세요. 필요 없으면 패스를 누르세요.",
    anchor: ".action-bar",
...

  {
    ...
    body: "그 패의 다음 패가 도라입니다. 도라를 한 장 가지고 있을 때마다 점수가 한 단계씩 올라갑니다. 도라는 손패에서 노란색으로 계속 반짝입니다.",
    anchor: ".dora-slot",
  ...
  },
  {
    ...
    body: "누가 무엇을 버렸는지 전부 남습니다. 패 오른쪽 아래 동그라미는 가져온 패를 그대로 버렸다는 표시(쯔모기리)이고, 점선은 그 패가 상대 손패의 어느 자리에서 나왔는지 알려 줍니다.",
    anchor: ".river-bottom",
  ...
  // ─────────── 증강 읽기 — 이 게임의 정체 ───────────
  {
    ...
    // 마우스가 없는 기기에는 hover가 없다 — 거기서는 한 번 눌러야 열린다
    // (styles.css: `.aug-pill:focus .aug-tip`). 한쪽만 적으면 폰에서는 막힌다.
    todo: "증강 이름에 마우스를 올려 보세요. 폰·태블릿에서는 한 번 누르면 열립니다.",
  ...
  },
  {
    id: "aug-pin",
    chapter: "증강 읽기",
    title: "눌러 두면 설명이 고정됩니다",
    body: "마우스를 떼면 설명이 사라져서, 읽는 동안에는 판을 볼 수 없습니다. 눌러서 고정하면 설명과 판을 나란히 볼 수 있습니다. 여러 개를 동시에 고정할 수 있고, 다시 누르거나 Esc를 누르면 해제됩니다.",
    todo: "증강 이름을 한 번 눌러 설명을 고정해 보세요. 고정되면 📌 표시가 붙습니다.",
    anchor: ".own-top-main .np-augs",
    when: (c) => !c.draftOpen && c.hit(MY_PILLS) && afterFirstDiscard(c),
  ...
  },
  {
    id: "aug-detail",
    chapter: "증강 읽기",
    title: "설명은 요약과 상세 두 단계입니다",
    // 마지막 줄은 **치우는 법**이다. 앞 강의가 시켜서 고정해 둔 상자가 판 한가운데
    // 남아 남은 튜토리얼 내내 바닥을 가린다(2026-08-18 실측). 여는 법을 가르쳤으면
    // 닫는 법도 같은 자리에서 말해야 한다.
    body: "펼쳐진 설명 안의 ‘자세히’ 버튼을 누르면 작동 원리와 주의점까지 나옵니다. Shift 키를 누르고 있는 동안에도 같은 내용이 펼쳐집니다. 다 읽었으면 Esc를 누르거나 증강 이름을 다시 눌러 닫으세요.",
    todo: "고정해 둔 설명에서 ‘자세히’ 버튼을 눌러 보세요.",
    /*
     * **눌러야 할 것이 있는 상자**를 비춘다 — 펼쳐진 설명 그 자체다.
     *
     * 이름표 알약 줄(`np-augs`)을 가리키고 있었는데, 정작 눌러야 할 «자세히 ▾»는
     * 그 위에 뜬 설명 상자 안이라 딤에 잠겨 있었다(2026-08-18 실측) — 연금술사
     * 버튼에서 겪은 것과 같은 실수다.
     *
     * ⚠ `"A, B"`처럼 둘을 적어 "A가 있으면 A"를 노리는 것은 여기서는 안 통한다.
     * `rectOf`는 **문서 순서로** 첫 번째를 집는데 설명 상자는 알약 **안에** 있어서,
     * 언제나 바깥쪽(알약 줄)이 먼저 걸린다. 이 강의는 설명이 떠 있을 때만 서므로
     * (`when`) 상자 하나만 가리키면 된다.
     */
    anchor: MY_TIP,
  ...
  },
  {
    id: "aug-others",
    chapter: "증강 읽기",
    title: "상대 이름표의 표시도 알아 두세요",
    body: "🔒는 상대의 무장해제로 이번 국에만 잠긴 증강입니다. 🕐N국은 쿨다운, ♻는 재장전으로 되살린 증강, 🎲는 주사위로 얻은 증강입니다. 🎯 퀘스트 증강은 게이지가 조건 진행도를 보여 줍니다.",
    anchor: ".nameplate",
    when: (c) => !c.draftOpen && handsFree(c),
  },

  // ─────────── 화면 도구 — 좌하단 토글 → 오른쪽 위 셋(📖·📘·⚙) 순 ───────────
  /*
   * 예전에는 여기 «배율 +/−» 단계가 있었다. 2026-08-24에 그 손잡이를 없앴다 —
   * 배율은 이제 창 크기에서 자동으로 나온다(uiScale.ts). 가리킬 버튼이 없으므로
   * 단계도 함께 뺀다. 그래도 화면이 안 맞는 사람에게는 `LayoutHint`(App.tsx)가
   * 브라우저 확대를 안내한다.
   */
  {
    /*
     * 좌하단 빠른 토글 — **화면 도구 중 맨 먼저** 짚는다.
     *
     * 자리 때문이다. 이 줄은 손패 바로 옆(왼쪽 아래)이고 나머지 셋(📖·📘·⚙)은 전부
     * 오른쪽 위다. 예전 순서(📖 → 📘 → 좌하단 → ⚙)는 시선을 오른쪽 위로 보냈다가
     * 왼쪽 아래로 내렸다가 다시 오른쪽 위로 올렸다 — "왔다갔다가 어지럽다"
     * (2026-09-15 사용자 지적). 손 옆의 것을 먼저 끝내고, 그다음 오른쪽 위 셋을
     * 한 줄로 짚는다.
     *
    ...
    id: "quick-toggles",
    chapter: "화면 도구",
    title: "왼쪽 아래 버튼 네 개는 게임 중에 바로 켜고 끌 수 있습니다",
    body: "자동정렬을 끄면 손패를 끌어 순서를 바꿀 수 있고, 후로없음을 켜면 치·퐁 기회를 자동으로 넘깁니다. 자동화료와 자동버림은 버튼을 대신 눌러 주는 기능이니 배우는 동안에는 꺼 두세요. 좁은 화면에서는 이 버튼들이 손패 바로 위에 표시됩니다.",
    todo: "자동정렬을 한 번 눌러 꺼 보세요. 그러면 손패를 직접 끌어서 옮길 수 있습니다.",
  ...
  },
  {
    id: "settings",
    chapter: "화면 도구",
    title: "⚙ 설정에서 조작 방식, 소리, 효과를 바꿀 수 있습니다",
    /*
     ...
     * 게임 무효 요청.
     */
    body: "패를 버리는 방식(두 번 눌러 버리기, 우클릭 쯔모기리), 내 오름패 표시, 화면 효과와 소리, 연출 속도를 여기서 바꿀 수 있습니다. 맨 아래에는 게임 무효 요청도 있습니다. 자동정렬이나 자동화료는 여기가 아니라 아까 본 왼쪽 아래 버튼에 있습니다.",
    todo: "오른쪽 위 ⚙ 버튼을 눌러 열어 보세요.",
    anchor: ".settings-btn",
    when: (c) => !c.draftOpen && handsFree(c),
  ...
  {
    /*
     * **정산 화면에서만 코치가 통째로 막혀 있었다** (QA 4차 onboard 확정 3).
     *
     * 튜토리얼의 목표는 «리치 → 화료»인데, 정작 화료해서 점수가 오가는 그 화면에서는
     * `allowedNow`가 마무리 말고 어떤 강의도 못 뜨게 막았다. 화면에는 「핑후 1판 ·
     * 탕야오 1판 … 3판 30부」와 ±점수가 뜨는데, 처음 화료한 사람이 자기가 왜 8,700점을
     * 받았는지 물어볼 곳이 그 자리에 하나도 없었다.
     *
     * 막아 둔 이유 자체는 옳다 — 판의 사실만 보는 강의들이 국이 끝난 뒤에도 그대로
     * 성립해서 "이제 기다립니다"가 정산 화면 위에 떴다(`CoachCtx.roundOver` 주석).
     * 그래서 통째로 여는 대신 **정산 화면을 위해 쓴 강의 하나만** 더 통과시킨다.
     *
     * 마무리보다 **앞에** 둔다 — 둘 다 성립하는 순간이 오는데, 점수를 읽는 이야기가
     * "여기까지가 기본입니다"보다 뒤에 오면 판을 접은 뒤에 뜨게 된다.
     */
    id: "result-score",
    chapter: "한 판 끝내기",
    // 정산 화면은 곧 사라진다(다음 국). 읽던 강의를 밀어내고 끼어들어야 자리를 잡는다.
    urgent: true,
    title: "점수는 판과 부로 정해집니다",
    body: "가운데 원의 판 수는 역과 도라를 합한 값이고, 작은 글씨의 부 수는 손패 모양에 따라 붙는 추가 점수입니다. 이 둘로 실제 점수가 계산되고, 아래 표의 숫자가 이번에 실제로 오간 점수입니다. 역 이름과 판·부 표시를 누르면 설명이 뜹니다.",
    anchor: ".result-han-circle",
    when: (c) => c.roundOver && c.won,
  },
  {
    /*
    ...
    /*
     * **마지막에 실전과의 차이를 한 줄 붙인다** (QA 4차 onboard 확정 2).
     *
     * 이 판은 처음부터 끝까지 "시간 제한이 없습니다"라고 가르친다(`welcome`·`draft-pick`).
     * 그런데 실전은 한 결정에 30초고(`HumanAgent.DECISION_TIMEOUT_MS`), 넘기면 서버가
     * 대신 패스하거나 방금 가져온 패를 버린다. 그 사실을 튜토리얼이 한 번도 말하지
     * 않으면, 다 배우고 나간 사람이 첫 실전에서 배운 것과 정반대의 판을 만난다.
     * `codex` 강의가 같은 형태의 caveat("실제 대국에서는 …")를 이미 쓰고 있다.
     * 나가기 직전인 이 자리가 그 말을 들을 자리다.
     */
    body: "리치를 걸고 화료까지 해 보셨습니다. 실제 대국에서는 한 수마다 30초의 제한 시간이 있습니다. 시간이 다 되면 자동으로 패스하거나 방금 가져온 패를 그대로 버립니다. 증강 고르기는 첫 번째만 시간이 넉넉합니다. 나머지는 게임을 하면서 익히면 됩니다. 알겠어요 버튼을 누르면 이 판을 끝내고 처음 화면으로 돌아갑니다. 튜토리얼 버튼은 로그인 화면과 홈 양쪽에 있으니 언제든 다시 오세요.",
 ...
 *
 * **정산 화면을 위해 쓴 강의만** 통과한다(`RESULT_LESSONS`) — 점수 풀이와 마무리 둘.
 * 판의 사실만 보는 나머지 강의는 국이 끝난 뒤에도 그대로 성립하므로 전부 막는다
 * (`CoachCtx.roundOver` 주석). 강의마다 `when`에 조건을 하나씩 더
 * 다는 대신 픽커에서 한 번에 막는다 — 조건을 빠뜨린 강의가 하나만 있어도 같은 일이
 * 다시 생기고, 그건 강의를 새로 얹을 때마다 되풀이될 실수다.
 */
const RESULT_LESSONS: ReadonlySet<string> = new Set(["result-score", "outro"]);

function allowedNow(c: CoachCtx, lesson: Lesson): boolean {
  return !c.roundOver || RESULT_LESSONS.has(lesson.id);
}

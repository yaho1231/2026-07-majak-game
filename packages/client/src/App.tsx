      ...
      {auth === null && replayData !== null ? (
        ...
        <AuthScreen
          ...
          onGuest={() => {
             ...
             * 랜딩이 "체험은 설명 없이 바로 한 판"이라고 적어 놓았다(버튼 이름은
             * «체험하기»). 적어 놓은 것과
             * 실제가 달라지면 안 된다.
             *
 ...
 * 있었다. 2026-08-19 사용자 지시로 지웠다.
 *
 * 처음에는 이 그림들을 로그인 뒤 «증강 예시» 화면으로 옮길 생각이었고 계획
 * 문서까지 있었다. 그 계획도 같은 날 접었다 — **"창의력을 제한하는 것 같다"**
 * (사용자). 증강이 무엇을 하는지 미리 그림으로 다 보여 주면, 판 위에서
 * 알아내는 재미가 없어진다는 판단이다.
 *
 * 그러니 이 자리에 예시를 다시 세우지 마라. 증강 설명은 지금 있는 것 —
 * 드래프트 카드의 요약·설명, 도감의 상세 — 까지가 전부다.
 */
      ...
      <div className="lobby-card auth-card">
        ...
        {gateOn && tab === "register" ? (
          <p className="auth-gate-note">
            ...
            코드가 없다면 <b>계정 없이 시작</b>의 <b>체험하기</b>로 지금 플레이할 수 있습니다.
          </p>
        ) : null}
        ...
        {/* ⚠ 여기 «증강 예시» 패널(3종, 바뀌기 전 → 바뀐 뒤)이 있었다. 2026-08-19
            사용자 지시로 지웠다 — 옮기는 것이 아니라 없애는 것이다. 로그인 화면에는
            로고와 로그인 상자만 선다(위 LANDING_SHOWCASE 자리의 주석 참고). */}
      </div>
    </div>
  );
}
...
const HELP_BASICS: HelpSection[] = [
  ...
  },
  {
    ...
    paras: [
      ...
      // 서든데스(westEntry)와 토비가 두 모드 모두 켜져 있고, 같은 화면의 로비 툴팁은
 ...
 *
 * **서는 자리는 두 화면이 다르다**: 랜딩은 맨 위(그 화면은 카드 한 장이다), 홈은
 * **대국 카드 바로 위**다. 홈에서 맨 위는 로고 줄 위쪽 여백이라 아무도 안 본다
 * (홈 렌더의 주석 참고).
 *
  ...
  return (
    <div className="home">
      ...
      <main className="home-main">
        ...
        {/* 공지는 **대국 카드 바로 위**에 선다 (§4-3).

            예전에는 `.home`의 맨 위, 로고 줄보다도 위에 있었다. 거기는 사람이
            홈에 들어와서 보는 자리가 아니다 — 시선은 로고를 지나 바로 "＋ 방
            만들기"로 간다. 띠가 화면 꼭대기 여백에 얇게 얹혀 있어서 있어도 못
            봤고, 없어져도 몰랐다(2026-08-19 사용자 보고).

            이 자리는 **누르러 오는 것 바로 위**다. 공지가 서면 대국 카드가 그만큼
            내려가고, 내리면 원래 자리로 돌아온다 — 생기고 없어지는 것이 눈에
            보이는 자리다. 로그인 화면은 그대로 맨 위에 둔다(거기엔 이 열이 없다). */}
        <NoticeBanner notice={props.serverInfo?.notice} />
...
function PromptTimer(props: {
   ...
   *
   * 자리 순서는 «은행 + 유예»다 (2026-08-30 사용자 지시 2차). **매 순 초기화되며
   * 먼저 닳는 쪽(유예)이 뒤에 선다** — 앞자리는 이번 국 내내 거의 그대로 서 있는
   * 은행 잔액이라, 흐르는 숫자를 뒤에 둬야 「고정값 + 이번 순의 카운트다운」으로
   * 읽힌다. 반대로 뒀더니(1차 구현) 왕초보 방에서 「293 + 30초」처럼 앞이 흐르고
   * 뒤가 서서 어느 쪽이 이번 순의 시계인지가 뒤집혀 보였다.
   *   10 + 30초 → 10 + 29초 → … → 10 + 0초 → 9 + 0초 → …
  ...
  const countText =
    bankMs <= 0
      ...
      : grace > 0
        ? `${whole(bank)} + ${precise ? tenth(grace) : whole(grace)}초`
        : `${precise ? tenth(bank) : whole(bank)} + 0초`;
  /*
  ...
  return (
    <div className="overlay">
      <div className="gameover-panel">
        <h2>대국 종료</h2>
        {/* 왜 끝났는지 — 남2국에서 갑자기 순위표가 뜨면(토비) 버그로 읽혔다.

 ...
 *
 * 규칙(사용자 확정):
 * - 숙련자 = 종전 그대로 (타패 5 + 30초, 증강 30초/첫 판 75초).
 * - 초심자 = 증강 120초, 타패 60 + 10초.
...
describe("ROOM_PACES — 사용자가 못박은 값", () => {
  it("숙련자는 종전 상수 그대로다", () => {
    expect(ROOM_PACES.expert).toEqual({
      ...
      turnBankMs: TURN_BANK_MS,
      draftMs: DECISION_TIMEOUT_MS,
      firstDraftMs: FIRST_DRAFT_TIMEOUT_MS,
    });
  });

  it("초심자 = 증강 120초 · 타패 60 + 10초", () => {
  ...
  it("좌석 최대 대기는 컨트롤러 그물(90초)을 넘어설 수 있음을 알린다", () => {
    ...
    expect(paceMaxSeatMs("expert")).toBe(75_000);
...
describe("HumanAgent.setPace", () => {
  ...
  it("기본(안 꽂으면)은 숙련자 — 종전 마감 그대로", () => {
    ...
    expect(turnDeadline(agent, sock)).toBe(TURN_GRACE_MS + TURN_BANK_MS);
  });

  it("초심자 좌석은 타패 70초(60 + 은행 10) · 증강 120초를 받는다", () => {
    const sock = new FakeSocket();
    const agent = new HumanAgent("p0", "Alice", sock.asWs());
    agent.setPace("beginner");
    expect(turnDeadline(agent, sock)).toBe(70_000);
  ...
  it("은행은 그 속도의 값으로 다시 찬다 (resetBank)", async () => {
    ...
    expect(turnDeadline(agent, sock)).toBe(70_000 - 5_000);
    agent.resetBank();
    expect(turnDeadline(agent, sock)).toBe(70_000);
  });

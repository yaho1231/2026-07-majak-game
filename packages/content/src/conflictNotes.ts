...
const CONFLICT_REASON: Record<string, string> = {
  die_hard:
    "넷 다 크게 잃는 국을 막는 방어 증강이어서 이 증강의 발동 조건이 생기지 않는다.",
  yakuman_shield: "둘 다 정산 마지막에 손실을 되돌리기 때문에 서로의 결과가 충돌한다.",
  true_dragon:
    "국사·치또이·구련 계열과 성립하지 않는 깡은 14장·몸통 4개를 전제로 한다. 손패 교환은 17장인 손과 바꿀 상대가 없어 발동하지 못한다.",
  stealth_riichi: "전부 내가 건 리치의 형태를 바꾸는 증강이어서 리치를 숨기는 효과와 맞지 않는다.",
  open_riichi_reveal:
    "전부 리치 중에 대기를 바꾸거나 리치 형태를 바꾸는 증강이어서 공개한 오름패가 맞지 않게 된다.",
  avenger: "둘 다 후리텐과 무역 상태에서도 론할 수 있게 하는 증강이어서 효과가 그대로 겹친다.",
  regret: "둘 다 유국 때의 손패를 다음 국 배패로 바꾸는 증강이어서 나중에 적용되는 쪽이 앞의 결과를 없앤다.",
  frame_up: "상대 바닥에 넣은 패 한 장 때문에 편식의 한 무늬 계산이 성립하지 않게 된다.",
  hidden_blade: "둘 다 리치 없이 뒷도라를 적용하는 증강이어서 멘젠 다마텐 론에서 효과가 겹친다.",
  free_riichi_discard: "둘 다 리치를 취소하는 증강인데, 이 증강은 선언 시점의 손패를 그 국이 끝날 때까지 고정한다.",
  seat_swap: "손패 장수가 다른 상대와는 바꿀 수 없어서, 17장짜리 손과 함께 가지면 발동할 수 없다.",
  full_hand_swap: "손패 장수가 다른 상대와는 바꿀 수 없어서, 17장짜리 손과 함께 가지면 발동할 수 없다.",
};
...
export function withConflictNotes(defs: readonly AugmentDef[]): AugmentDef[] {
  ...
  return defs.map((d) => {
    ...
    const line = `함께 가질 수 없는 증강: ${names}.${reason === undefined ? "" : ` ${reason}`}`;

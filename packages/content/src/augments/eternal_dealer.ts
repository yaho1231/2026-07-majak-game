...
export const eternalDealer: AugmentDef = defineAugment({
  ...
  name: "만년 오야",
  description:
    "(상시 · 연장은 게임 내 3회) 나는 계속 오야다 — 내가 화료하면 언제나 오야 점수(약 1.5배)로 계산되고, 내 자풍은 늘 '동'이라 역패 동이 항상 붙는다. 게다가 내가 화료하면 다음 국의 오야가 내 자리로 온다(게임 내 3회).",
  detail:
    "(상시 · 연장은 게임 내 3회) 세 가지가 한꺼번에 걸린다. ① 자리가 어디든 내 화료는 오야 화료로 계산되어 점수가 약 1.5배가 된다. ② 점수 계산에서 내 자풍이 항상 '동'이라 동을 커쯔로 모으면 늘 역패 1판이 붙고, 장풍까지 동인 국이면 더블동(2판)이 된다. ③ 내가 화료하면 다음 국의 오야가 내 자리로 옮겨 온다(연장). 다만 국이 무한히 늘어나지 않게 ③은 게임 내 3회까지만 발동하고, 남은 횟수는 전원에게 보인다. 내가 진짜 오야인 국에 화료한 것은 원래 규칙대로의 연장이므로 횟수를 쓰지 않는다.",
  install(ctx) {

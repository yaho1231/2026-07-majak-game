 ...
 *
 * 대응: 혼합 커쯔는 청일색·혼일색·삼색동각·또이또이 부수를 스스로 깨 화력이 낮다.
...
export const mixedTriplet: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 자신에게만 커쯔의 무늬 제한이 사라진다. 숫자만 같으면 커쯔가 되고 화료·텐파이·대기 판정과 펑·깡에 모두 적용되며, 슌쯔는 기존처럼 같은 무늬로만 만든다. 다만 혼색으로 만든 커쯔는 청일색·혼일색·삼색동각을 성립시키지 않고 또이또이·산안커의 부수도 낮아진다.",
  install(ctx) {

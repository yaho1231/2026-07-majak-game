...
export const cornucopia: AugmentDef = defineAugment({
  ...
  name: "수상한 주사위",
  description:
    "무작위 증강 2개를 획득한다. 무엇이 나왔는지는 전원에게 공개된다.",
  detail:
    "획득하는 순간 카탈로그에서 무작위로 뽑힌 증강 2개가 함께 지급된다.\n\n이미 보유한 증강, 보유 증강과 함께 가질 수 없는 증강, 이 모드에서 쓰이지 않는 증강은 후보에서 제외된다. 후보가 모자라면 1개만, 또는 하나도 지급되지 않을 수 있다. 지급은 획득 순간 한 번뿐이며, 지급된 증강은 전원에게 공개된다.",
  install(ctx) {
    const { holder } = ctx;

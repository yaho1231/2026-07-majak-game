...
export const joker: AugmentDef = defineAugment({
  ...
  detail:
    "(2국에 1회) 발동할 때 **손패 1장을 골라 백으로 바꾼다** — 백이 손에 없어도 조커를 만들 수 있다(이미 백인 패를 고르면 아무것도 바뀌지 않는다). 손에 있는 백은 전부 조커이고 머리도 몸통도 된다. 무엇이 될지는 고르지 않으며 화료하는 순간 가장 높은 점수가 나오는 형태가 채택된다. 조커가 넓힌 대기는 후리텐이 되지 않는다. 백으로 치·퐁·깡은 할 수 없고 도라는 백 그대로 세므로 도라를 백으로 바꾸면 그 판수는 사라진다.",
  install(ctx) {

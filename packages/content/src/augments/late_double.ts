...
export const lateDouble: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 7순 안에 선언한 리치는 모두 더블리치 2판으로 값하고, 그렇게 더블리치로 취급된 리치에는 화료 시 **+1판**이 더 붙는다(합계 3판 · 역만에는 미적용). 앞서 몇 장을 버렸거나 후로로 순서가 흐트러졌어도 상관없다. 이중 선언이 승격시킨 8순 이후의 리치에도 같은 +1판이 붙는다. 그 밖의 8순 이후 리치는 표준 판정 그대로라 보너스가 없다. 일발·천화 같은 다른 첫순 특전은 표준 그대로다.",
  install(ctx) {
    const { holder } = ctx;

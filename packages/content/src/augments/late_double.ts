...
export const lateDouble: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 승격된 리치는 화료 시 합계 **3판**이 된다(역만에는 미적용). 앞서 몇 장을 버렸거나 후로로 순서가 흐트러졌어도 상관없다. 이중 선언이 승격시킨 8순 이후의 리치에도 같은 +1판이 붙고, 그 밖의 8순 이후 리치는 표준 판정 그대로다. 일발·천화 같은 다른 첫순 특전은 표준 그대로다.",
  install(ctx) {
    const { holder } = ctx;

 ...
 *
 * 도파민 순간: 홀더의 점수봉이 문턱(4만 5천)에 다가서는 순간부터 전 테이블에 등정 게이지가 켜진다.
...
export const unification: AugmentDef = defineAugment({
  ...
  name: "천하통일",
  description:
    "(상시) 내 점수가 목표에 도달하면 남은 국을 전부 무시하고 즉시 우승으로 게임이 끝난다. 목표는 45000점에서 시작하지만, 증강이 나에게 얹어 준 점수만큼 함께 올라간다 — 마작으로 벌어야 한다.",
  detail:
    "(상시) 내 점수가 목표에 도달하면 그 국의 정산 직후 남은 국을 전부 무시하고 게임이 즉시 끝나며 내가 우승한다.\n\n목표는 45000점에서 시작한다. 다만 다른 증강이 정산에서 나에게 얹어 준 점수(유국역만·승승장구·배수·뱅크 발행 …)는 누적해서 목표를 그만큼 밀어 올린다 — 증강으로 번 점수는 문턱에 세지 않는다는 뜻이고, 실제로 필요한 것은 '마작으로 번 45000점'이다. 반대로 잃은 점수는 목표를 내리지 않는다.\n\n지금 넘어야 하는 목표 점수는 전원에게 공개된다.",
  install(ctx) {
    const { holder } = ctx;

    // 문턱 = 기본 4만 5천 + 증강이 지금까지 얹어 준 점수. 마작으로 번 4만 5천에만 반응한다.
    ...
    //  문턱은 아직 기본값 45000이라 설명과 어긋나지 않는다.)

...
export const unification: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 판정은 각 국의 정산 직후에 이루어진다. 끝내는 것이 내 능력이지 우승이 보장되지는 않는다 — 같은 정산에서 나보다 높은 사람이 있으면 그쪽이 1위다.\n\n점수의 출처는 따지지 않는다 — 다른 증강이 얹어 준 점수여도 점수판이 문턱에 닿으면 그대로 끝이다.\n\n문턱은 판 길이를 따라간다 — 반장전은 55,000점, 동풍전은 45,000점이다.",
  install(ctx) {
    const { holder } = ctx;

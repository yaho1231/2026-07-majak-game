 ...
 *
 * 동풍전 1·반장전 2회, 자기 턴에 선언하면 그 순간 **텐파이인 상대가 누구인지**가
 ...
 *
 * 구현:
 * - 커스텀 액션 `tenpai_scan_use {}` — turn.act·자기 턴·동풍전 1·반장전 2회.
...
/** 이미 이 게임에서 발동했는가 (게임 단위 — roundKey 없음) */
...
export const tenpaiScan: AugmentDef = defineAugment({
  ...
  detail:
    "(동풍전 1회 · 반장전 2회) 자기 순에 선언하면 그 순간 세 상대 중 텐파이인 사람이 누구인지가 오직 당신 화면에만 표시된다. 리치를 걸지 않은 다마텐도 잡아내지만 손패 내용이나 대기패까지는 알 수 없다. 결과는 선언한 순간의 스냅샷이며, 발동 사실도 밝혀진 목록도 상대에게는 공개되지 않는다.",
  ...
  // 상대들의 텐파이를 훑는 정보 증강(동풍전 1·반장전 2회). 텐파이 여부와 무관하게

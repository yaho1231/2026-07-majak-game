...
export const bloodContract: AugmentDef = defineAugment({
  ...
  detail:
    "(매 국 1회) 자신의 첫 타패 전에 지정 역 목록(탕야오·핑후·또이또이·혼일색·청일색·삼색·일기통관·치또이) 중 하나를 골라 전원에게 공개 계약한다. 그 국에 계약 역을 포함해 화료하면 획득 점수가 1.5배가 된다. 배수가 걸리는 것은 **손의 화료점뿐**이다 — 회수하는 리치봉과 본장 수령분은 그대로 더해진다. 계약은 국당 하나뿐이고 하지 않아도 되며, 무엇을 걸었는지는 전원에게 공개된다.",
  install(ctx) {

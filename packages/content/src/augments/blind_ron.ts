...
export const blindRon: AugmentDef = defineAugment({
  ...
  detail:
    "언제 켜지나 — 뽑는 순간이 아니라 그 다음에 시작되는 국 하나다. 켜지면 네 명 모두에게 공개되므로, 이 국의 론이 어디로 날아갈지 모른다는 사실은 전원이 알고 시작한다. 국이 끝나면 저절로 꺼지고 다시 켜지지 않는다.\n\n무엇이 바뀌나 — 점수 계산은 평소 그대로다. 바뀌는 것은 누가 무느냐 하나뿐이다. 화료자가 받는 점수도, 오간 점수의 총합도 변하지 않는다. 대상은 자리에 앉은 네 명 전원에서 고르며 보유자와 화료자도 빠지지 않는다 — 한 사람당 25%다.\n\n경계 — 쯔모에는 적용되지 않는다(애초에 쏜 사람이 없다). 한 사람이 더블론으로 둘에게 물어야 하면 그 지불 전체가 같은 한 명에게 통째로 옮겨 간다. 화료자 자신이 뽑히면 받을 점수를 자기가 내므로 그 화료의 증감은 0이 된다. 대상이 무는 금액은 본장 몫까지 포함해 쏜 사람이 낼 뻔한 전액 그대로이고, 책임지불(파오)로 쏜 사람이 절반만 무는 판이면 그 절반만 옮겨 간다.\n\n무엇이 걸린 판인가 — 이 국에는 안전패가 없다. 완전히 접고 아무 위험패도 안 버린 사람이, 남이 쏜 만관을 대신 무는 일이 실제로 일어난다. 반대로 밀고 나가는 쪽에는 방총의 두려움이 3/4만큼 옅어진 국이다. 보유자에게도 똑같이 적용되므로, 이 국에 크게 나올 손이 있을수록 스스로 위험해진다.",
  install(ctx) {
    const { holder } = ctx;

...
export const letItRide: AugmentDef = defineAugment({
  ...
  name: "판돈 굴리기",
  description:
    "(상시) 연속으로 화료할수록 내가 얻는 점수의 배수가 오른다 — 1번째 화료 1배, 2연속 2배, 3연속 3배, 4연속 이상 4배. 방총하거나 유국이면 다시 1배부터 시작한다.",
  detail:
    "(상시) 화료해서 점수를 얻을 때 그 획득분에 배수가 곱해진다. 첫 화료는 1배(그대로), 연속 두 번째 화료는 2배, 세 번째는 3배, 네 번째부터는 4배가 상한이다. 내가 방총하거나 유국으로 국이 끝나면 연승이 초기화되어 다음 화료는 다시 1배로 돌아가지만, 잃는 점수는 하나도 없다. 늘어난 몫은 뱅크가 지급하며 지금 몇 배인지는 전원에게 공개된다.",
  install(ctx) {
    const { holder } = ctx;

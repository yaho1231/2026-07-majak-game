...
export const soulHunt: AugmentDef = defineAugment({
  ...
  name: "혼 사냥",
  description:
    "(상시) 리치 중인 상대에게서 론하면 그의 리치를 통째로 강탈한다 — 리치를 걸지 않았어도 내 화료가 리치로 취급되며 뒷도라가 적용된다.",
  detail:
    "(상시) 리치 중인 상대의 버림패로 론하면 그 리치가 내 것이 된다. 내가 리치를 걸지 않았어도 화료가 리치로 취급되어 뒷도라가 뒤집히며, 리치 공탁은 화료자가 공탁을 가져가는 표준 정산으로 회수된다. 쯔모 화료나 리치를 걸지 않은 상대에게서 잡은 론에는 효과가 발생하지 않는다.",
  install(ctx) {
    const { holder } = ctx;

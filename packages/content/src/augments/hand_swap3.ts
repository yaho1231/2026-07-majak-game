...
export const handSwap3: AugmentDef = defineAugment({
  ...
  name: "등가교환",
  description:
    "(게임 내 2회 · 매 국 1회) 자기 순에 상대 한 명을 지정하면 그 손패가 나에게만 공개되고, 넘길 내 3장과 가져올 상대 3장을 각각 골라 통째로 맞바꾼다. 리치를 선언한 상대에게는 쓸 수 없다.",
  detail:
    "(게임 내 2회 · 매 국 1회) 자기 순에 상대 한 명을 지정하면 그 손패 전체가 나에게만 진짜 패로 공개된다. 이어서 넘길 내 3장과 가져올 상대 3장을 각각 한 번에 골라 맞바꾸며, 무작위 없이 전부 내가 고르고 양쪽 손패 장수도 그대로 유지된다. 리치한 상대는 지정할 수 없고 지정한 뒤 상대가 리치하면 교환이 중단된다. 다만 **숨은 리치는 리치가 아닌 사람으로 보이므로 그대로 지정할 수 있고**, 3장이 갈리는 순간 그 리치는 당사자에게만 알려진 채 풀린다.\n\n**횟수는 대상을 지정하는 순간 소비된다** — 교환을 끝내지 못하고 국이 끝나거나 상대가 리치를 걸면 그 1회는 돌아오지 않는다. 넘기기로 고른 3장은 아직 손에 있으므로 그중 한 장을 버리거나 깡에 써도 되며, 그러면 **그 선택이 무효가 되어 다시 고를 수 있다**.\n\n무엇이 오갔는지는 **당사자 둘에게만** 화면에 뜬다 — 제3자는 대상 지정 사실만 알 뿐 갈린 패는 보지 못한다.",
  install(ctx) {

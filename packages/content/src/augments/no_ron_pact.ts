...
export const noRonPact: AugmentDef = defineAugment({
  ...
  name: "불가침 조약",
  description:
    "(상시) 매 국 첫 6순 동안 론당하지 않는다. 단 리치·후로(안깡 포함)를 하면 조약이 사라진다.",
  detail:
    "파기 조건은 둘이다 — 리치를 걸거나, **손에 몸통이 하나라도 생기는 것**. 치·퐁·대명깡은 물론 안깡도, 멘젠이 유지되는 묵계 퐁도 파기 사유다.\n\n상대의 쯔모 화료나 유국 노텐 벌점은 막지 못한다. 조약이 유효한지, 파기·만료됐는지는 국 내내 전원에게 공개된다.",
  install(ctx) {
    const { holder } = ctx;

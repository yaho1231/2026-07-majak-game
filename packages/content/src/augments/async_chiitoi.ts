/**
 * 비대칭 치또이 (async_chiitoi, prism) — **무늬가 달라도 숫자가 같으면 치또이 머리다.**
...
export const asyncChiitoi: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 치또이쯔 7쌍이 무늬를 가리지 않는다. 1만+1통처럼 숫자만 같으면 한 쌍으로 인정되어, 원래는 성립할 수 없는 배열로도 리치가 걸린다. 자패는 기존처럼 같은 종류 2장이어야 하고, 같은 패는 최대 2장까지만 쓸 수 있다(4장을 2쌍으로 쓰지 못한다).",
  install(ctx) {

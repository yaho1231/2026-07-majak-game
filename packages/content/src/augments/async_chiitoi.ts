...
export const asyncChiitoi: AugmentDef = defineAugment({
  ...
  name: "비대칭",
  description:
    "(상시) 치또이쯔에서 무늬가 달라도 숫자가 같으면 한 쌍으로 인정된다 — 1만+1통도 머리다. 안커가 있으면 머리보다 안커로 우선 취급된다.",
  detail:
    "(상시) 치또이쯔 7쌍이 무늬를 가리지 않는다. 1만+1통처럼 숫자만 같으면 한 쌍으로 인정되어, 원래는 성립할 수 없는 배열로도 리치가 걸린다. 자패는 기존처럼 같은 종류 2장이어야 하고, 같은 패는 최대 2장까지만 쓸 수 있다(4장을 2쌍으로 쓰지 못한다). 같은 패가 3장 모이면 그 손은 치또이가 아니라 일반 손으로 읽힌다 — 안커가 있으면 머리보다 안커로 우선 취급된다.",
  install(ctx) {

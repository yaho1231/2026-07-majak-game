 ...
 *
 * 구현:
 * - 액션 payload가 `{}` → `{ rank }`(1~9)로 바뀌었다. 클라이언트는 실제 손패를 눌러
 *   숫자를 고른다(App.tsx ARM_MODE `red_touch: "hand"`) — 누른 패의 숫자가 선택이고, 같은
 *   숫자의 손패가 제자리에서 붉게 미리 보인 뒤 [확인]으로 낸다(2026-09-25, docs/59 U02.
 *   예전에는 숫자마다 손패를 다시 그린 전용 모달이었다).
...
export const redFiveTouch: AugmentDef = defineAugment({
  ...
  name: "붉은 손길",
  description:
    "(게임 내 1회) 자기 순에 숫자 하나(1~9)를 지정하면, 그 뒤로 게임이 끝날 때까지 내 손에 들어오는 그 숫자가 전부 적도라가 된다. 리치 중에는 사용할 수 없다. 지정한 숫자는 모두에게 공개된다.",
  detail:
    "숫자 하나(1~9)를 지정하면 그 뒤로 게임이 끝날 때까지 내 손에 들어오는 그 숫자가 전부 적도라가 된다. 발동 시점의 손패, 쯔모, 다음 국의 배패에도 적용된다.\n\n손에 없는 숫자는 고를 수 없다. 후로로 가져온 패에는 적용되지 않고, 내가 버린 패를 상대가 가져가도 상대에게는 적도라가 아니다.",

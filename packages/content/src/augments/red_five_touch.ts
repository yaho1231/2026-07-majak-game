...
export const redFiveTouch: AugmentDef = defineAugment({
  ...
  name: "붉은 손길",
  description:
    "(게임 내 1회 · 리치 중에는 쓸 수 없다) 자기 순에 숫자 하나(1~9)를 지정하면, 그 뒤로 내 손에 들어오는 그 숫자가 게임이 끝날 때까지 전부 적도라가 된다. 지정한 숫자는 전원에게 공개된다.",
  detail:
    "(게임 내 1회) 발동 시점의 손패, 이후의 쯔모, 다음 국 배패까지 계속 적용된다. 손에 없는 숫자는 고를 수 없다.\n\n이 적도라에는 소유자가 각인되어, 버린 패를 상대가 후로로 가져가도 상대의 점수로는 세지 않는다. 각인은 **손패에 들어올 때만** 붙어, 후로로 가져온 패에는 붙지 않는다.",

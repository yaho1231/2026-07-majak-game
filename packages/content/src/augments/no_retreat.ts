...
export const noRetreat: AugmentDef = defineAugment({
  ...
  name: "물러설 수 없는 선언",
  description:
    "(2국에 1회) 텐파이 상태에서 버릴 패를 골라 선언하는 리치다. 공탁 1,000점을 내지 않고, 그 국의 리치·일발이 각각 2판, 뒷도라는 **장당 2판**이 된다. 역만 손에는 붙지 않는다.",
  detail:
    "(2국에 1회) 스텔스 리치처럼 **이 증강 자체가 리치를 건다** — 텐파이 상태에서 버릴 패를 고르면 그 패를 버리며 리치가 성립하고, 공탁 1,000점은 내지 않는다. 순 제한은 없어 리치를 걸 수 있는 어느 순에나 쓸 수 있다. 손이 잠기고 후로를 못 하는 것은 표준 리치와 같다.\n\n화료 시 리치 1판이 2판, 일발 1판이 2판이 되고, **뒷도라는 뒤집힌 장수만큼 통째로 두 배가 된다** — 뒷도라 3장이면 3판이 아니라 6판이다(이 증강의 진짜 화력은 여기 있다). 더블리치는 이미 2판이므로 더 오르지 않는다.\n\n⚠ **역만 손에는 이 추가 판이 하나도 붙지 않는다.** 역만은 판수로 세는 손이 아니라서, 역만이 뜨는 순간 이 선언은 공탁 면제만 남는다.",

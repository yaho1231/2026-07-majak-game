...
export const noRetreat: AugmentDef = defineAugment({
  ...
  name: "물러설 수 없는 선언",
  description:
    "(2국에 1회 · 리치는 국당 한 번) 텐파이에서 버릴 패를 골라 리치를 건다. 공탁 1,000점이 면제되고 그 국의 리치·일발이 각각 2판, 뒷도라는 **장당 2판**이 된다. 역만 손에는 붙지 않는다.",
  detail:
    "텐파이에서 버릴 패를 고르면 그 자리에서 리치가 선다. 공탁 1,000점을 내지 않고, 그 국의 리치·일발이 각각 2판이 된다.\n\n뒷도라는 뒤집힌 장수만큼 통째로 두 배라 **3장이면 6판**이다. 더블리치는 이미 2판이라 더 오르지 않고, 역만이 뜨면 추가 판이 하나도 붙지 않아 공탁 면제만 남는다.\n\n리치는 국당 한 번이므로, 리치를 선언하는 증강을 여럿 들었어도 그 국에 쓸 수 있는 것은 그중 하나뿐이다.",

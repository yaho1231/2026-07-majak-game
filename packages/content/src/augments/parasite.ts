...
export const parasite: AugmentDef = defineAugment({
  ...
  name: "기생충",
  description:
    "(매 국 1회) 자기 순에 상대 한 명에게 기생한다. 그 국 정산에서 숙주가 얻는 점수의 절반을 대신 가져온다.",
  detail:
    "(매 국 1회) 자기 순에 상대 한 명을 공개 지정해 기생한다. 그 국 정산에서 숙주가 얻는 점수의 절반(100점 단위)을 대신 가져오며, 숙주가 잃는 국에는 아무 영향이 없고 테이블 총점도 변하지 않는다. 지정은 국이 끝나면 풀리므로, 다음 국에 다시 자기 순이 오면 원하는 상대로 새로 지정한다. 지정하지 않고 넘긴 국에는 아무 효과도 없다.\n\n⚠ **숙주가 누구에게서 벌었는지는 따지지 않는다** — 내가 숙주에게 방총해도 그 화료는 '숙주가 얻는 점수'이므로 절반을 가져온다. 결과적으로 **숙주에게 쏜 방총은 반값이 된다**(8,000점 방총 → 실질 4,000점). 쏘기 쉬운 상대에게 기생하는 것은 그 자체로 방어 수단이다.",
  install(ctx) {

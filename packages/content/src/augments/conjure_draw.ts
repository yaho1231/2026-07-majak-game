...
export const conjureDraw: AugmentDef = defineAugment({
  ...
  name: "소환",
  description:
    "(매 국 1회) 자기 순에 손패 1장을 지목하면, 다음 내 쯔모가 그 패의 복제(생성패)로 바뀐다 — 내가 부른 패가 다음 쯔모가 된다.",
  detail:
    "(매 국 1회) 자기 순에 손패 1장을 지목해 그 패의 종류를 목표로 삼는다. 다음 내 쯔모는 패산에서 뽑히되 그 자리에서 부른 패의 복제(생성패)로 바뀌어 손에 들어온다 — 패산은 평소대로 한 장 줄고 손패 장수도 그대로다. 깡으로 뽑는 영상패에는 반응하지 않는다. 무엇을 불렀는지는 발동 즉시 전원에게 공개된다.",
  install(ctx) {

...
export const timePressure: AugmentDef = defineAugment({
  ...
  detail:
    "드래프트에서 고르는 순간, 패를 받기도 전에 저절로 발동해 그 국 하나 동안만 유지된다. 그 국에는 자리에 앉은 전원이 모든 결정을 5초 안에 내려야 한다 — 타패는 물론 론·치·펑·깡을 물어보는 자리, 액티브 증강의 선택까지 전부 같은 시계를 본다. 자신도 예외가 아니다.\n\n시간을 넘기면 안전한 쪽으로 자동 진행된다. 버릴 차례였다면 쯔모한 패를 그대로 버리고(쯔모기리), 울지 말지 묻는 자리였다면 패스하며, 되돌릴 수 없는 발동의 마무리 단계였다면 남은 후보 중 하나가 무작위로 선택된다.\n\n리치를 건 상대 앞에서 5초 안에 안전패를 고르는 것은 평소의 마작과 전혀 다른 게임이다. 다만 같은 제한이 자신에게도 걸려 있으므로, 이 국에는 자기 손도 서둘러 두어야 한다.",
  install(ctx) {

...
export const frameUp: AugmentDef = defineAugment({
  ...
  detail:
    "(2국에 1회) 자기 순에 버릴 패 한 장을 골라 상대 한 명의 바닥에 놓는다. 그 패는 그 사람이 버린 것으로 기록되어, 그가 그 종류로 기다리고 있었다면 후리텐에 걸린다. 동시에 그 패가 내 바닥에 남지 않아 내 후리텐은 회피된다. 다만 실제로 버린 사람은 나이므로 다른 상대의 론 반응은 평소대로 열려 있고 그 패로 쏘이면 책임도 내가 진다. 심긴 패는 전원에게 공개되며, 리치 중이거나 국의 첫 바퀴에는 쓸 수 없고 봉인술사에게 잠긴 패는 남의 명의로도 심을 수 없다. 중장패(2~8 수패)를 심으면 그 국의 유국만관 자격을 잃는다 — 명의만 옮겼을 뿐 흘린 것은 나이기 때문이다(요구패는 무관).",
  /*

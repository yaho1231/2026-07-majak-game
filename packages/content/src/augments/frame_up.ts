...
export const frameUp: AugmentDef = defineAugment({
  ...
  name: "누명",
  description:
    "(2국에 1회) 자기 순에 내가 버릴 패를 지목한 상대의 바닥에 놓는다. 그 상대가 버린 것으로 기록되어 그 상대는 후리텐에 걸리고, 내 바닥에는 남지 않아 나는 후리텐에 걸리지 않는다.",
  detail:
    "내가 버릴 패를 상대의 바닥에 놓는다. 그 상대가 버린 것으로 기록되어 그 상대는 후리텐에 걸리고, 나는 후리텐에 걸리지 않는다.\n\n실제로 버린 사람은 나이므로 다른 상대는 그 패로 론할 수 있고, 쏘이면 내가 지불한다. 리치 중이거나 국의 첫 바퀴에는 사용할 수 없다. 중장패(2~8)를 놓으면 그 국의 유국만관 자격을 잃는다.",
  /*
   ...
   *
   * 2026-08-08 에는 그 자리에 `["picky_eater"]` 를 남겨 뒀다 — "심긴 패 한 장이
   * 편식 퀘스트를 통째로 깬다"는 이유였다. **그 근거도 이미 사라졌다.**
   * 편식은 그 뒤 `TILE_DISCARDED.player === holder` 만 세는 자기 목록으로 옮겼고
   * (`picky_eater.ts` myDiscardKinds), 심긴 패로는 퀘스트가 깨지지 않는다 —
   * 실측으로 확인했다(QA synergy3 handedit 확정 8, 2026-08-23).
   * 게다가 그 피해는 **상대편** 편식 보유자에게 가는 것이라, 같은 사람이 둘 다
   * 드는 것을 막는 `conflicts` 로는 애초에 막을 수 없었다. 근거 없이 후보 하나를
   * 뺏을 뿐이라 지운다.
   */
  install(ctx) {

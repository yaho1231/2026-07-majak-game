...
export const genesis: AugmentDef = defineAugment({
  ...
  name: "개벽",
  description:
    "(동풍전 1회 · 반장전 2회) 자기 순에 발동하면 손패의 자패는 무작위 수패로, 수패는 무작위 자패로 통째로 뒤바뀐다. 새 패는 패산의 실물에서 오고 원래 손패는 패산으로 돌아간다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 자기 순에 발동하면 손패의 자패는 수패로, 수패는 자패로 통째로 뒤바뀐다. 새 패는 패산에서 실물로 가져오고(패산에 그 분류가 모자랄 때만 생성) 원래 손패는 패산 맨 밑으로 들어가 계속 돈다. 어떤 패가 오는지는 완전히 무작위다. 리치 중에는 발동할 수 없다.",
  install(ctx) {

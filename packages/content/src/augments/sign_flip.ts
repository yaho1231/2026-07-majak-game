...
export const signFlip: AugmentDef = defineAugment({
  ...
  detail:
    "드래프트에서 고르는 순간, 패를 받기도 전에 저절로 발동해 그 국 하나 동안만 유지된다. 그 국에 내 점수가 움직이는 모든 경로에서 부호가 뒤집힌다 — 방총으로 8000점을 잃을 자리에서는 뱅크가 8000점을 지급하고, 화료로 1000점을 벌 자리에서는 그 1000점을 뱅크가 가져간다. 리치 공탁 1000점, 유국 텐파이료, 본장 지불도 모두 뒤집힌다.\n\n상대의 점수는 정상적으로 움직인다. 내가 쏘인 점수는 화료자에게 그대로 가고, 내가 받는 점수는 뱅크가 따로 발행한다.\n\n그 국에는 마작의 상식이 통째로 뒤집힌다. 위험패를 골라 버리고 상대의 가장 큰 손에 일부러 쏘이는 것이 최선이며, 화료는 오히려 피해야 한다. 발동은 전원에게 공개되므로 상대는 '저 사람에게는 화료하지 않는다'로 맞설 수 있다.",
  install(ctx) {
    const { holder } = ctx;

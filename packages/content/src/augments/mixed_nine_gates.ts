...
export const mixedNineGates: AugmentDef = defineAugment({
  ...
  name: "뒤섞인 아홉 개의 연꽃",
  description:
    "(상시 · 멘젠 한정) 구련보등이 무늬를 가리지 않는다 — 만·통·삭을 하나로 보고 1,112,345,678,999 + 아무 수패 1장이면 역만이다. 자패는 한 장도 섞이면 안 되고, 몸통이 하나라도 생기면(안깡 포함) 성립하지 않는다.",
  detail:
    "(상시) 멘젠 화료형 14장이 **모두 수패**이고, 무늬와 무관하게 랭크가 1,112,345,678,999에 아무 랭크 한 장을 더한 배열이면 역만이 된다.\n\n제약 둘을 분명히 해 둔다. **자패(동남서북·백발중)는 한 장도 섞일 수 없다** — 무늬를 무시하는 것은 만·통·삭 사이의 이야기지 자패까지 포함하는 것이 아니다. 그리고 **멘젠이어야 한다** — 치·퐁·대명깡을 한 번이라도 하면 성립하지 않는다(안깡도 손에 몸통으로 남아 이 형태를 깬다). 그래서 손이 이미 1,112,345,678,999 뼈대를 이룬 동안에는 치·퐁·깡 버튼이 아예 뜨지 않는다 — 그 순간의 후로는 이 역만을 부수는 수뿐이기 때문이다.\n\n무늬가 한 종류뿐이면 표준 구련보등이 그대로 적용되므로 이 역은 무늬가 두 종류 이상 섞였을 때 값한다. 추가 보너스는 없다.",
  install(ctx) {

 ...
 *
 * # 2026-09-07 (사용자 지시) — 장사진의 **4연속 깡도** 무늬를 안 가린다
 *
 * 장사진(snake_kan)은 넉 장을 «슌쯔의 일종»으로 세우는 물건이라(`WinContext.meldToSet`),
 * 슌쯔의 무늬 국경이 사라진 국에 4연속 깡에만 그 국경이 남을 이유가 없다. 판정은
 * `isRunQuad(kinds, wrap, mixed)` 한 곳으로 모여 있고, 이 증강은 종전대로
 * `scoring.mixedRuns`만 켠다 — 그 값을 읽는 쪽이 하나 늘었을 뿐이다.
 * (커쯔 쪽은 그대로 동수의 결속 담당이다. 혼색 4연속 깡은 커쯔로 세지 않는다.)
 */
...
export const brokenBorder: AugmentDef = defineAugment({
  ...
  name: "무너진 국경",
  description:
    "(동풍전 2국에 1회 · 반장전 3국에 1회) 국의 첫 순에 발동하면 이번 국 동안 슌쯔의 무늬 제한이 사라져 2만·3통·4삭도 한 몸통으로 인정된다.",
  detail:
    "국의 첫 순에 발동하면 이번 국 동안 슌쯔가 무늬를 가리지 않는다. 2만·3통·4삭도 한 몸통이고 치도 할 수 있다. 장사진을 함께 가지고 있으면 4연속 깡도 무늬를 가리지 않는다.\n\n한 장이라도 버린 뒤에는 발동할 수 없다. 커쯔와 머리는 여전히 같은 무늬여야 하고, 무늬가 섞인 슌쯔로는 청일색·삼색동순·일기통관이 성립하지 않는다.",

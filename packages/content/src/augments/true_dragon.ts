...
export const trueDragon: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 배패를 16장 받고 멘쯔 5개 + 머리 1개(총 17장)로 화료한다. 화료·텐파이·후리텐·대기 판정이 모두 5멘쯔 형태로 바뀌며 화료 시 +3판을 얻는다(역만에는 미적용). 14장·4멘쯔를 전제하는 국사·치또이·구련류 증강, 그리고 성립하지 않는 깡과는 함께 가질 수 없다. 무장해제로 잠기면 가장 쓸모없는 패 3장이 패산으로 돌아가 평범한 손패 장수로 복귀한다 — 용은 풀리지만 그 국을 계속 싸울 수는 있다.",

...
export const bigHand: AugmentDef = defineAugment({
  ...
  name: "큰손",
  description:
    "(2국에 1회) 국의 첫 순에 액티브 버튼이 활성화되며, 선언하면 그 국에 화료했을 때 **내가 받는 총액**이 최소 만관(오야 12,000 · 자 8,000)이 된다 — 손의 값이 아니라 본장·공탁까지 합친 수령액 기준이다.",
  detail:
    "(2국에 1회) 아직 버리지도 울지도 않은 국의 첫 순에만 선언할 수 있다. 선언한 국에 화료하면 부족한 차액을 뱅크가 채워 만관(오야 12,000 · 자 8,000)을 맞춰 준다.\n\n⚠ **하한이 걸리는 것은 손의 값이 아니라 그 국에 내가 받는 총액이다.** 본장 수령분과 회수하는 리치봉(공탁)이 전부 이 총액에 들어간다. 리치봉 두 개를 회수했다면 손 자체는 6,000점이어도 총액은 8,000이 되어 뱅크가 한 푼도 채워 주지 않는다. 배수 증강(일확천금 등)이 걸려 있으면 배수가 다 적용된 **뒤의** 값을 보고 채운다.\n\n그 국에 화료하지 못하면 점수 변화는 없지만, **선언한 순간 2국 쿨다운은 이미 시작된다.**",
  install(ctx) {

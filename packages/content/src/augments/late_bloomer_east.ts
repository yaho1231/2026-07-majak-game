...
export const lateBloomerEast: AugmentDef = defineAugment({
  ...
  name: "대기만성 (동풍전)",
  description:
    "(상시 · 동풍전 전용 · 게임 시작 드래프트에서만 등장) 동4국부터 만개한다. 남입 연장도 포함된다. 만개한 뒤로는 후리텐을 무시하고 론할 수 있고, 화료에 +2판이 붙는다.",
  draftStages: ["gameStart"],
  modes: ["tonpuu"],
  detail:
    "동4국부터 만개한다. 남입 연장도 포함된다. 만개한 뒤로는 후리텐을 무시하고 론할 수 있고, 역이 하나도 없어도 화료할 수 있으며, 화료에 +2판이 붙는다. 다만 머리 1개와 몸통 4개 형태는 갖춰야 한다.\n\n+2판은 역만에는 붙지 않는다. 만개 전까지는 아무 효과도 없다.",
  install(ctx) {
    const { holder } = ctx;

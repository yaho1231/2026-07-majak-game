...
export const lateBloomerEast: AugmentDef = defineAugment({
  ...
  modes: ["tonpuu"],
  detail:
    "(상시 · 동풍전 전용 · 게임 시작 드래프트에서만 등장) 동4국(남입 연장 포함)에 들어서는 순간 만개해, 그 이후의 모든 국에서 후리텐이 적용되지 않는다. 또한 머리 1개와 몸통 4개(4멘쯔)로 손이 완성되면 역이 하나도 없어도 그대로 화료할 수 있다. 만개 후의 화료에는 +2판이 붙는다(역만에는 미적용). 만개 사실은 전원에게 공개되며, 만개 전까지는 아무 효과도 없다.",
  install(ctx) {
    const { holder } = ctx;

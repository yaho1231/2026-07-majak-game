...
export const devilsAdvance: AugmentDef = defineAugment({
  ...
  detail:
    "(게임 내 1회 · 게임 시작 드래프트 전용) 획득 후 첫 국이 시작될 때 뱅크에서 10,000점을 받고 그 사실이 '가불 10000'으로 전원에게 공개된다. 이후 자신이 만관 이상으로 화료하는 순간 빚이 터져 상대 세 명이 각각 3,000점을 낸다. 그 합계 9,000점은 빚을 갚는 돈이라 내 점수에는 더해지지 않는다 — 화료점은 표준 그대로이고 상대 셋만 줄어든다. 폭발은 게임당 한 번뿐이며 터지고 나면 공개 표시가 '청산'으로 바뀐다. 끝내 만관을 못 쳐도 상환이나 위약금은 없어 받은 10,000점은 온전히 내 것이다.",
  install(ctx) {
    const { holder } = ctx;

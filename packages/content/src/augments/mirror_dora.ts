...
export const mirrorDora: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 도라는 원래 표시패의 '다음' 패지만, 이 증강을 가진 사람은 표시패의 '앞' 패도 도라로 센다. 순환은 정확히 역방향이다 — 1의 앞은 9, 동의 앞은 북, 백의 앞은 중이다. 표도라·깡도라는 물론 뒷도라 표시패에도 적용되며, 뒷도라 쪽은 평소처럼 뒷도라를 세는 손(리치한 손)에만 얹힌다.\n\n표시패 한 장이 곧 두 종류의 도라가 되므로 손패의 도라 밀도가 통째로 두 배가 된다. 다만 이 도라는 자신에게만 유효하고, 무엇이 내 도라인지는 전원에게 공개된다 — 상대는 그 패를 흘리지 않는 식으로 대응할 수 있다.",
  install(ctx) {
    const { holder } = ctx;

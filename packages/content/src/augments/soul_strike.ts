...
export const soulStrike: AugmentDef = defineAugment({
  ...
  detail:
    "(2국에 1회) 텐파이 상태의 자기 순에 발동한다. 버릴 패를 고르면 그 패로 리치를 선언하고, 그대로 연속 6쯔모에 들어간다 — 상대 셋의 순서를 건너뛰고 여섯 번 연달아 뽑고 버린다. 이 리치는 2판으로 취급하며, 첫 순에 걸어 더블리치가 되면 3판으로 취급한다. 연속 쯔모 도중의 쯔모 화료는 몇 번째 쯔모든 언제나 일발이 붙는다.\n\n다음 조건 중 하나가 충족되면 그 자리에서 끝난다(위가 우선). ① 타가에게 방총당하면 국이 그대로 끝난다. ② 타가가 후로하면 후로한 사람부터 진행된다. ③ 여섯 장을 다 뽑고 그 패를 버리면 하가부터 진행된다.\n\n안깡은 다섯 번째 쯔모까지만 횟수를 소모한다. 여섯 장을 다 뽑은 뒤에는 타패하기 전까지 횟수를 쓰지 않고 안깡 → 영상 쯔모를 이어 갈 수 있으므로, 마지막 한 장을 쥔 채 깡을 거듭해 손을 더 키울 수 있다.\n\n연속 쯔모는 국의 순번을 실제로 소모한다 — 패산에서 그만큼 빠지므로 해저가 앞당겨지고, 그 패들은 상대에게 돌아가지 않는다. 그동안 자신이 버리는 여섯 장은 평소대로 전부 론 대상이며, 타가는 울어서 폭주를 끊을 수 있다.",
  install(ctx) {

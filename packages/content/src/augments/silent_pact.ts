...
export const silentPact: AugmentDef = defineAugment({
  ...
  detail:
    "(매 국 1회) 남의 버림패로 퐁을 하되 멘젠(닫힌 손)이 깨지지 않는다. 멘쯔는 눈에 보이게 눕지만 그 뒤로도 리치를 걸 수 있고 멘젠쯔모·멘젠 론 부수가 그대로 붙는다. ⚠ **멘젠이 유지되는 것은 이 증강의 버튼으로 부른 딱 한 번의 퐁뿐이다.** 같은 국에 평범한 퐁·치·대명깡을 하나라도 더 하면 그 순간 손이 열려 리치·멘젠쯔모·멘젠 론 부수를 전부 잃는다(이미 묵계로 부른 멘쯔는 그 뒤에도 손을 열지 않는다). 다만 커쯔가 하나 생기므로 핑후는 성립하지 않는다.",
  /**

...
export const silentPact: AugmentDef = defineAugment({
  ...
  name: "묵계",
  description:
    "(매 국 1회) ⚠ **이 증강으로 부른 퐁 1회만** 멘젠이 유지된다. 같은 국에 평범한 퐁·치·대명깡을 하나라도 더 하면 손이 열려 전부 잃는다(안깡은 예외).",
  detail:
    "(매 국 1회) 몸통은 눈에 보이게 눕지만 그 뒤로도 리치를 걸 수 있고 멘젠쯔모·멘젠 론 부수가 그대로 붙는다. 다만 커쯔가 생기므로 핑후는 성립하지 않는다. 평범한 퐁·치·대명깡이 더해지면 그 자리에서 잃지만, 이미 묵계로 부른 몸통은 손을 열지 않는다.",
  /**

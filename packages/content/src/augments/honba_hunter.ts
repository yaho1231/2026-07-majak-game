...
export const honbaHunter: AugmentDef = defineAugment({
  ...
  name: "본장 사냥꾼",
  description:
    "(상시) 나에게만 본장 1개당 추가 점수가 300점이 아니라 1,500점이 된다. 5본장이면 화료에 +7,500점 — 상한은 없다.",
  detail:
    "(상시) 본장 1개당 추가 점수가 300점 대신 1,500점이 된다. 론이면 쏜 사람이 전액을 내고 쯔모면 셋이 나눠 내며 상한은 없다. 내 앞의 본장 표시는 실제 가치만큼 부풀어 전원에게 보인다.",
  install(ctx) {
    const { holder } = ctx;

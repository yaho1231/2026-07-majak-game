...
export const windLineage: AugmentDef = defineAugment({
  ...
  name: "바람의 계보",
  description:
    "(상시) 자패로 슌쯔를 만든다 — 동남서·남서북·백발중도 한 몸통이 된다. 자풍이나 장풍이 든 바람 몸통, 그리고 백발중 몸통은 하나당 1판이다.",
  detail:
    "동→남→서→북, 백→발→중으로 이어지는 석 장이 슌쯔가 된다. 치로도 만들고, 네 바람을 한 장씩 모으면 안깡으로 선언해 영상패도 뽑는다.\n\n판은 **몸통마다** 붙는다 — ① 자풍이나 장풍이 든 바람 몸통 하나에 1판 ② 백발중 몸통 하나에 1판. 한 몸통에 자풍과 장풍이 둘 다 들어 있어도 1판이다. 동장·남가라면 동남서가 1판(동=장풍, 남=자풍), 여기에 남서북까지 더하면 2판이다. 백발중을 둘 만들어도 2판. 자풍도 장풍도 없는 바람 몸통은 몸통으로만 서고 판은 없다.\n\n**커쯔는 계보의 대상이 아니다** — 동동동·백백백은 예전처럼 기존 자풍·장풍·삼원 역패로만 값한다. 후로해도 판수는 같다. 자패 슌쯔는 청일색·삼색동순·일기통관에는 값하지 않는다.",
  install(ctx) {

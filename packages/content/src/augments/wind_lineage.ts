 ...
 *
 * 값(2026-08-27 문안 정비 — **성능 변경 없음**, 표현만 다시 썼다):
 * **자풍·장풍을 포함한 몸통, 또는 백발중으로 만든 몸통은 각각 1판**이다. 아래 세 역이
 * 그 셋을 하나씩 맡는다(SEAT/PREVALENT/DRAGON). 셋은 겹칠 수 있고, 자풍과 장풍이 같은
 * 좌석(동1국 오야 등)은 한 장에 두 역이 걸려 2판이다 — 표준 더블 동과 같은 셈이다.
 * 같은 갈래는 몸통이 몇 개든 한 번만 센다(전부 boolean 판정이다).
 *
...
export const windLineage: AugmentDef = defineAugment({
  ...
  name: "바람의 계보",
  description:
    "(상시) 자패로 슌쯔를 만든다 — 동남서·남서북·백발중도 한 몸통이 된다. 자풍·장풍을 포함한 몸통, 또는 백발중으로 만든 몸통은 각각 1판으로 친다.",
  detail:
    "동→남→서→북, 백→발→중으로 이어지는 석 장이 슌쯔가 된다. 치로도 만들고, 네 바람을 한 장씩 모으면 안깡으로 선언해 영상패도 뽑는다.\n\n판은 세 갈래로, 각각 1판이다 — ① 자풍이 들어간 바람 몸통 ② 장풍이 들어간 바람 몸통 ③ 백발중 몸통. 셋은 겹칠 수 있어 동1국의 서가가 동남서를 만들면 동(장풍)·서(자풍) 둘이 걸려 2판이고, 오야처럼 자풍과 장풍이 같으면 그 한 장에 2판이 붙는다(더블 동과 같은 셈).\n\n같은 갈래는 몸통이 몇 개든 한 번만 센다 — 동남서와 남서북을 함께 들어도 서는 1판이고, 백발중을 둘 만들어도 1판이다. 자패 슌쯔는 청일색·삼색동순·일기통관에는 값하지 않는다.",
  install(ctx) {

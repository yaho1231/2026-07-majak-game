...
export const northTrader: AugmentDef = defineAugment({
  ...
  name: "북풍 상인",
  description:
    "(상시) 삼인마작의 북빼기. 자기 순에 손의 北을 빼놓고 영상패로 보충하며, 빼놓은 北은 한 장당 도라 1판으로 값한다.",
  detail:
    "(상시) 보충 쯔모로 화료하면 영상개화가 붙는다. 리치 중에는 쓸 수 없고, 영상패가 바닥나거나(누구의 것이든 깡 네 번) 패산이 다 떨어지면 北이 손에 있어도 못 뺀다.\n\n뺄 때마다 패산이 한 장 줄어 그 국은 모두에게 그만큼 빨리 끝난다. 첫 순에 빼면 **내 천화·지화가 깨진다**(상대의 것에는 영향이 없다).",
  install(ctx) {

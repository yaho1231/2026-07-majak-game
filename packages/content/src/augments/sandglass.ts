/**
 * 모래시계 (sandglass, prism) — 손패 3장을 패산 맨 위 3장과 맞바꾼다.
 *
 * (2국에 1회) 자기 순에 손패 3장을 고르면, 그 3장이 패산 맨 위로 올라가고 원래 맨 위에
 * 있던 3장이 손으로 들어온다. 무엇이 들어올지는 미리 볼 수 없다(2026-09-24 사용자 확정).
 * 패산이 3장 이하로 남았으면 쓸 수 없다. 리치 중에는 손이 고정돼 있으므로 쓸 수 없다.
 *
 * 뒤집힌 모래시계(`hourglass`)와는 다른 카드다 — 저쪽은 유국을 거부하고, 이쪽은 손패와
 * 패산 윗부분을 뒤집는다.
 *
 * ## 구현
 * - 커스텀 이벤트 `SandglassSwapped` 하나. 리듀서가 패산 앞 3장을 손으로, 고른 3장을
 *   패산 앞(0번 자리부터)으로 옮긴다. 장수 변화가 없어 유국 타이밍이 그대로다.
 * - 내보낸 패 중에 쯔모패가 있으면 들어온 마지막 패를 새 쯔모패로 세운다(미래를 보는 자와
 *   같은 규약). 쯔모패를 남겼으면 쯔모패는 그대로다.
 * - 후보는 손패 3장 조합 전부(C(14,3)=364)다. 화면은 모달에서 3장을 고르게 한다.
 * - 쿨다운은 공용 "N국에 1회" 도구(`trackRoundSeq`·`cooldownUse`)를 쓴다.
...
import { cooldownReady, cooldownUse, replaceDrawnTile, trackRoundSeq } from "../util.js";
import { plan } from "./botPlan.js";
import { worstHandTiles } from "./botHelpers.js";
import { handAlteredMark } from "./handAltered.js";

const ID = "sandglass";
const ACTION = "sandglass_swap";
/** 이 증강이 만들어내는 이벤트 */
const SANDGLASS_SWAPPED = "SandglassSwapped";
/** 바꾸는 장수 */
const SWAP_TILES = 3;
/** 쿨다운 — 한 번 쓰면 이만큼 국이 지나야 다시 열린다 (동풍전·반장전 공통) */
const COOLDOWN_ROUNDS = 2;

interface SandglassSwappedPayload {
  player: PlayerId;
...
export const sandglass: AugmentDef = defineAugment({
  ...
  name: "모래시계",
  description:
    "(2국에 1회) 자기 순에 손패 3장을 골라 패산 맨 위 3장과 바꾼다. 패산이 3장 이하로 남으면 사용할 수 없다.",
  detail:
    "손패에서 고른 3장을 패산 맨 위 3장과 맞바꾼다. 가져올 3장은 미리 볼 수 없다.\n\n내가 넣은 3장은 패산 맨 위에 놓여 다음 쯔모로 나간다. 리치 중에는 사용할 수 없다.",
  install(ctx) {
    const { engine, holder } = ctx;

    if (!engine.reducers.has(SANDGLASS_SWAPPED)) {
      engine.reducers.register(SANDGLASS_SWAPPED, (state, event) => {
        const p = event.payload as SandglassSwappedPayload;

 ...
 *
 * ## 2026-08-18 (사용자 지시) — **2026-08-15의 "순수 교환"을 되돌린다**
 *
 * 2026-08-15에 "3장을 전부 내가 고르고 바닥에 버리는 패도 없는 순수 교환"으로 바꿨었다.
 * 그 결과 리스크가 통째로 사라져 **누르면 무조건 이득인 버튼**이 됐다 — 손패는 항상
 * 좋아지고 발동마다 +1판이 확정으로 붙는다. 이 증강의 정체성은 하이리스크
 * 하이리턴이라는 사용자 판단으로, 아래 2026-07-22 사양으로 복귀한다:
 * **무작위 3장이 뽑히고, 그중 바닥에 버릴 1장만 내가 고른다.**
 *
...
export const futureSight: AugmentDef = defineAugment({
  ...
  name: "미래를 보는 자",
  description:
    "손패에서 무작위 3장을 뽑아 패산 위 3장과 교환한다. 뽑힌 3장 중 하나를 골라 내 바닥에 버린다. 화료하면 교환 횟수마다 +1판을 받는다.",
  detail:
    "손패에서 무작위 3장이 뽑혀 패산 위 3장과 바뀐다. 뽑힌 3장 중 하나는 내가 골라 내 바닥에 버리고, 나머지는 패산 밑으로 간다. 이번 국의 교환 횟수만큼 화료에 +1판(역만 제외).\n\n손패에서 무작위 3장이 뽑혀 패산 위 3장과 바뀐다. 뽑힌 3장 중 하나는 내가 골라 내 바닥에 버리고, 나머지는 패산 밑으로 간다. 이번 국의 교환 횟수만큼 화료에 +1판이 더해진다. 역만에는 적용되지 않는다.\n\n뽑힌 3장은 상대에게도 공개된다. 버튼을 누른 순간 쿨다운이 시작되고, 판은 실제로 교환했을 때만 더해진다. 버린 패는 후리텐 판정에 들어가지만 론이나 후로의 대상은 아니다. 리치 중에는 사용할 수 없다.",
  install(ctx) {

...
export const futureSight: AugmentDef = defineAugment({
  ...
  name: "미래를 보는 자",
  description:
    "(3순에 1회) 손패에서 무작위 3장이 뽑히고, 버릴 1장을 고르면 나머지 2장은 패산 맨 밑으로 가고 3장을 새로 받는다. 교환할 때마다 층이 쌓여 화료하면 층 하나당 +1판이 붙는다.",
  detail:
    "(3순에 1회) ⚠ **뽑힌 3장은 상대에게도 그대로 공개된다.** 버튼을 누른 순간 발동이 소모되어 물러나도 쿨다운은 돌아간다(층은 실제로 교환했을 때만 쌓인다). 층은 국이 바뀌면 초기화되고 역만에는 미적용이다. 버린 패는 후리텐을 만들지만 남의 론·후로 대상은 아니다. 리치 중에는 쓸 수 없다.",
  install(ctx) {

...
export const futureSight: AugmentDef = defineAugment({
  ...
  name: "미래를 보는 자",
  description:
    "(3순에 1회) 액티브 버튼을 누르면 손패에서 무작위 3장이 뽑히고, 그중 바닥에 버릴 1장을 직접 고른 뒤(나머지 2장은 패산 맨 밑으로) 패산 위 3장을 가져온다. 쓸 때마다 층이 쌓여 그 국에 화료하면 층 하나당 +1판을 얻는다.",
  detail:
    "(3순에 1회) 자기 순에 액티브 버튼을 눌러야 발동한다. 버튼을 누른 뒤에야 손패에서 뽑힌 무작위 3장이 제시되고, 그중 버릴 1장을 직접 고르면 나머지 2장은 패산 맨 밑으로 들어가며 패산에서 3장을 새로 받는다. 한 번 누르면 교환은 취소할 수 없고, 고르지 않으면 무작위로 한 장이 버려진다. 새로 받은 3장은 전원에게 공개된다. 한 번 쓰면 내 순이 세 번 지나야 다시 열리며, 깡을 쳐도 같은 순에는 다시 열리지 않는다. 교환할 때마다 층이 1씩 쌓여 그 국에 화료하면 층 하나당 +1판을 얻고, 쌓인 판수는 내 이름표의 증강 표식에 계속 표시된다. 층은 국이 바뀌면 초기화된다. 바닥으로 보낸 패는 내 바닥에 쌓여 그 종류로는 내가 론할 수 없게 되지만(후리텐), 턴 중간에 놓는 것이라 다른 사람의 론·후로 대상은 되지 않는다. 리치 중에는 쓸 수 없다.",
  install(ctx) {

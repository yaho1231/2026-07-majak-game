...
export const cliffBloom: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 깡할 때마다 **남아 있는 영상패를 모두 보고** 그중에서 직접 고른다. 영상패는 국 시작 시 4장이고 누군가 깡을 칠 때마다 한 장씩 소모되어 보충되지 않으므로, 고를 수 있는 폭은 깡이 늘수록 좁아진다(네 번째 깡에서는 남은 한 장뿐이다). 같은 국에서 두 번째 깡을 완성하면 텐파이였는지 무엇을 쥐고 있었는지와 무관하게 손패가 완성형으로 재구성되어 영상개화로 즉시 화료한다. 재구성은 지금 손패에서 살릴 수 있는 패를 최대한 살린 화료형으로 계산되므로, 걸어오던 방향의 역이 그대로 붙을 수 있다. 만개한 국의 화료에서는 영상개화가 1판이 아니라 4판으로 계산된다(역만에는 미적용). 만개하지 않은 국의 화료에는 아무것도 얹히지 않는다.",
  install(ctx) {

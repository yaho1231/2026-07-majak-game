...
export const pushRiichi: AugmentDef = defineAugment({
  ...
  name: "등 떠밀기",
  description:
    "(매 국 1회) 자기 순에 상대 한 명에게 낙인을 찍는다(전원 공개). 그가 리치 가능한 상태에서 패를 버리는 순간 그 버림이 강제 리치가 된다. 그렇게 떠민 사람을 **직격 론**으로 잡으면 **+2판**.",
  detail:
    "상대 한 명에게 낙인을 찍으면(전원 공개), 그가 멘젠 텐파이로 패를 버리는 순간 강제로 리치가 걸린다 — 다마텐으로 숨을 수 없다. 그렇게 떠민 사람을 직격 론하면 +2판(역만 제외).\n\n리치봉을 낼 점수와 패산이 있어야 하고, 후로한 손이나 리치가 봉인된 손에는 걸리지 않는다. 낙인은 리치가 성립하거나 국이 끝나면 사라진다.",
  install(ctx) {

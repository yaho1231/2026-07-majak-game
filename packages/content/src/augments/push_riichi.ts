...
export const pushRiichi: AugmentDef = defineAugment({
  ...
  detail:
    "(매 국 1회) 자기 순에 상대 한 명에게 낙인을 찍는다. 낙인자가 멘젠 텐파이 상태로 패를 버리려 하면 그 버림이 자동으로 리치가 되어 리치봉이 강제로 던져진다 — 다마텐으로 숨을 수 없다. 리치봉을 낼 점수와 패산이 남아 있고 리치가 봉인돼 있지 않아야 성립한다. 낙인은 찍은 국 동안만 살아 있고, **강제든 스스로 건 것이든** 리치가 성립하거나 국이 끝나면 소멸한다. 후로해 멘젠이 깨진 손에는 조건이 서지 않는다.",
  install(ctx) {

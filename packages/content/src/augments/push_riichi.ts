 ...
 * 직격 론으로 잡으면 +2판.** 이 카드는 파워 티어 23점으로 카탈로그 최하위권이었다 —
 * 상대에게 리치봉 1,000점과 일발·우라도라를 쥐여 주면서 내가 얻는 것은 "다마텐을 못 쓴다"는
 * 정보뿐이라, 순수 손해로 끝나는 국이 흔했다. 판수는 이웃 카드 카운터(`counter`)의
 * "선리치자 직격 론 +3판"을 기준으로 잡았다가, 저판 오야에서 스윙이 너무 커
 * 2026-08-27 사용자 지시로 +2판으로 내렸다(내가 지목한 한 사람에게서,
...
export const pushRiichi: AugmentDef = defineAugment({
  ...
  name: "등 떠밀기",
  description:
    "(매 국 1회) 자기 순에 상대 한 명에게 낙인을 찍는다. 낙인은 모두에게 공개된다. 그 사람이 리치할 수 있는 상태에서 패를 버리면 그 버림이 강제로 리치가 된다. 그렇게 리치하게 된 사람을 직격 론하면 2판이 추가된다.",
  detail:
    "상대 한 명에게 낙인을 찍으면 그 사람이 멘젠 텐파이 상태에서 패를 버리는 순간 강제로 리치가 걸린다. 다마텐을 할 수 없다. 낙인은 모두에게 공개된다. 그렇게 리치하게 된 사람을 직격 론하면 2판이 추가된다(역만 제외).\n\n리치봉을 낼 점수와 패산이 남아 있어야 하고, 후로한 손이나 리치가 봉인된 손에는 걸리지 않는다. 낙인은 리치가 성립하거나 국이 끝나면 사라진다.",
  install(ctx) {

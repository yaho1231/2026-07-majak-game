...
export const hiddenRiver: AugmentDef = defineAugment({
  ...
  name: "안개 덮인 바닥",
  description:
    "(동풍전 1회 · 반장전 2회) 자기 순에 선언하면 그 국이 끝날 때까지 네 사람 모두의 바닥에서 최근 6장만 공개되고, 그 앞의 버림패는 장수만 보인다. 보유자만 네 바닥을 그대로 읽는다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 선언하기 전의 바닥은 평소대로 보이고, 선언한 순간부터 국이 끝날 때까지 안개가 지속된다. 최근 6장은 전원에게 보이므로 론·후로 판정과 직전 한 바퀴의 현물 수비는 그대로 유지된다.\n\n안개는 국이 끝나면 걷히고, 횟수가 남아 있으면 다음 국에 다시 걸 수 있다.",

...
export const evenWorld: AugmentDef = defineAugment({
  ...
  detail:
    "발동하면 손패의 홀수 수패가 전부 한 칸 위 짝수가 된다(1→2, 3→4, 5→6, 7→8, 9→8). 자패만 그대로다.\n\n도라인 홀수 패도, 적도라(빨간 5)도 예외 없이 짝수가 된다(빨간 5는 적도라 1판을 잃는다). 바꾼 **결과**가 도라가 되는 것은 막지 않는다. 리치 중에는 쓸 수 없다.",
  install(ctx) {

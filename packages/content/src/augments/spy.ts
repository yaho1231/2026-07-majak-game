...
export const spy: AugmentDef = defineAugment({
  ...
  detail:
    "화료 자체는 성립하고 지불자들이 내는 액수도 그대로다 — 돈의 도착지만 바뀐다. 무엇을 찍었는지는 나만 알고 상대에게는 지정 사실만 공개된다.\n\n지정은 국이 바뀔 때마다 다시 찍어 갱신할 수 있고, 새로 찍기 전까지는 계속 유효하다. 내 화료에는 발동하지 않는다.",
  // 상대 화료를 방해하는 카드들과 함께 있으면 모순 (스파이는 상대가 화료하길 원함)
  conflicts: [
    "disarm",           // 봇을 무장해제해 화료 능력 제한
    "call_seal",        // 상대의 울음(화료 경로)을 봉쇄
    "riichi_seal",      // 상대의 리치 봉쇄
  ],
  install(ctx) {

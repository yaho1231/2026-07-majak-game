 ...
 * 스파이 (spy, prism).
 *
 * 게임당 1회, 자기 턴에 손패의 패 1종을 **비밀리에** 지정한다. 그 뒤로 상대가 그 종류를
 ...
 *   이 증강의 전부다.
 * - 내 화료에는 발동하지 않는다(이미 내가 받는다). 지정은 한 번뿐이고, 한 번 지정하면
 *   게임이 끝날 때까지 유효하다.
 * - 리미트는 "지정 1회"라는 **횟수뿐**이다(무페널티 원칙) — 헛다리를 짚어도 잃는 것은 없다.
 *
...
const markAction: ActionDef<{ tileId: TileId }> = {
  ...
  validate: (req, { state }) => {
    ...
    if (stringOf(state, markKey(req.player)) !== null) return "already marked";
...
export const spy: AugmentDef = defineAugment({
  ...
  name: "스파이",
  description:
    "(게임 내 1회) 자기 순에 손패의 패 1종을 비밀리에 지정한다. 이후 상대가 그 종류를 오름패로 화료하면 그 화료의 점수가 전부 나에게 온다.",
  detail:
    "(게임 내 1회) 자기 순에 손패의 패 1종을 비밀리에 지정한다. 이후 상대 누구든 그 종류를 오름패로 화료하면 화료 자체는 성립하되 그가 받을 점수가 전부 나에게 오고, 지불자들이 내는 액수는 그대로다 — 돈의 도착지만 바뀐다. 무엇을 찍었는지는 나만 알고 상대에게는 지정 사실만 공개된다. 내 화료에는 발동하지 않는다. 지정은 게임이 끝날 때까지 유효하다.",
  install(ctx) {
    ...
    // 지정 전에는 손패 아무 패나 후보 (합법성 최종 판정은 validate)
    ctx.holderTurnOptions((state) => {
      if (stringOf(state, markKey(holder)) !== null) return [];
    ...
    ctx.reaction(ROUND_SETTLED, (event, rc) => {
      ...
      rc.emit(augmentDataSet(viewKey("*", `${ID}:caught:${holder}`), marked));
    });
  },

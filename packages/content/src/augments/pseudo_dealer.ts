 ...
 *  - 보유자가 화료·텐파이하면 **진짜 연장(렌짱)** 이 되고, 다음 국 오야 로테이션도
 *    보유자 자리에서 이어진다. 원래 오야는 오야 자격과 연장 권리를 함께 잃는다.
 *
 * 상태 흐름 (augmentData):
...
export const pseudoDealer: AugmentDef = defineAugment({
  ...
  detail:
    "(2국에 1회) 자기 순에 선언하면 오야 자리가 즉시 나에게 넘어오고 네 사람의 자풍이 내 자리를 기준으로 다시 매겨진다 — 동을 안고 있던 사람이 객풍이 되기도 한다. 화료 점수는 오야 배율(1.5배)이 되고, 그 국에 화료하거나 텐파이로 유국하면 연장이 걸리며 이후 오야 로테이션도 내 자리에서 이어진다. 원래 오야는 오야 자격과 연장 권리를 함께 잃는다. 이미 내가 오야인 국에는 선언할 수 없다.",

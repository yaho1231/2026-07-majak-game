...
export const disarm: AugmentDef = defineAugment({
  ...
  /*
   * "이미 바꿔 놓은 것은 잠기는 순간 되돌아간다"를 **일반 규칙으로 적으면 거짓**이다.
   *
   * 되돌림은 구조상 대상 증강이 스스로 해야 한다 — 무장해제는 `AUGMENT_DISARMED`를
   * 먼저 통보할 뿐이고, 무엇을 어떻게 되감을지는 그 증강만 안다(무장해제가 남의
   * 물리 상태를 일반적으로 되돌릴 방법은 없다). 실제로 그 통보에 반응하는 것은
   * 진짜 용(손패 3장 반납)·초읽기(제한 시간 해제)·편식(진행도 표시 정리) 셋뿐이라, 삼원의 의지가 물질화해 세운 대삼원은 잠가도
   * 그대로 남는다(QA text 확정 31). 문구가 약속한 범위를 구현이 못 지키는 것이 아니라
   * **문구가 하드코딩된 몇 건을 일반 규칙처럼 적어 둔** 경우라 문장을 사실에 맞춘다
   * (되돌림이 필요한 증강이 늘면 그 증강 쪽에 리액션을 다는 것이 규약이다).
   */
  detail:
    "(동풍전 1회 · 반장전 2회) 자기 순에 상대 한 명의 증강 하나를 지목하면 그 증강이 이번 국이 끝날 때까지 완전히 잠긴다. 상시 규칙(만년 오야의 오야 고정·천하무적의 무방총)도, 정산 개입도, 액티브 버튼도 전부 사라진다. 다만 그 증강이 **이미 만들어 둔 것**(물질화한 패·바뀐 손패 같은 지나간 결과)은 대체로 그대로 남는다 — 잠기는 순간 스스로 되감는 증강만 원래대로 돌아간다(진짜 용은 필요 없는 패 3장을 패산에 반납해 평범한 손패로 돌아오고, 초읽기는 5초 제한을 그 자리에서 푼다). 한 국에 한 명의 증강 하나만 잠글 수 있고, 지목은 전원에게 공개되며 국이 끝나면 증강도 원래대로 돌아온다.",
  install(ctx) {

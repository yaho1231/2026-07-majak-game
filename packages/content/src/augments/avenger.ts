...
export const avenger: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 론으로 방총당하는 순간 그 상대가 '원수'로 지정되어 전원에게 공개된다. 그 뒤로 원수의 버림패에 한해 후리텐 규칙과 역 필요 조건이 모두 무시되어, 자신이 버린 패로도 역 없는 손으로도 론할 수 있다. 원수를 론으로 잡으면 그 화료에 +2판이 얹히고(역만에는 미적용) 원한이 풀려 해제된다. 원수의 버림패가 아닌 화료(쯔모·다른 사람 론)에는 판이 붙지 않으며, 다른 사람에게 방총당하면 원수가 그쪽으로 갱신된다. 방총당하기 전까지는 아무 효과도 없다.",

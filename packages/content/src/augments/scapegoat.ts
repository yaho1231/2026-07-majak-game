...
export const scapegoat: AugmentDef = defineAugment({
  ...
  detail:
    "판수를 빼면 내가 받는 점수는 그대로다. 나머지 두 명은 한 푼도 내지 않으며, 그 국 정산에서 다른 증강이 새로 부과하는 지불까지 지목당한 사람에게 몰린다.\n\n지목은 자기 순에 공개로 하고 한 번 정하면 그 국에는 못 바꾼다. **지목하지 못한 국에는 판수도 붙지 않는다** — 판수와 지불 재배선은 같은 조건에서 함께 선다. **론으로 화료할 때도 판수가 붙지 않고 아무것도 바뀌지 않는다.** 판수는 역만에는 얹히지 않는다. 전액을 문 사람은 점수가 모자라면 마이너스로 떨어져 탈락할 수 있다.",
  install(ctx) {

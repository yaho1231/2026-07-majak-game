...
export const giantGod: AugmentDef = defineAugment({
  ...
  name: "마작의 거신병",
  description:
    "(동풍전 1회 · 반장전 2회) 내 바닥에 국사무쌍 13종(1m9m·1p9p·1s9s·동남서북·백발중)이 한 장씩 전부 깔리면 발동할 수 있고, 그 13장을 손으로 끌어올리고 손패를 바닥으로 내던져 그 자리에서 국사무쌍 텐파이가 된다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 내 바닥에 국사무쌍의 요구패 13종이 한 장씩 전부 쌓이면 액티브 버튼이 켜진다. 발동하면 바닥의 그 13장이 손으로 올라오고 지금 손패 13장이 그 자리로 내려가 손패 장수는 그대로 유지된다. 배패 상태(손패 13장)에서 발동하면 순수 국사무쌍 13면 대기가 되고, 막 쯔모한 14장 상태라면 뽑은 한 장이 남는다. 발동은 전원에게 공개된다.",
  install(ctx) {

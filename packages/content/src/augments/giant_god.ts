...
export const giantGod: AugmentDef = defineAugment({
  ...
  name: "마작의 거신병",
  description:
    "(매 국 1회) **조건: 국사무쌍 13종(1만9만·1통9통·1삭9삭·동남서북·백발중)을 내가 직접 전부 버려 둬야 한다.** 13종이 모두 내 바닥에 깔린 뒤 내 순이 오면 버튼이 켜지고, 발동하면 그 13장을 손으로 끌어올려 국사무쌍 13면 대기가 된 뒤 **다음 순에 반드시 화료한다**.",
  detail:
    "(매 국 1회) **증강이 요구패를 깔아 주지 않는다 — 내가 손수 버려서 모아야 한다.** 내 바닥에 국사무쌍의 요구패 13종이 한 장씩 전부 쌓이기 전까지는 버튼이 아예 나타나지 않는다. 남의 바닥은 세지 않는다.\n\n발동하면 바닥의 그 13장과 내 손패가 통째로 자리를 바꿔 국사무쌍 13면 대기가 서고, **다음 순 내 쯔모가 반드시 오름패로 온다**(쯔모 선언은 평소대로 내가 한다). 방금 쯔모한 패는 내려가지 않고 손에 남으므로, 그 한 장이 요구패였다면 그 자리에서 바로 화료할 수 있다.\n\n**요구패에 대한 내 버림 이력은 각성과 함께 통째로 지워진다** — 같은 요구패를 두 번 버렸어도 13면 대기 전체의 후리텐이 풀린다. 그래서 다음 쯔모를 기다리는 동안 상대가 요구패를 버리면 론으로 먼저 끝낼 수도 있다.\n\n**치·퐁·깡을 한 번이라도 하면 그 국에는 각성할 수 없다.** 각성은 바닥의 요구패와 내 손패를 통째로 맞바꾸는 것이라 후로로 손패가 줄면 자리가 맞지 않는다 — 국사무쌍이 멘젠 전용인 것과 같은 이유다.\n\n한 국에 한 번만 쓸 수 있다. 리치 중에는 쓸 수 없고, 발동은 전원에게 공개된다.",
  install(ctx) {

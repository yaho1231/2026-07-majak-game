...
/**
 * 손패에서 **가장 고립된 패의 자리** — 이어짐만 보는 값싼 판정이다(`spareTile.isolationCost`).
 *
 * 값이 낮을수록 먼저 뽑힌다. 1순위는 **이어짐**(같은 패·이웃 패가 손에 몇 장 붙어
 * 있는가), 2순위는 자패 → 노두패(1·9) → 그 밖의 수패다 — 예전에는 이어짐이 같으면
 * 손패 순서 앞쪽이 그냥 뽑혀, 한 장 남은 자패와 외톨이 수패가 나란히 0점일 때 자패가
 * 아니라 앞에 있던 수패가 사라졌다("한 장 남은 한자패가 안 사라지고 다른 게 사라진다",
 * 2026-08-17 보고).
 *
 * ⚠ 재료로 태울 패를 고르는 데는 이제 이것을 쓰지 않는다. 이어짐만 세면 손을 모양으로
 * 읽지 못해 이미 완성된 몸통이 깨진다(2026-09-04 보고, #467) — 재료 선정은 결과 손의
 * 샹텐을 직접 재는 `spareTile.pickSpareTiles`가 하고, 이 함수는 그 **동점 판정**과
 * "이번 순에 버릴 만한 패"(frame_up·hand_swap3·joker)처럼 손이 실제로 바뀌지 않는
 * 자리에 쓴다.
 */

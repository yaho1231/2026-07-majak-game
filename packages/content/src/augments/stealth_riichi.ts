...
export const stealthRiichi: AugmentDef = defineAugment({
  ...
  description:
    "(매 국 1회 · 리치는 국당 한 번) 텐파이에서 보이지 않는 리치를 건다. 상대에게는 평범한 타패로 보이지만 화료하면 리치로 취급되어 리치 1판·일발·뒷도라가 붙고, 공탁 1,000점도 내지 않는다.",
  detail:
    "텐파이에서 보이지 않는 리치를 건다. 상대에게는 평범한 타패로 보이고 리치봉도 내지 않지만, 화료하면 리치(1판·일발·뒷도라)로 취급된다.\n\n손패가 고정되는 것은 표준 리치와 같다. 상대는 나를 리치하지 않은 사람으로 보므로 손패를 바꾸는 증강의 대상이 될 수 있고, 그러면 이 리치는 풀린다. 등 떠밀기의 낙인에 걸리면 공개 리치가 된다.\n\n리치는 국당 한 번이므로 리치 증강을 여러 개 가지고 있어도 그 국에는 하나만 사용할 수 있다.",

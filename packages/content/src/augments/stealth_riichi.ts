...
export const stealthRiichi: AugmentDef = defineAugment({
  ...
  description:
    "(매 국 1회 · 리치는 국당 한 번) 텐파이 상태에서 보이지 않는 리치를 건다. 상대에게는 평범한 타패로 보이지만 화료 시에는 리치로 취급되며(리치 1판·일발·뒷도라), 공탁 1,000점도 내지 않는다.",
  detail:
    "(매 국 1회 · 리치는 국당 한 번) 텐파이 상태에서 손패를 직접 눌러 발동한다. 그 패를 버리면서 리치가 성립하지만 리치 선언 표시도 리치봉도 상대의 화면에 나타나지 않으며, 공탁 1,000점도 내지 않는다. 화료하면 리치로 취급되어 리치·일발·뒷도라가 전부 적용되고 정산 화면에서 리치였음이 그때 공개된다. 표준 리치와 마찬가지로 손은 잠겨 쯔모패만 버릴 수 있고 후로도 할 수 없다. ⚠ 은닉에는 값이 하나 있다 — 남들은 나를 리치가 아닌 사람으로 취급하므로, 손을 바꾸는 증강(통째로 바꾸기·손패 3장 교환·자리 바꿈)이 나를 대상으로 삼을 수 있다. 그렇게 손이 바뀌면 이 리치는 풀린다(나만 알게 된다).",

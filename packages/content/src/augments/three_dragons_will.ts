...
export const threeDragonsWill: AugmentDef = defineAugment({
  ...
  name: "삼원의 의지",
  description:
    "(상시) 백·발·중 중 두 종류를 커쯔로 세우고 나머지 한 종류를 한 장이라도 쥐고 있으면, 자기 순에 발동해 그 한 장을 커쯔로 완성한다 — 삼원패 9장이 필요한 대삼원이 7장에서 선다.",
  detail:
    "(상시) 백·발·중 중 두 종류를 커쯔로 세우고 나머지 한 종류를 한 장이라도 쥔 상태에서 발동하면, 부족한 두 장이 손패의 가장 쓸모없는 잡패에서 물질화해 커쯔를 채운다. 손패 장수는 변하지 않고 세 커쯔가 실제로 손에 서므로 대삼원이 정식으로 성립한다. 재료로 쓸 잡패가 손에 2장 없거나 리치 중이면 발동할 수 없다.",
  install(ctx) {

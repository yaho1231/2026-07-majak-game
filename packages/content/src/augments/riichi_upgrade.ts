...
export const riichiUpgrade: AugmentDef = defineAugment({
  ...
  name: "이중 선언",
  description:
    "(상시) 리치를 선언하면 언제나 더블리치가 되고, 내 하가(다음 차례 사람)는 내가 그 리치를 지고 있는 동안 리치를 걸 수 없게 된다.",
  detail:
    "(상시) 봉인된 하가는 리치·일발·뒷도라를 통째로 잃는다. 봉인은 **내가 그 리치를 지고 있는 동안**만 이어지고, 이미 리치를 건 사람에게는 소급하지 않으며 봉인 대상은 전원에게 공개된다.\n\n이 증강이 없었어도 더블리치였을 리치라면 트리플리치가 되어 리치가 4판으로 값한다(역만에는 미적용).",
  install(ctx) {
    const { holder } = ctx;

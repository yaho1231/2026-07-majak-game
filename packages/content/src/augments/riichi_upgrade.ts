...
export const riichiUpgrade: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 리치를 선언하면 몇 번째 버림이든 언제나 더블리치(2판)로 승격된다. 동시에 자신의 하가의 리치가 **내가 그 리치를 지고 있는 동안** 봉인되어 그 사람은 리치·일발·뒷도라를 통째로 잃는다. 이미 리치를 건 사람에게는 소급하지 않으며 봉인 대상은 전원에게 공개된다. 이 증강이 없었어도 더블리치였을 리치(첫 버림, 또는 뒤늦은 출진이 승격시킨 7순 이내의 리치)라면 트리플리치가 되어 리치가 4판으로 값한다(역만에는 미적용).",
  install(ctx) {
    const { holder } = ctx;

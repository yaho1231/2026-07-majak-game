...
export const pickyEater: AugmentDef = defineAugment({
  ...
  detail:
    "(2국에 1회) 국이 시작된 뒤 버리는 패를 만·통·삭 중 한 무늬로만 유지하면 된다. 자패는 언제 버려도 무방하며, 그렇게 12장을 버리면 액티브 버튼이 열린다. 발동하면 만·통·삭 중 원하는 색을 골라 손패의 수패를 전부 그 색으로 바꾼다 — 숫자는 그대로 유지되고, 새 패는 패산의 같은 숫자 실물과 맞바꿔 오며, 통일된 색으로 청일색까지 인정된다(단색 세계와 완전히 같은 효과다).\n\n다른 무늬의 수패를 한 장이라도 버리면 그 국의 퀘스트는 즉시 실패하고, 다음 국이 되면 진행도가 처음부터 다시 센다. 12장을 채웠더라도 그 국을 넘기면 사라진다.\n\n버림패는 전부 공개돼 있으므로 상대는 한참 전부터 이 사람이 무엇을 준비하는지 알 수 있다 — 대응할 시간이 아주 길다는 것이 첫 순에 공짜로 물드는 단색 세계와의 차이다.",
  install(ctx) {

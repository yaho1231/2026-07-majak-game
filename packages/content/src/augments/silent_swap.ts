...
export const silentSwap: AugmentDef = defineAugment({
  ...
  detail:
    "(매 국 1회 · 그 국에 아무도 리치를 걸지 않았을 때) 자기 순에 **상대 세 명의 바닥**에서 한 장을 골라 손으로 가져오고, 그 순의 쯔모패는 패산 맨 밑으로 돌아간다. 내 바닥은 대상이 아니다. 리치가 하나라도 걸린 국에서는 발동할 수 없다. 가져온 뒤 이어지는 같은 순의 버림에는 방총 위험이 그대로 적용된다. 원래 주인의 바닥 기록은 남아 그 사람의 후리텐은 유지된다. 발동한 국에 화료하면 +2판을 얻는다(역만에는 미적용).\n\n가져온 패는 그 순의 쯔모패가 된다 — 그 패로 화료할 수 있고, 그때는 쯔모 화료로 값한다. 다만 실체는 남이 버린 패로 나는 것이므로, 내가 후리텐이면 그 패로는 화료할 수 없다. 또한 '리치가 걸리지 않은 국' 조건은 **보이는 리치**만 센다: 스텔스 리치가 서 있어도 발동된다. 쯔모 없이 한 장을 더 얻는 셈이라 그 국은 모두에게 1쯔모만큼 길어진다.",
  install(ctx) {

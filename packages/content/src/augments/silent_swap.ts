 ...
 *   가장 쉬운 사용법이 되어 있었다. 후보가 수십 장이 되어도 클라이언트는 따로 모달을
 *   띄우지 않고 상대 바닥의 **실제 버림패를 클릭해** 고르게 한다(opp-river 무장, 2026-09-25
 *   docs/59 U11 — 같은 패를 두 곳에 그리던 전용 모달을 걷었다). payload는 `{ tileId }`
 *   하나로 유지한다.
...
export const silentSwap: AugmentDef = defineAugment({
  ...
  name: "정적의 손",
  description:
    "아무도 리치를 걸지 않은 국에 쯔모패 대신 상대 바닥에서 1장을 가져올 수 있다. 후리텐이더라도 가져온 패로 쯔모 화료할 수 있다.",
  detail:
    "아무도 리치하지 않은 국에 쯔모패 대신 상대 바닥에서 1장을 가져온다. 가져온 패는 그 순의 쯔모패가 되어, 후리텐이어도 쯔모 화료가 된다.\n\n내 바닥의 패와 내가 버렸던 패는 가져올 수 없다.",
  install(ctx) {

...
export const freeRiichiDiscard: AugmentDef = defineAugment({
  ...
  name: "자유 선언",
  description:
    "(상시) 리치를 걸면 그 순간의 손패로 오름패가 고정되고, 이후 자기 순마다 손패에서 아무 패나 자유롭게 버릴 수 있다.",
  detail:
    "고정된 손패를 기준으로 오름패·화료형·텐파이·후리텐이 판정되고, 화료 시 공개되는 손패도 첫 리치 때의 모습이다.\n\n물리 손패를 아무리 바꿔도 오름패는 변하지 않는다. 대기패를 버리면 후리텐은 그대로 걸리며, 리치 뒤에는 깡을 칠 수 없다.",
  ...
  conflicts: [
    ...
    // 손바닥 뒤집기는 2026-08-15부터 리치를 풀지 않고 **한 번 자유 타패로 대기를 갈아탄다**.
    // 그런데 이쪽은 리치 시점 손패를 스냅샷으로 고정해 오름패가 아예 갱신되지 않으므로,
    // 갈아탄 대기가 한 번도 반영되지 않아 그 증강이 통째로 죽은 픽이 된다(B급 무효).
    // (자유 타패 자체도 이쪽이 상시로 이미 준다.)
    "palm_flip",
  ],
  install(ctx) {

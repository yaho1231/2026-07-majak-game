...
export const doraConceal: AugmentDef = defineAugment({
  ...
  name: "가려진 도라",
  description:
    "(상시) 도라 표시패가 상대들에게는 뒷면으로 덮인다 — 이번 게임의 도라는 당신만 알 수 있다. 단 관련 증강으로 확인하는 도라는 막을 수 없다.",
  detail:
    "(상시) 국이 시작되는 순간부터 도라 표시패가 당신에게만 보이고 세 상대에게는 뒷면으로 덮인다. 증강 자체는 전원에게 공개된다. 표시패는 종국 공개에서 뒤집히며 뒷도라는 표준대로 화료 때만 열린다.",
  install(ctx) {
    const { holder } = ctx;

...
export const honorReturn: AugmentDef = defineAugment({
  ...
  name: "귀환",
  description:
    "(동풍전 1회 · 반장전 2회) 발동 시점까지 이번 국에 내가 버린 자패를 가장 최근 것부터 최대 4장 기억해, 다음 국 배패에 그대로 되받는다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 기억은 **버튼을 누른 순간에 확정된다** — 그 뒤에 버리는 자패(바람·삼원)는 목록에 들어가지 않는다. 무엇이 부활하는지는 발동 즉시 전원에게 공개된다.\n\n되받는 자패는 배패 13장 안에서 교체되어 손패 장수는 그대로다. 자패를 한 장도 버리지 않았거나 리치 중이면 발동할 수 없다.",
  install(ctx) {

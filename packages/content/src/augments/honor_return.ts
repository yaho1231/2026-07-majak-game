...
export const honorReturn: AugmentDef = defineAugment({
  ...
  name: "귀환",
  description:
    "(동풍전 1회 · 반장전 2회) 자기 순에 발동하면 이번 국에 내가 버린 자패를 최대 4장까지 기억해, 다음 국 배패에 그대로 되받는다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 자기 순에 발동하면 이번 국에 버린 자패(바람·삼원)가 가장 최근 것부터 최대 네 장까지 기억되어 다음 국 배패에 그대로 섞여 돌아온다. 되받는 자패는 배패 13장 안에서 교체되므로 손패 장수는 그대로이며, 무엇이 부활하는지는 발동 즉시 전원에게 공개된다. 자패를 한 장도 버리지 않았다면 발동할 수 없다.",
  install(ctx) {

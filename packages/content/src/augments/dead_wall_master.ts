...
export const deadWallMaster: AugmentDef = defineAugment({
  ...
  detail:
    "(상시 열람 · 매 국 2회 교환) 왕패가 항상 전부 보인다. 국 시작 시 14장이지만 **깡의 영상 쯔모는 보충되지 않으므로 누군가 깡을 칠 때마다 한 장씩 줄어든다** — 깡 네 번이 나온 국의 왕패는 10장이다.\n\n자리 구성은 — 왼쪽 4장은 깡으로 뽑는 영상패, 그 뒤 짝수 자리는 도라 표시패, 각 표시패의 오른쪽 옆이 뒷도라 표시패다. 아직 아무것도 버리지 않은 국의 첫 순에 한해 왕패 아무 자리의 패와 손패 1장을 맞바꾸며, 국당 2번까지 반복할 수 있다. 내보낸 손패가 왕패의 그 자리를 대신 채우므로 도라·뒷도라 표시패까지 바꿀 수 있다. 첫 순을 넘기면 그 국의 교환 기회는 사라지고 다음 국에 다시 2장이 채워진다.",

...
export const rinshanPreview: AugmentDef = defineAugment({
  ...
  name: "영상 정찰",
  description:
    "(상시 열람 · 매 국 1회 교환) 다음 깡에서 가져올 영상패(왕패 맨 앞 1장)를 항상 나만 볼 수 있고, 자기 순에 깡을 하지 않고도 그 영상패를 내 쯔모패와 맞바꾼다.",
  detail:
    "(상시 열람 · 매 국 1회 교환) 맞바꾼 쯔모패가 왕패 맨 앞자리로 들어가므로 왕패 장수는 그대로이고, 다음 영상패는 방금 내가 넣은 그 패가 된다.\n\n열람은 나만 하지만 **교환은 전원에게 공개된다** — 다만 내가 넣은 패가 무엇인지는 공개되지 않는다. 남은 영상패가 없으면 열람도 교환도 닫힌다.",
  install(ctx) {

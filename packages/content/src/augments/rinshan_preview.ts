...
export const rinshanPreview: AugmentDef = defineAugment({
  ...
  detail:
    "열람은 **남은 영상패 전부**(최대 4장)다. 깡으로 뽑힌 자리는 보충되지 않으므로 깡이 나올수록 보이는 장수도 줄고, 다 떨어지면 열람도 발동도 닫힌다.\n\n맞바꾼 쯔모패는 왕패 맨 앞자리로 들어가므로 왕패 장수와 도라 표시패 자리는 그대로다. 깡으로 뽑은 패가 아니므로 영상개화는 붙지 않는다.\n\n열람은 나만 하지만 **발동은 전원에게 공개된다** — 순서만 바꿔도 다음에 깡을 치는 사람이 뽑는 패가 달라지기 때문이다. 다만 바뀐 순서도, 내가 넣은 패도 공개되지 않는다.",
  install(ctx) {

...
export const rinshanPreview: AugmentDef = defineAugment({
  ...
  name: "영상 정찰",
  description:
    "(상시 열람 · 매 국 1회) 남은 영상패를 항상 나만 볼 수 있고, 자기 순에 그 순서를 원하는 대로 바꾼다. 그때 한 장을 고르면 내 쯔모패와 맞바꾸고, 고르지 않으면 순서만 확정된다.",
  detail:
    "남은 영상패(최대 4장)를 항상 나만 볼 수 있고, 자기 순에 그 순서를 바꾼다. 그때 한 장을 고르면 내 쯔모패와 맞바꾼다.\n\n발동한 사실은 모두에게 공개되지만, 바뀐 순서와 넣은 패는 공개되지 않는다. 맞바꾼 패로는 영상개화가 성립하지 않고, 영상패가 다 떨어지면 더 사용할 수 없다.",
  install(ctx) {

...
export const voidKan: AugmentDef = defineAugment({
  ...
  name: "성립하지 않는 깡",
  description:
    "(상시) 내가 텐파이면, 타가가 깡을 선언하는 순간 그 깡패가 오름패인지와 상관없이 창깡으로 화료할 수 있다. 안깡도 예외가 아니다.",
  detail:
    "(상시) 텐파이 상태에서 타가가 안깡이나 가깡을 선언하면, 그 깡패가 내 오름패가 되도록 손패 한 장이 그 자리에서 바뀌어 창깡 론이 열린다. 국사무쌍만 안깡을 창깡할 수 있다는 표준 예외도 무력화된다. 추가 점수는 없으며, 대명깡(남의 버림패로 부르는 깡)은 원래 창깡 대상이 아니므로 걸리지 않는다.",
  install(ctx) {
    const { holder } = ctx;

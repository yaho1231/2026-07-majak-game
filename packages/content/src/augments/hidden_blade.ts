...
export const hiddenBlade: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 리치를 선언하지 않은 채 멘젠으로 론 화료하면 +2판이 붙고, 리치를 걸지 않았는데도 뒷도라가 뒤집힌다. 리치 역이 붙는 것이 아니라 판만 얹는 것이라 다른 역이 하나 이상 있어야 하며, 쯔모 화료·후로 화료나 실제로 리치를 건 손에는 붙지 않는다 — 그 국에 리치를 걸었다면 나중에 물리더라도 되살아나지 않는다.",

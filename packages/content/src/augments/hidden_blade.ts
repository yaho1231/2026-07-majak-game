...
export const hiddenBlade: AugmentDef = defineAugment({
  ...
  name: "숨은 칼날",
  description:
    "(상시) 리치를 선언하지 않은 멘젠 론 화료에서 리치를 2판으로 취급하며, 리치를 걸지 않았어도 뒷도라가 적용된다.",
  detail:
    "(상시) 리치를 선언하지 않은 채 멘젠으로 론 화료하면 리치를 2판으로 취급하고, 리치를 걸지 않았는데도 뒷도라가 뒤집힌다. 다른 역이 하나 이상 있어야 하며, 쯔모 화료·후로 화료나 실제로 리치를 건 손에는 붙지 않는다.",
  install(ctx) {
    const { holder } = ctx;

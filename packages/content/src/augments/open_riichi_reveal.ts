 ...
 * 공탁 1000점을 걸고 손이 잠기며, 자신의 손패 전체와 오름패(대기)가 전원에게 공개된다:
...
export const openRiichiReveal: AugmentDef = defineAugment({
  ...
  tier: "gold",
  category: "riichi",
  // 난도 2 (2026-08-27 재평가): 리치 + '오름패가 공개된다' 한 줄. 특수 역·부수 계산이 없다.
  complexity: 2,
  name: "오픈 리치",
  description:
    "손패와 오름패를 공개하고 리치를 건다. 리치를 걸지 않은 사람에게서 방총으로 화료하면 역만이 된다.",
  detail:
    "손패와 오름패를 공개하고 리치한다. 리치를 걸지 않은 사람이 쏘면 그 화료는 역만이 된다.\n\n쯔모로 화료하거나 리치한 사람에게서 론하면 리치가 3판이 된다(역만 제외). 공개된 손패와 오름패는 그 국이 끝날 때까지 모두에게 보인다.\n\n리치는 국당 한 번이므로 리치 증강을 여럿 가지고 있어도 그 국에는 하나만 사용할 수 있다.",
   ...
   *
   * `last_stand`는 리치 자체를 무르는 쪽이고, `palm_flip`은 리치를 유지한 채 한 번
   * 자유 타패로 대기를 바꾼다(2026-08-15 개편 — 공개 목록과 실제 대기가 어긋나는 것은 같다).
   * `tile_dyeing`은 스스로
  ...
  install(ctx) {
    ...
    /*
     * 선언한 국에는 **손패 전체**가 전원에게 공개된다 (오름패 배지는 그대로 유지).
     * 정석 오픈 리치대로 손을 눕히는 쪽이 카드 이름과 맞고, 직격 역만의 대가로
     * 치르는 정보 손해도 여기서 나온다.
     */
    engine.rules.addModifier<VisibilityRule>("visibility.hand", {
      source: ctx.instanceId,
      layer: ctx.layer,
      apply: (cur, rctx) => {
        if (rctx.zoneOwner !== holder) return cur;
        const state = rctx.state as GameState | undefined;
        if (state === undefined) return cur;
        return flagOf(state, declaredKey(state, holder)) ? "public" : cur;
      },
    });

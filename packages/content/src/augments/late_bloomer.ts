...
export const lateBloomer: AugmentDef = defineAugment({
  ...
  name: "대기만성 (반장전)",
  description:
    "(상시 · 반장전 전용 · 게임 시작 드래프트에서만 등장) 남4국(서입 연장 포함)에 들어서면 만개한다 — 그 뒤로는 후리텐을 무시하고 론할 수 있고, 화료에 +3판이 붙는다.",
  draftStages: ["gameStart"],
  // 남4국 템포는 반장전 전용 — 동풍전에는 동4국판(late_bloomer_east)이 대신 나온다.
  modes: ["hanchan"],
  detail:
    "(상시 · 반장전 전용 · 게임 시작 드래프트에서만 등장) 만개 후에는 머리 1개와 몸통 4개(4멘쯔)로 손이 완성되면 역이 없어도 화료할 수 있다. +3판은 역만에는 붙지 않는다.\n\n만개 사실은 전원에게 공개되며, 만개 전까지는 아무 효과도 없다.",
  install(ctx) {
    const { holder } = ctx;

...
export const bottomYaku: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 화료하는 순간 버림패 이력 전체를 판정해 '역류 통관' 2판·'미련 없음' 1판을 얹는다. 남이 후로해 간 버림패도 그대로 세고, 멘젠·후로는 가리지 않는다.\n\n⚠ 둘 다 도라처럼 **판만 더하는 보조역**이라, 손에서 나온 역이 하나도 없으면 화료가 성립하지 않는다. 화료형도 스스로 완성해야 하며 유국에는 적용되지 않는다.",
  install(ctx) {
    const yaku = ctx.yaku;

...
export const bottomYaku: AugmentDef = defineAugment({
  ...
  name: "바닥의 족보",
  description:
    "(상시) 화료 시 내 바닥(버림패)이 판을 얹어 준다 — 한 무늬의 1~9를 모두 버렸으면 '역류 통관' 2판, 같은 패를 3장 이상 버렸으면 '미련 없음' 1판. 둘은 겹쳐 최대 3판. ⚠ 이 둘만으로는 화료할 수 없다 — 손에 진짜 역이 하나는 있어야 한다.",
  detail:
    "(상시) 쯔모·론으로 화료하는 순간 자신의 버림패를 판정해 판을 얹어 준다. 만·통·삭 중 한 무늬의 1~9를 모두 버렸으면 '역류 통관' 2판, 같은 패를 3장 이상 버렸으면 종류를 가리지 않고 '미련 없음' 1판으로 취급되며, 두 조건이 함께 성립하면 3판이다. 판정은 **내가 버린 이력 전체**를 보므로 남이 내 버림패를 후로해 가도 그대로 남는다. 멘젠·후로는 가리지 않는다. ⚠ 이 두 역은 도라처럼 **판만 더하는 보조역**이라 이것만으로는 화료할 수 없다 — 손에서 나온 역(리치·탕야오·역패 등)이 하나라도 있어야 판이 붙고, 역이 하나도 없으면 두 조건을 다 채워도 화료가 성립하지 않는다. 화료형(표준형·치또이·국사 등)도 스스로 완성해야 하며, 유국에는 적용되지 않는다.",
  install(ctx) {
    const yaku = ctx.yaku;

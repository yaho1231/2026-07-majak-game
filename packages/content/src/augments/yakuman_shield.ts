...
export const yakumanShield: AugmentDef = defineAugment({
  ...
  detail:
    "(상시 · 횟수 제한 없음) **역만·셈수역만·유국역만**은 내가 낸 몫이 전액 돌아와 손실이 0이 된다. **배만·삼배만**은 내가 낸 몫의 절반이 돌아온다(100점 단위 내림). **하네만 이하**에는 발동하지 않는다. 직격(론)이면 내가 문 화료점, 쯔모면 내 분담분이 기준이다. 환급분은 전액 뱅크가 내며, 화료자의 수령액은 줄지 않는다(유국역만도 같다).",
  install(ctx) {
    const { holder } = ctx;

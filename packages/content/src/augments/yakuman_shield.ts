 ...
 * 상한이 한 번도 물리지 않아 본장까지 환급됐다(QA defcall 확정 2). 환급분은
 * **전액 뱅크가 발행한다 — 화료자의 수령액은 깎지 않는다**(2026-08-31, 유국역만
 * 경로와 정책 통일). 그래서 방어막이 여럿 깔려도 화료자의 타점은 그대로다.
 *
 ...
 * nagashi 쪽에서 방어막 보유자에게는 지불을 부과하지 않도록 연동한다. 그쪽 면제분도
 * **전액 뱅크가 낸다**(화료자 수령은 줄지 않는다) — 이제 화료 방어와 재원이 같다.
 * 표식이 남지 않는 경로라 아래 리액션이 `drawSpecial`을 직접 보고
 * 막아낸 횟수를 센다.
 */
...
export const yakumanShield: AugmentDef = defineAugment({
  ...
  name: "역만 방어술",
  description:
    "(상시 · 횟수 제한 없음) 역만(유국역만 포함)으로 잃는 점수는 전액, 배만·삼배만으로 잃는 점수는 절반이 돌아온다. 본장·공탁은 돌려받지 못한다.",
  detail:
    "역만(셈수역만·유국역만 포함)으로 잃은 점수는 전액 돌아와 손실이 0이 되고, 배만·삼배만은 절반이 돌아온다(100점 단위 내림). 하네만 이하에는 발동하지 않는다.\n\n돌아오는 점수는 화료한 사람이 아니라 게임이 지급하므로 화료한 사람이 받는 점수는 줄지 않는다. 본장·공탁은 돌려받지 못한다.",
  install(ctx) {
    const { holder } = ctx;
    ...
    settleInterceptor(ctx, SETTLE_STAGE.Shield, (event, ic) => {
      ...
      // 손실 전액 환급 → 보유자 손실 0. 환급분은 뱅크가 발행한다
      // (제로섬 불변식은 아니다 — 프로젝트 허용).
      //
      ...
      /*
       * **환급분은 전액 뱅크가 발행한다 — 화료자의 수령액은 손대지 않는다.**
       *
       * 예전에는 환급분을 그 화료자(들)의 이득에서 (이득 한도까지) 깎았다. 그래서
       * 방어막이 여럿에게 걸리면 화료자 수령이 겹겹이 깎여, 32,000 역만 쯔모에
       * 셋이 방어막을 들었을 때 화료자가 받는 것이 **본장 300점뿐**이었다
       * (QA synergy4 disrupt 의심 항목). 내 손과 무관한 남의 드래프트 결과가
       * 내 타점을 0으로 만든다.
       *
       * 같은 카드의 **유국역만 경로는 이미 정반대 정책을 명시**하고 있다
       * (nagashi_yakuman.ts: "면제분은 뱅크가 낸다 — 화료자 몫은 깎지 않는다").
       * 두 경로가 어긋나 있었으므로 유국역만 쪽으로 통일한다 — 무페널티 원칙
       * (PROJECT_CHARTER): 한쪽을 지키느라 다른 쪽을 손해 보게 하지 않는다.
       *
       * 상한은 따로 두지 않는다. 발행액은 `min(-loss, 내가 그 역만들에 낸 몫)`으로
       * 이미 **보유자의 실제 손실** 안에 갇혀 있어서, 보유자는 아무리 커도 0을
       * 넘겨 이득을 보지 않는다(부호를 뒤집는 `die_hard`가 25,000 상한을 두는 이유가
       * 여기엔 없고, 상한이 없어 테이블 합이 부풀던 `sign_flip`과도 다르다).
       * 역만은 원래 큰 금액이고, 그 전액 면제가 이 카드가 파는 것이다.
       */

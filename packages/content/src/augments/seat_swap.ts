...
export const seatSwap: AugmentDef = defineAugment({
  ...
  name: "자리 바꿈",
  description:
    "(동풍전 2회 · 반장전 3회 · 매 국 1회) 내 첫 순에 상대 한 명을 지정하면 그 자리에서 즉시 자리와 손패를 통째로 맞바꾼다 — 자풍·오야·차례는 물론 상대의 손패·후로까지 가져온다. 리치를 선언한 상대에게는 쓸 수 없다.",
  detail:
    "(동풍전 2회 · 반장전 3회 · 매 국 1회) 그 국에서 내가 아직 한 장도 버리지 않은 내 순이면 상대 한 명을 지정해 즉시 그 상대와 자리와 손을 통째로 맞바꾼다. 상대의 자리(자풍·오야·차례)뿐 아니라 손패와 후로까지 내 것이 되고 내 손은 상대에게 넘어간다 — 내가 방금 뽑은 쯔모패 한 장만 내게 남아 그대로 버림을 이어 간다. 효과는 다음 국이 아니라 그 국에서 즉시 적용된다. 리치한 상대는 지정할 수 없지만, **숨은 리치는 리치가 아닌 사람으로 보이므로 그대로 지정할 수 있고**, 손이 바뀌는 순간 그 리치는 당사자에게만 알려진 채 풀린다.\n\n손패 장수와 후로 개수가 나와 같은 상대만 대상이 된다 — 내가 후로하지 않았다면 이미 후로한 상대는 목록에 뜨지 않는다.\n\n쯔모패가 없는 상태(치·퐁 직후)에는 발동할 수 없고, **내가 리치를 선언한 뒤에는 내 손이 동결되므로 발동할 수 없다**.",
  install(ctx) {
    const { engine } = ctx;

...
export const seatSwap: AugmentDef = defineAugment({
  ...
  name: "자리 바꿈",
  description:
    "(동풍전 2회 · 반장전 3회 · 매 국 1회) 첫 순에 상대 한 명을 지정해 자리와 손패를 통째로 맞바꾼다 — 자풍·오야·차례에 후로까지 넘어온다. 리치를 선언한 상대는 지정할 수 없다.",
  detail:
    "지목한 상대와 자리·손패를 통째로 뒤바꾼다 — 자풍과 오야, 쌓아 둔 후로까지 함께 넘어온다. 내가 아직 한 장도 버리지 않은 순에, 손패 장수와 후로 개수가 같은 상대에게만 쓴다. 쯔모패가 없는 상태(치·퐁 직후)나 내가 리치를 건 뒤에는 발동하지 않는다.\n\n**리치를 선언한 상대는 지정할 수 없다** — 숨은 리치는 그대로 지정할 수 있고, 손이 바뀌는 순간 풀린다.",
  install(ctx) {
    const { engine } = ctx;

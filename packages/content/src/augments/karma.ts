 ...
 * 12,000이 터지면 그 사람이 **-11,200점이 된 채로 그 국을 계속 쳤다** — 토비
...
export const karma: AugmentDef = defineAugment({
  ...
  name: "카르마",
  description:
    "잃은 점수가 게이지로 쌓이고, 8,000 이상이면 사용해서 상대 셋에게서 게이지의 1/3씩 빼앗는다.",
  detail:
    "국 정산에서 잃은 점수가 업보 게이지로 쌓인다. 게이지가 8,000 이상이면 자기 순에 사용해 상대 셋에게서 각각 게이지의 1/3씩 빼앗는다. 금액은 100점 단위로 내림한다.\n\n한 사람에게서 빼앗는 상한은 그 사람의 남은 점수이며, 상한 때문에 못 받은 점수는 사라진다. 사용하면 게이지는 0이 된다. 반전이나 부활로 손실이 플러스가 되어도 원래 잃은 만큼 쌓인다.",
  /*
   ...
   * 태우면 그 국의 적립은 그대로 남지만 태울 기회 자체가 상대의 토비·종국으로 사라진다.
   */
  bot: plan({
    intent: "score",
    fleeting: true,

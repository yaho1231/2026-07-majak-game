...
export const karma: AugmentDef = defineAugment({
  ...
  name: "카르마",
  description:
    "(상시 적립 · 게이지 8,000 이상일 때 발동) **국 정산에서** 점수를 잃을 때마다 그 손실이 '업보' 게이지로 쌓이고(전원 공개), 자기 순에 게이지를 태워 상대 셋에게서 1/3씩 뜯는다 — 받는 것은 실제로 뜯긴 만큼이고, 상대의 남은 점수를 넘는 몫은 그냥 사라진다.",
  detail:
    "(상시 적립 · 게이지 8,000 이상일 때 발동) 국 정산에서 점수를 잃으면 방총이든 쯔모당함이든 그 손실액이 업보 게이지에 그대로 적립되고, 게이지 수치는 전원에게 공개된다. 게이지가 8,000 이상이면 자기 순에 게이지를 태워 상대 세 명에게서 뜯어낸다(뱅크가 아니라 상대 주머니에서 나온다).\n\n**실제로 받는 액수는 게이지보다 적을 수 있다.** 1인당 몫은 게이지를 3으로 나눈 뒤 100점 단위로 내림한 값이고, 여기에 **그 사람의 남은 점수**라는 상한이 또 걸린다. 못 받은 몫은 다른 사람에게 넘어가지 않고 그대로 사라진다.\n\n적립되는 것은 **국 정산의 증감뿐**이다 — 리치 선언으로 낸 공탁이나 다른 사람의 업보에 뜯긴 점수처럼 국 중간에 빠져나가는 점수는 쌓이지 않는다.\n\n태우면 게이지는 (얼마를 받았든) 0이 되고 다시 처음부터 쌓인다.",

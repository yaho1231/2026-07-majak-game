...
export const karma: AugmentDef = defineAugment({
  ...
  detail:
    "(상시 적립 · 게이지 8,000 이상일 때 발동) 국 정산에서 점수를 잃으면 방총이든 쯔모당함이든 그 손실액이 업보 게이지에 그대로 적립되고, 게이지 수치는 전원에게 공개된다. 게이지가 8,000 이상이면 자기 순에 게이지를 태워 상대 세 명에게서 뜯어낸다(뱅크가 아니라 상대 주머니에서 나온다).\n\n**실제로 받는 액수는 게이지보다 적을 수 있다.** 1인당 몫은 게이지를 3으로 나눈 뒤 100점 단위로 내림한 값이고, 여기에 **그 사람의 남은 점수**라는 상한이 또 걸린다(빈털터리에게서는 더 못 뜯는다 — 국 도중에 남을 마이너스로 만들지 않기 위해서다). 못 받은 몫은 다른 사람에게 넘어가지 않고 그대로 사라진다. 예를 들어 게이지 12,000이라도 한 명이 800점만 남았다면 그 사람에게서는 800점만 오고 나머지 3,200점은 없던 것이 된다.\n\n태우면 게이지는 (얼마를 받았든) 0이 되고 다시 처음부터 쌓인다.",

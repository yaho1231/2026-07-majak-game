...
export const graveRob: AugmentDef = defineAugment({
  ...
  name: "무덤 도굴",
  description:
    "(동풍전 1회 · 반장전 2회) 자기 순에 상대들이 **최근에 버린 10장** 중 1장을 파내 그대로 화료한다. 지불은 쯔모 취급으로 세 명이 분담하며, 후리텐이면 화료할 수 없다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 화료가 성립하는 패만 후보로 뜬다. 그보다 오래전에 흘린 패, 안개로 가려진 패, 내 바닥의 패는 파낼 수 없다. 그 순의 쯔모패는 패산으로 돌아간다.\n\n원주인의 바닥 기록은 남아 그 사람의 후리텐 판정도 유지된다. 남이 버린 패로 나는 것이라 **내가 후리텐이면** 그 패는 후보에도 오르지 않는다.",
  install(ctx) {

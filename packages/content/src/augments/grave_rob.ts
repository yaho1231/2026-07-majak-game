 ...
 * 도굴은 버림을 소비하지 않으므로 곧바로 같은 턴의 turn.act로 돌아오는데, 그때
 * `turn.autoWin` 규칙(아래 install)이 켜져 있어 FlowController가 프롬프트를 열지 않고
 * **표준 쯔모를 곧바로 성립시킨다** — 정산은 `win` 액션과 같은 sys.settleWin이다.
 * 화면에서는 "도굴 → 화료"가 한 박자로 이어진다(2026-08-31 이전에는 «쯔모»를 다시 눌러야
 * 했다 — 옛 주석은 그 두 박자를 적고 있었다, 2026-09-25 docs/59 정정).
 * 원주인의 바닥 기록(discardedKinds)은 건드리지 않아 그 상대의 후리텐 판정은 그대로 유지된다.
 */
...
/**
 * 파낼 수 있는 무덤의 깊이 — **최근 버림 6장**까지만.
 *
 * 예전에는 상대 셋의 바닥 **전체**가 대상이라, 순이 쌓일수록 후보가 수십 장으로 불어나
 * 사실상 "언젠가 한 번은 반드시 화료"가 됐다. 무덤이 얕아야 "지금 저 패가 아직 위에
 * 있을 때 파낸다"는 타이밍 판단이 생긴다.
 *
 * ⚠ 밸런스 2026-08-27: **10 → 6**. 10장은 상대 셋이 대략 세 순 남짓 버린 양이라
 * 창이 거의 늘 열려 있었다 — "지금 파낸다"는 판단이 사실상 "아무 때나 파낸다"였다.
 * 6장이면 상대 셋의 최근 두 순 정도라, 노린 패가 밀려나기 전에 써야 한다.
 */
const GRAVE_DEPTH = 6;

/**
 * 상대들의 바닥에서 **가장 최근 GRAVE_DEPTH장(6장)** (내 바닥은 후리텐 존중으로 제외).
 *
...
export const graveRob: AugmentDef = defineAugment({
  ...
  name: "무덤 도굴",
  description:
    "(동풍전 1회 · 반장전 2회) 자기 순에 상대들이 최근에 버린 6장 중 1장을 골라 그 패로 화료한다. 지불은 쯔모 취급으로 세 명이 나눠 내며, 후리텐이면 화료할 수 없다.",
  detail:
    "텐파이일 때 능력을 사용하고 표시된 후보 중 1장을 고르면 바로 화료한다. 쯔모 버튼은 따로 누르지 않는다.\n\n후보는 상대 셋이 최근에 버린 6장 중 내 화료패만 표시된다. 내 바닥의 패와 안개에 가려진 패는 고를 수 없고, 내가 후리텐이면 후보가 표시되지 않는다. 리치 중에는 대부분 후리텐이므로 사실상 사용할 수 없다.\n\n지불은 쯔모 취급이므로 상대 셋이 나눠 낸다.",
  install(ctx) {

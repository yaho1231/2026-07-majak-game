...
export const graveRob: AugmentDef = defineAugment({
  ...
  detail:
    "후보는 상대 셋의 바닥을 시간 순으로 훑어 **가장 최근 6장**까지다 — 그보다 오래전에 흘린 패는 이미 창 밖이라 파낼 수 없다. 그중 화료가 성립하는 패만 후보로 뜨고, 지불은 쯔모 취급이라 상대 셋이 나눠 낸다. 안개로 가려진 패, 안개로 가려진 패, 내 바닥의 패는 파낼 수 없다. 그 순의 쯔모패는 패산으로 돌아간다.\n\n원주인의 바닥 기록은 남아 그 사람의 후리텐 판정도 유지된다. **내가 후리텐이면** 그 패는 후보에 오르지 않는다.",
  install(ctx) {

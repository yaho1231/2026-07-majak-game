...
export const haiteiLord: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 패산의 마지막 한 장을 자신이 뽑을 때 텐파이였다면, 대기와 무관하게 그 패가 오름패가 되어 해저로월 쯔모로 화료하고 **+3판**이 붙는다(역만에는 미적용).\n\n텐파이가 아니면 아무 일도 일어나지 않으며, 영상 쯔모는 해저패가 아니므로 발동하지 않는다. 해저패를 만지는 다른 증강보다 이쪽이 우선한다.",
  install(ctx) {

 ...
 *
 * 아래 리액션의 조건을 그대로 담은 순수 함수다. 함수로 뽑은 이유는 `conjure_draw`·
 * `giant_god`가 **같은 한 장**에 `tileKindChanged`를 쏘기 때문이다. 둘 이상이 쏘면
 * 나중에 설치된 쪽, 즉 **드래프트 픽 순서**가 승패를 정했고 진 쪽은 조용히 죽었다
 * (2026-08-23 QA synergy3 kandora 확정 2 · 2026-08-31 synergy4 handedit 확정 1).
 *
 * 이 함수는 그 조정의 **최상위 술어**다 — 세 카드가 전부 `drawMutators.ts`를 통해
 * 이것을 읽고, 진 쪽이 스스로 물러난다(예약은 남긴다). 우선순위와 근거는 그 파일에
 * 모여 있다. "이미 다른 증강이 바꿨나"를 보는 방식으로는 못 고친다 — 그게 바로
 * 순서 의존이다.
 *
...
export const haiteiLord: AugmentDef = defineAugment({
  ...
  detail:
    "텐파이 상태로 패산 마지막 패(해저패)를 쯔모하면 대기와 상관없이 그 패가 오름패가 되어 화료하고 +3판이 붙는다. 역만에는 붙지 않는다.\n\n깡의 영상패 쯔모는 해저패가 아니다.",
  install(ctx) {

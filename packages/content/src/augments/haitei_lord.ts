...
export const haiteiLord: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 패산의 마지막 한 장을 자신이 뽑고 그때 텐파이였다면, 그 패가 무엇이든 자신의 오름패로 바뀐다. 무엇을 기다리고 있었는지는 상관없으며 그대로 해저로월 쯔모로 화료할 수 있고 +3판을 얻는다(역만에는 미적용). 텐파이가 아니면 아무 일도 일어나지 않으며, 영상 쯔모는 해저패가 아니므로 발동하지 않는다.",
  install(ctx) {

...
export const triplePeek: AugmentDef = defineAugment({
  ...
  detail:
    "(2국에 1회) 자기 순에 선언하면 앞으로 내게 배정될 다음 쯔모 세 장의 '종류'가 나에게만 공개된다 — 실제 패가 아니라 무엇이 올지 그 종류만 안다. 한 번 켜면 **그 국이 끝날 때까지 계속** 보이고, 누군가 퐁·치·깡을 해 쯔모 차례가 밀려도 **지금 기준으로 다시 계산**되어 어긋나지 않는다. 국 끝물에 내 몫의 쯔모가 세 장이 안 남으면 남은 만큼만 보인다. 무엇을 봤는지는 나만 알고 상대에게는 발동 사실만 공개된다. 한 번 쓰면 **다음 국은 건너뛰고** 그 다음 국에 다시 채워진다.\n\n선언 시점에 내 몫의 쯔모가 3장 남지 않았으면 발동할 수 없다.",

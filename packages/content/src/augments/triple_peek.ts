...
export const triplePeek: AugmentDef = defineAugment({
  ...
  detail:
    "(매 국 1회) 자기 순에 선언하면 앞으로 내게 배정될 다음 쯔모 세 장의 '종류'가 나에게만 공개된다 — 실제 패가 아니라 무엇이 올지 그 종류만 안다. 한 번 켜면 **그 국이 끝날 때까지 계속** 보이고, 쯔모·버림·후로로 판이 움직일 때마다 **지금 기준으로 다시 계산**된다. 내가 한 장 뽑으면 앞의 한 장이 빠지고 뒤에서 새 한 장이 들어오며, 누군가 펑·치·깡을 해 쯔모 차례가 밀려도 그 자리에서 배정을 다시 잡아 어긋나지 않는다. 국 끝물에 내 몫의 쯔모가 세 장이 안 남으면 남은 만큼만 보인다. 무엇을 봤는지는 나만 알고 상대에게는 발동 사실만 공개되며, 사용 횟수는 국이 바뀌면 다시 채워진다.\n\n선언 시점에 내 몫의 쯔모가 3장 남지 않았으면 발동할 수 없다.",

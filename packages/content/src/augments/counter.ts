...
export const counter: AugmentDef = defineAugment({
  ...
  name: "카운터",
  description:
    "(매 국 1회) 나보다 먼저 리치를 건 상대에게 추격 리치로 반격한다 — 내 공탁 1,000점을 그 상대가 대납하고 그 상대의 일발이 사라진다.",
  detail:
    "(매 국 1회) 대상은 그 국에 **스스로** 가장 먼저 리치를 건 한 사람뿐이고(남이 강제한 리치는 대상이 아니다), 대납액은 그 상대의 남은 점수까지만이다. 내가 공탁을 내지 않는 리치로 추격하면 그 1,000점은 내 점수로 들어온다.\n\n내가 먼저 화료하면 그 상대가 받았을 점수를 뱅크에서 더 받고, 그 상대의 버림패를 직격 론으로 잡으면 +4판까지 얻는다.",
  install(ctx) {
    ...
    ctx.reaction(TILE_DISCARDED, (event, rc) => {
      ...
      if (p.player !== holder) {
         ...
         * 대납 1,000 + 직격 +4판을 자기 의사 없이 한 번에 맞았다(실측 한 국 +22,000).
       ...
       * 앞에 있어서 일발 소멸도, 반격 플래그(= 손 가치 강탈 · 직격 +4판의 전제)도,

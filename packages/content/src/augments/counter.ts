...
export const counter: AugmentDef = defineAugment({
  ...
  detail:
    "추격 리치를 거는 순간 내 공탁 1,000점을 그 상대가 대납하고 그 상대의 일발이 사라진다. 대상은 그 국에 스스로 가장 먼저 리치를 건 한 사람이고(남이 강제한 리치는 대상이 아니다), 대납액은 그 상대의 남은 점수까지만이다.\n\n내가 그 국을 먼저 화료하면(그 상대가 함께 오르지 않았다면) 그 상대의 손이 올랐을 때 받았을 최고 점수를 뱅크에서 더 받는다. 그 상대의 버림패를 **직격 론**으로 잡으면 여기에 +3판이 더 붙는다.",
  install(ctx) {

...
export const ankanDora: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 안깡을 하는 순간 그 깡에 들어간 네 장이 자신에게만 도라로 점등되어 화료 시 +4판이 된다. 표준 깡도라 표시패는 평소대로 뒤집히고 그와는 별개다. 안깡을 여럿 하면 묶음마다 +4판씩 쌓인다. 도라가 되는 것은 깡에 들어간 그 네 장뿐이라 손패에 같은 종류의 패가 남아 있어도 판이 붙지 않으며, 랭크가 섞인 깡(장사진의 3-4-5-6, 바람의 계보의 동남서북)도 네 장 전부 도라가 된다. 상대 손에는 적용되지 않고, 역만 손에는 추가 판을 얻지 않는다.",
  install(ctx) {
    const { holder } = ctx;

...
export const ankanDora: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 안깡을 하는 순간 그 패 종류가 자신에게만 도라로 점등된다. 도라 표시패는 뒤집지 않으며, 화료 시 손패와 후로에 있는 그 종류의 패가 전부 도라로 값한다(장당 1판). 안깡을 여럿 하면 각 종류가 모두 개인 도라가 된다. 다만 상대 손에는 적용되지 않고, 역만 손에는 추가 판을 얻지 않는다.",
  install(ctx) {
    const { holder } = ctx;

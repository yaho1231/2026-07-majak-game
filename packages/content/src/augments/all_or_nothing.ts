...
export const allOrNothing: AugmentDef = defineAugment({
  ...
  detail:
    "(매 국 1회) 리치 선언과 동시에 올인을 건다. 현재 점수의 절반(1000점 단위 내림)이 판돈으로 전원에게 공개되고, 그 리치로 화료하면 판돈과 같은 금액을 뱅크에서 추가로 받는다. 반대로 타가가 론이나 쯔모로 화료하면 판돈의 절반이 뱅크로 넘어간다 — 절반만 돌려받는 셈이다. 유국으로 끝난 국은 아무도 이기지 않았으므로 잃지 않으며, 그 리치가 풀리면(승부수) 판돈도 함께 사라진다.",
  install(ctx) {

...
export const aotenjouCeiling: AugmentDef = defineAugment({
  ...
  name: "뚫린 천장",
  description:
    "(상시) 내 화료에서 만관·하네만·배만 같은 단계 상한이 사라진다 — 만관 위의 판이 잘리지 않고 두 판마다 만관 하나씩 그대로 쌓이며, 늘어난 몫은 타가가 낸다.",
  detail:
    "(상시) 내가 화료할 때 점수 단계 상한이 적용되지 않는다. 만관(5판) 위로는 판이 둘 오를 때마다 만관 한 개분이 더 붙어 8판이면 만관 2.5개, 역만(13판 환산)이면 만관 5개가 되며 상한 없이 계속 늘어난다. 만관 미만에서도 상한이 없어 4판 40부처럼 만관으로 깎이던 손이 제 값을 받는다. 늘어난 점수는 뱅크가 아니라 지불자에게서 가져온다 — 론이면 방총자가, 쯔모면 표준 비율대로 셋이 나눠 낸다.",
  install(ctx) {
    const { holder } = ctx;

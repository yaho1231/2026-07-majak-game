...
export const tableFlip: AugmentDef = defineAugment({
  ...
  name: "밥상 뒤엎기",
  description:
    "(매 국 1회) 첫 순에 손패를 전부 반납하고 패산에서 새 손패를 받는다 — 반납한 손패는 전원에게 공개된다.",
  detail:
    "(매 국 1회) 국의 첫 순에 마음에 안 드는 배패를 통째로 엎을 수 있다. 손패는 패산 맨 밑으로 반납되고 패산 위에서 같은 장수를 새로 받으며 패산 총량은 변하지 않는다. 엎어서 반납한 손패는 **그 순의 쯔모패까지 포함해** 발동 순간 전원의 화면에 잠깐 펼쳐졌다가 사라진다 — 무엇을 버렸는지 상대가 다 보므로 새 손의 방향까지 읽힌다.\n\n**쯔모 없이 맞은 순(치·퐁 직후)에는 발동할 수 없다.**",
  install(ctx) {

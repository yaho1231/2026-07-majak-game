...
export const nagashiYakuman: AugmentDef = defineAugment({
  ...
  detail:
    "(상시) 황패유국 시, 그 국에 내가 버린 패가 **한 장도 빠짐없이** 요구패(1·9 수패)나 자패면 유국만관이 역만이 된다.\n\n" +
    "바뀌는 것은 하나 — **울려도 무효가 되지 않는다.** 정통 규칙에서는 내 버림패를 누가 울어 가는 순간 사라지지만 여기서는 그대로 성립한다. 대신 판정은 **내가 버린 모든 패**를 보므로, 요구패가 아닌 패를 한 장이라도 버렸다면 그것이 울려 나갔어도 성립하지 않는다.\n\n" +
    "지불은 쯔모 역만과 같다 — 오야면 각 16,000점, 자면 오야에게 16,000점·자에게 8,000점씩 받으며 노텐 벌점과 함께 정산된다. 역만 방어술을 든 상대는 면제되지만 그 몫은 뱅크가 대신 내므로 내 수령액은 줄지 않는다.",
  install(ctx) {
    const { holder } = ctx;

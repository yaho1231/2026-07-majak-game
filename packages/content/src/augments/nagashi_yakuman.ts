...
export const nagashiYakuman: AugmentDef = defineAugment({
  ...
  detail:
    "황패유국 시 내가 버린 패가 **한 장도 빠짐없이** 요구패나 자패여야 유국만관이 역만으로 바뀐다 — 아닌 패를 한 장이라도 버렸다면 그것이 울려 나갔어도 성립하지 않는다.\n\n지불은 쯔모 역만과 같고 노텐 벌점과 함께 정산된다. 역만 방어술을 든 상대는 이 지불에서 면제되지만 내 수령액은 줄지 않는다.",
  install(ctx) {
    const { holder } = ctx;

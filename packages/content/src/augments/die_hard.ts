...
export const dieHard: AugmentDef = defineAugment({
  ...
  detail:
    "(동풍전 1회 · 반장전 2회) 국 정산 결과 점수가 0 미만이 되면 내려간 깊이만큼 부호가 뒤집혀 그대로 점수가 된다(−8000점 → +8000점). 정산 결과가 0 이상이면 발동하지 않으며, 도비 판정보다 먼저 반영되어 그 자리에서 되살아난다. 되돌려 받은 몫은 결과 화면의 증감에도 함께 표시된다.",
  install(ctx) {
    const { holder } = ctx;

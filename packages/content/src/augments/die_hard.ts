...
export const dieHard: AugmentDef = defineAugment({
  ...
  name: "죽기살기",
  description:
    "(동풍전 1회 · 반장전 2회) 국 정산 결과 점수가 0 아래로 떨어지는 순간, 마이너스로 내려간 만큼을 그대로 플러스로 되돌려 받는다 — −8000점이 되면 즉시 +8000점. 역만 방어술·천하무적·불가침 조약·승승장구와는 함께 가질 수 없다.",
  detail:
    "(동풍전 1회 · 반장전 2회) 국 정산 결과 점수가 0 미만이 되면 내려간 깊이만큼 부호가 뒤집혀 그대로 점수가 된다(−8000점 → +8000점). 정산 결과가 0 이상이면 발동하지 않으며, 토비 판정보다 먼저 반영되어 그 자리에서 되살아난다. 되돌려 받은 몫은 결과 화면의 증감에도 함께 표시된다.\n\n**역만 방어술·천하무적·불가침 조약·승승장구와는 함께 가질 수 없다** — 네 종 모두 '크게 잃는 국'을 없애는 방어라 이 증강의 발동 조건 자체를 지워 버린다. 그중 하나를 이미 들고 있으면 이 카드는 후보에 뜨지 않는다(반대도 같다).",
  /*

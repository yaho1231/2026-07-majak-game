 ...
 *
 * 황패유국 정산에서 보유자는 **손이 어떻든 항상 텐파이로 취급**된다 — 노텐 벌점을 내지
...
export const alwaysTenpai: AugmentDef = defineAugment({
  ...
  name: "승승장구",
  description:
    "(상시) 황패유국 시 손이 어떻든 항상 텐파이로 취급된다 — 노텐 벌점을 내지 않고, 노텐인 상대 한 명당 2000점을 추가로 받는다(셋 다 노텐이면 +6000).",
  detail:
    "(상시) 황패유국 정산에서 실제 손이 노텐이어도 텐파이로 취급되어 노텐 벌점을 내지 않고 표준 텐파이 분배를 받는다. 여기에 더해 노텐인 상대 한 명당 2000점을 추가로 받는다(셋 다 노텐이면 +6000). 오야일 때 텐파이 취급이 되면 연장(렌짱)도 그대로 적용된다. 누군가 화료해 국이 끝나면 아무 일도 일어나지 않는다.",
  install(ctx) {
    const { holder } = ctx;

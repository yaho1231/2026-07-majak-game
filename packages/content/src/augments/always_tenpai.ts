...
export const alwaysTenpai: AugmentDef = defineAugment({
  ...
  name: "승승장구",
  description:
    "(상시) 황패유국 때 손패가 어떻든 항상 텐파이로 취급한다. 노텐 벌점을 내지 않고, 노텐인 상대 한 명당 2,000점을 추가로 받는다.",
  detail:
    "황패유국 때 손패가 어떻든 텐파이로 취급되고, 노텐인 상대에게서 각각 2,000점씩 더 받는다.\n\n황패유국 때 손패가 어떻든 텐파이로 취급하고, 노텐인 상대에게서 각각 2,000점씩 더 받는다.\n\n유국 정산에만 적용된다. 텐파이가 조건인 다른 증강(모래시계·미련)은 실제 손패로 판정한다.",
  install(ctx) {
    const { holder } = ctx;

...
export const hourglass: AugmentDef = defineAugment({
  ...
  detail:
    "황패유국이 선언되는 순간 내가 텐파이면 국이 끝나지 않고, 남은 영상패(최대 4장)를 나 혼자 연속으로 쯔모한다.\n\n그 국에 깡이 있었으면 그만큼 짧아지고, 영상패가 없으면 발동하지 않는다. 내 유국만관·유국역만이 성립한 국에는 연장하지 않는다. 연장 중 내 버림패는 평소대로 론 대상이다.",
  install(ctx) {

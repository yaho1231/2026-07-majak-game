...
describe("확정 3 · frame_up × bottom_yaku", () => {
  /**
   * p1(피해자)은 1만~6만만 버렸다(숫자 6종). p0 이 **9만을 p1 바닥에 심으면** 그 9만이
   * 7종째가 되는데, 예전에는 심긴 패가 p1 의 후리텐 이력에 들어가 p1 의 역류 통관(+2판)을
   * 공짜로 완성시켰다.
   *
   * ⚠ 2026-08-27 밸런스로 역류 통관 문턱이 9종 → **7종**이 됐다. 원래 이 장면은
   * "1~8만 + 심긴 9만 = 완주"였는데, 8종만으로 이미 성립해 버려 장면이 무의미해졌다.
   * 문턱보다 한 종 모자란 자리(6종)로 옮겨 **같은 것**을 검증한다.
   */
  it("심긴 9만은 피해자의 역류 통관을 완성시키지 않는다", () => {
    const base = withAugments(
      craft({
        ...
        discards: { p0: "1z", p1: "123456m", p2: "5z", p3: "6z" },
        phase: "turn.act",
        turnSeat: 0,
        drawnLastFor: "p0",
      }),
  ...
  it("보유자가 누명으로 흘린 9만은 자기 역류 통관에 그대로 남는다", () => {
    const base = withAugments(
      craft({
        ...
        // 6종만 버린 상태 — 누명으로 흘리는 9만이 7종째가 되어야 통관이 선다
        // (2026-08-27 문턱 9종 → 7종에 맞춰 조정. 예전 값 "12345678m"이면 9만 없이도 성립한다)
        discards: { p0: "123456m", p1: "2z3z", p2: "5z", p3: "6z" },
        phase: "turn.act",
        turnSeat: 0,
        drawnLastFor: "p0",
      }),

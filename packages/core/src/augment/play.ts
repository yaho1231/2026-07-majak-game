...
export const AUGMENT_PLAY: Readonly<Record<string, AugmentPlay>> = {
  ...
  genesis: {
    ...
    value: 1.1,
    /**
     * `hanBonus`는 8이었다 (자일색 + 대사희 = 더블 역만의 어림값). 그 값이면 봇이 보는
     * 기대 실점이 **자패 한 장에 2100점**이 되어, 어떤 손을 들고 있든 어떤 성격이든
     * 밀기 EV가 이길 수 없었다 — 개벽이 한 번 터지면 봇 셋이 전부 화료를 포기하고
     * 국이 끝날 때까지 자패를 한 장도 안 냈다(2026-09-08 사용자 보고).
     *
     * 8은 **최선의 경우**지 기대값이 아니다. 뒤집힌 열세 장은 무작위 자패 더미라
     * 실제로 서는 것은 자일색보다 혼일색·역패·소사희 쪽이 훨씬 잦고, 대개는 아예
     * 안 선다. 5로 내리면 같은 자리의 기대 실점이 1400점이 되어 **성격이 갈린다** —
     * 공격형·속공형·변덕형은 값나가는 텐파이를 들고 자패를 밀고, 수비형은 접는다.
     * 무서워하는 것 자체는 그대로다(평시의 25배). 판단이 성격보다 커지지 않을 뿐이다.
     */
    fired: { kind: "flag", danger: "honor", riskMul: 2.6, hanBonus: 5, minLevel: 0.35 },
  },
  /**

...
export const ironWall = defineAugment({
  ...
  detail:
    "자신이 이미 버린 패로도 론할 수 있어 후리텐이라는 개념이 사라진다. 실제로 후리텐 상태에서 론으로 잡아낸 화료에서는 +3판을 얻는다. 후리텐이 아닌 평범한 론에는 아무것도 얹히지 않는다.",
  install(ctx) {
...
export const openRiichi = defineAugment({
  ...
  name: "개문선언",
  description:
    "(상시) 리치의 멘젠 조건이 사라져 후로한 손으로도 리치를 선언할 수 있다. 후로한 채 리치로 화료하면 그 리치를 2판으로 취급한다. 일발·뒷도라는 그대로 붙지만, 손이 멘젠이 되는 것은 아니다.",
  detail:
    "치·퐁·깡으로 손을 연 뒤에도 리치를 선언할 수 있다. 손을 연 상태로 건 리치로 화료하면 그 리치는 2판으로 값한다. 원래 되는 멘젠 리치에는 아무것도 얹히지 않는다.\n\n일발·뒷도라·더블리치는 후로 손에서도 그대로 붙는다. 다만 **손이 멘젠이 되는 것은 아니라** 멘젠쯔모·핀후·치또이처럼 멘젠이어야 성립하는 역과 멘젠론 부수는 붙지 않는다.",
  install(ctx) {
...
export const yakulessWin = defineAugment({
  ...
  detail:
    "머리 1개와 몸통 4개(4멘쯔)로 손이 완성되면 역이 하나도 없어도 그대로 화료할 수 있고, 그렇게 역 0개로 오른 화료는 2판으로 취급된다. 역이 있는 손에는 아무것도 얹히지 않는다.",
  install(ctx) {

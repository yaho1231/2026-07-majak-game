...
export const openRiichi = defineAugment({
  ...
  detail:
    "(상시) 리치의 멘젠 조건이 사라져 치·펑·깡으로 손을 연 뒤에도 리치를 선언할 수 있다. 후로한 상태로 리치를 걸어 화료하면 그 리치를 2판으로 취급한다. 원래부터 되는 멘젠 리치에는 아무것도 얹히지 않는다. **풀리는 것은 '리치를 걸 수 있는가'뿐이고 손이 멘젠으로 취급되지는 않는다** — 후로한 손은 그대로 후로한 손이라 멘젠쯔모·핀후·치토이 같은 멘젠 한정 역이나 멘젠론 부수는 붙지 않는다.",
  install(ctx) {

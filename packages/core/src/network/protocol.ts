 ...
 *
 * `seat`은 **가려는 자리 번호**다(0=동 1=남 2=서 3=북).
 *
 * - 그 자리가 **비어 있으면 거기 앉는다.** 남은 사람들은 그대로다 — 자리가 곧 방위라서,
 *   동남서가 비어 있어도 북에 앉을 수 있다 (2026-09-09 사용자 지시).
 * - 그 자리에 **사람이 있으면** 앉아 있는 사람들 사이의 «잘라 붙이기»다: 집은 사람을
 *   빼고 그 자리에 끼워 사이에 있던 사람들이 한 칸씩 밀린다(빈자리 위치는 그대로).
 *   두 자리만 맞바꾸면 나머지의 상대 위치가 뜻하지 않게 뒤집히기 때문이다.
 */
export interface MoveSeatMessage {
  type: "moveSeat";
  /** 옮길 사람(또는 봇)의 좌석 id. */
  playerId: PlayerId;
  /** 가려는 자리 번호 (0-based, 빈자리도 된다). 범위 밖이면 서버가 조용히 무시한다. */
  seat: number;
}
...
export interface RoomRules {
   ...
   * 올라스에 1위가 이 점수 미만이면 남입/서입으로 넘어간다(이 점수 이상이면 종료).
   * 오야 연장(아가리야메)도 오야가 이 점수 이상일 때만 판을 접는다.
   */
  returnScore: number;
  /** 토비 — 0점 이하가 나오면 즉시 종료. */
  dobi: boolean;
...
export interface PromptMessage {
  ...
  deadlineMs?: number;
  /**
   * `deadlineMs` 중 **초읽기 은행**에서 나온 몫(ms).
   *
   * 제한 시간은 «매 순 초기화되는 유예(숙련자 30초) + 이번 국에 남은 은행(10초)»
   * 둘의 합인데, 화면에는 그 합이 «40초» 한 덩어리로 보였다 — 어느 쪽이 줄고 있는지,
   * 이번 순을 넘기면 무엇을 잃는지가 숫자에 전혀 없었다(2026-08-30 사용자 지시).
   * 이 값을 실어 보내면 클라이언트가 «30 + 10초»처럼 갈라서 세고, 유예가 먼저
   * 닳는 실제 소비 순서를 그대로 보여 준다.
   *
   * 없거나 0이면 갈라지지 않은 한 덩어리다(튜토리얼·초읽기 증강 국·구 서버).
   */
  bankMs?: number;
}
 ...
 *
 * 예전에는 "대국 종료"뿐이라, 남2국에서 갑자기 순위표가 뜨면(토비) 버그로 읽혔다.
...
export interface SpectateInsightMessage {
  ...
  /**
   * **클라이언트가 리플레이에서 직접 낸 값**이다 (서버를 거치지 않는다). 봇 모형이
   * 서버에만 있어 노텐 «추정»과 위험도 색칠이 없다 — 화면은 그 두 자리를 «없다»가
   * 아니라 «리플레이에서는 계산하지 않는다»로 적어야 한다.
   */
  replay?: true;
}

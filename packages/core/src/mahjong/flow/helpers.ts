 ...
 * 빼려고 백을 **문자 그대로 백으로** 두고 재는데(`furitenOptionsOf`), 조커 덕분에
 * 텐파이가 된 손은 그 계산에서 대기가 0개로 나와(`waits.length === 0`) 후리텐이 아예
 * 성립하지 않았다. 실제로 분열+조커+개벽 손이 **자기가 버린 패로 론**하는 것이
 * 보고됐다(2026-09-04, #467 — #469에 지워졌다가 2026-09-16 재적용).
 *
 ...
 * 화료패 종류만 자기 버림패 이력(`discardedKinds`)과 맞춰 본다. 후리텐을 끄는 증강
 * (대기만성 등)은 `win.furiten.enabled` 게이트 안쪽이라 영향이 없다.
 */

...
export interface YakuDef {
  ...
  /**
   * 독점 역만 — 이 역이 성립하면 **다른 역만을 전부 버리고** 이 역 하나로만 센다.
   * 위그드라실(발로만 화료 = 더블 역만)처럼 "다른 역만과 중첩되지 않는다"를 약속하는
   * 역용이다. 발 14장 손은 소삼원·자일색·스안커가 저절로 함께 서므로 이 표식이 없으면
   * 복합 합산이 약속을 넘는다.
   */
  exclusiveYakuman?: boolean;
  /**

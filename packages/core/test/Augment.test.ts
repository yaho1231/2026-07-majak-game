...
describe("DraftController — 개인별 3지선다", () => {
  ...
  /*
   * 채움 카탈로그. **좌석 4 × 6장(화면 3 + 교체 3) = 24장 이상**이어야 한다 —
   * 2026-08-22부터 좌석 칸 폴백이 좌석끼리 겹치지 않게 뽑으므로(QA 2차 synergy
   * 확정 1), 그보다 작으면 뒤쪽 좌석의 교체분이 짧아져 이 절의 검사가 «폴백이
   * 좁아서» 실패한다 — 재고 싶은 것은 그게 아니다. 티어 3 × 12 = 36장으로 둔다
   * (그래도 칸 임계 54 아래라 폴백 경로 자체는 그대로 지난다).
   */

...
- **게임 시작 시 1회 + 남장 진입 시 1회 — 게임 전체에서 총 2개.**
- 각 플레이어에게 **개인별 랜덤 3개 제시 → 1개 선택**.
- 3개 뽑기도 GameState의 시드 PRNG를 사용한다 (리플레이 보장).
- 스케줄(`augment.draft.schedule`)과 드래프트 로직 자체도 Rule/모듈이라,
  이후 "매 국 드래프트" 같은 변형 모드를 설정 변경만으로 실험할 수 있다.
- 총 2개뿐이므로 증강 하나의 무게가 크다 — 밸런스는 10_AUGMENT_SYSTEM에서 설계.

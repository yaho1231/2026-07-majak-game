...
- 가입: `register{username, password, adminCode?, signupCode?}` — 닉네임 2~12자
  (한글·영문·숫자·_-), **비밀번호 8자 이상 + 숫자 전용 금지 + 닉네임 포함 금지**
  (`SiteDb.register`, 8b차에서 4자+에서 강화됐다). 로그인은 기존 계정 호환을 위해
  길이를 검사하지 않는다. 이 규칙은 **로그인 화면 가입 탭에 그대로 적혀 있다** —
  적어 두지 않으면 방문자는 규칙을 모르는 채 거절만 반복해서 받는다.
...
## §2b 게스트 체험 (2026-08-07)

계정 없이 봇 3명과 1인 게임을 두는 길. **가입 게이트의 옆문이 아니다** — 손님은
*놀 수* 있을 뿐, 계정 공간(가입·리더보드·리플레이·제보)에는 들어오지 못한다.

- 프로토콜: `guestPlay{mode?}` (인증 **전**에 받는다) → `authOk{guest:true,
  sessionToken:""}` + `catalog` + `roomCreated` + 즉시 시작된 게임. 한 번의 클릭이
  곧 첫 패다(예전 경로는 7클릭 + 봇 추가 3번이었다).
- **신원**: 서버가 `손님#0000`~`손님#9999`를 발급한다. `#`는 닉네임 정규식에 없는
  문자라 **그 이름으로는 가입할 수 없다** — 사칭이 양방향으로 닫혀 있다. DB에는
  아무 행도 쓰지 않고(`users.id`는 가짜 `-1`), 세션 토큰도 없다.
- **권한**: 라우터의 화이트리스트(`GUEST_ALLOWED_MESSAGES`) — `action`·`draftPick`·
  `roundContinue`·`handOrder`·`leaveRoom`뿐. 나머지는 전부 `GUEST_FORBIDDEN`.
  기본이 거부이므로 앞으로 메시지가 늘어도 손님에게 새 길이 열리지 않는다.
  특히 `createRoom`·`joinRoom`이 없어 방 코드 공간을 파거나 남의 방에 낄 수 없다.
- **기록 없음**: 샌드박스와 같다 — 리플레이 파일(writer 미생성)·게임 인덱스
  (`recordGame`)·누적 통계(`finishStats(persist=false)`)·증강 집계 전부 건너뛴다.
  방은 `liveGames`에서 빠지고, 남이 `joinRoom{code}`하면 `ROOM_NOT_FOUND`(존재를 감춤).
- **드래프트는 실전 그대로** 준다 — 증강 없는 체험은 이 게임의 체험이 아니다.
- **남용 방어는 기존 장치를 그대로 쓴다**(병렬 장치를 만들지 않는다):
  `rateLimited`(인증과 같은 창: 연결당 12회/분 · IP당 30회/분) → `roomCreateLimited`
  (IP당 20회/10분) → `MAX_ROOMS`(200) → `MAX_GUEST_ROOMS`(16) →
  `MAX_GUEST_ROOMS_PER_IP`(3).
- **동시 판 상한의 근거는 CPU다.** 봇 3명의 판단은 이 서버의 단일 이벤트 루프에서
  돈다. `npm run arena -- --games 10 --augments`로 잰 값이 반장전 한 판(봇 4)에
  CPU 13.6초 — 봇 3명이면 판당 약 10초다. 판은 봇 생각 딜레이 때문에 벽시계로
  10~15분 이어지므로, **동시에 도는 게스트 판 하나가 코어의 약 1.5%**를 계속 먹는다.
  40판은 코어의 60%로 실계정 대국·리더보드·정적 서빙의 몫을 손님이 가져가서 16으로
  낮췄다(약 25%). 하드웨어가 바뀌면 arena로 다시 재고 이 숫자를 옮긴다.
- **IP당 상한이 따로 필요한 이유**: 전역 상한만으로는 한 사람이 손님 자리를 통째로
  먹는 것을 못 막는다. 연결당 게스트 방은 하나지만(새 `guestPlay`는 앞 판을 접는다)
  IP당 동시 연결이 16개까지 열리므로 탭 16개면 손님 자리가 전부 한 사람 것이 된다.
  레이트리밋도 여기엔 안 듣는다 — 만들고 **유지**하는 것이지 만들고 버리기를
  반복하는 게 아니기 때문이다. 자기가 지금 붙들고 있는 판은 세지 않는다("한 판 더"가
  만석일 때 제 자리를 두고 거부당하지 않게).
- **생명주기**: 재접속 수단이 없으므로 소켓이 닫히거나 `leaveRoom`·`logout`이 오면
  그 판을 `requestAbort` + 방 삭제로 접는다(유령 좌석이 봇 셋과 30초씩 멎으며
  판을 끝까지 도는 것을 막는다). 종국 `gameOver`는 `canContinue:false`다.
- 클라이언트: 랜딩의 "▶ 게스트로 바로 체험" → 게임. 게임 밖에서는 홈 대신
  `GuestOutro`(한 판 더 / 도감 / 규칙 / **계정 만들고 계속하기**)를 그린다.
  "계정 만들기"는 로그인 화면으로 돌려보낼 뿐 — 가입 코드는 여전히 필요하다.
- 회귀 테스트: `packages/server/test/Guest.test.ts`(게이트 우회 불가·권한 화이트리스트·
  방 은닉·기록 없음·레이트리밋·동시 판 상한(전역·IP당)·연결 종료 시 방 소멸·serverInfo).

## §2b-1 튜토리얼 판 (2026-08-18)

게스트 체험의 **특수형**이다. 같은 문(`guestPlay`)에 `tutorial:true`를 실어 열고,
손님 방의 성질(기록 없음·손님 방 예산 공유·끊기면 §2-5의 '판 세워 두기')을 그대로
물려받는다. 왜 갈랐는가와 무엇을 고정하는가는 `TUTORIAL_ROOM_NOTE`에 한자리에 있다.

- **체험 ≠ 튜토리얼.** 체험은 진짜 판(무작위 배패·무작위 증강·최선의 봇)이고,
  튜토리얼은 **가르치기 위해 고정한 판**이다. 랜딩에도 그렇게 적어 두었으므로
  체험에는 코치를 얹지 않는다 — 적어 놓은 것과 실제가 달라지면 안 된다.
- 고정하는 것 넷: 시작 증강 `alchemist`(액티브 + 생성패), 고정 배패(1샹텐),
  봇의 리치·화료 금지(`restrictOptions` 재사용), **결정 제한 시간 사실상 없음**.
- 마지막 하나가 이 모드를 만든 이유다. 30초 제한이 그대로면 증강 설명을 읽는 사이
  선택창이 **무작위로** 대신 뽑아 간다(`HumanAgent.armDraft`). 30분(`TUTORIAL_
  DECISION_TIMEOUT_MS`)으로 두되 화면에는 마감을 **0으로 실어** 시계를 그리지
  않는다. "무제한"이 아니라 "사람이 못 닿는 값"인 이유는 그 상수의 주석에.
- 판을 고정하는 배관은 **샌드박스의 것을 그대로 쓴다**(`presetHands`·
  `presetAugments`) — 새 경로를 만들지 않았다. 드래프트는 끄지 않는다:
  증강을 고르는 것이 이 게임의 첫 조작이다.
- 로그인한 사람에게는 `practicePlay{tutorial:true}`가 같은 판을 준다. 홈의 문은
  둘로 갈라 두었다 — «🎓 튜토리얼»(고정 + 안내)과 «연습 대국»(무작위 + 안내 없음).
- 클라이언트: 코치는 `client/src/tutorial.ts`(강의 목록)와 App.tsx의
  `TutorialCoach`(말풍선·강조 링). **튜토리얼 동안에는 자동 화료·후로없음·자동버림이
  적용되지 않는다** — 저장된 설정 그대로 두고 적용만 멈춘다(안 그러면 "직접 눌러
  보세요"라고 말하는 동안 클라이언트가 먼저 답한다).
- 회귀 테스트: `packages/server/test/Tutorial.test.ts`(고정 증강·고정 배패·첫 순
  액티브 발동 가능·마감 0·체험은 종전 30초·기록 없음·끊겨도 판을 세워 둔다),
  `packages/client/test/tutorialCoach.test.ts`(강의 순서·속도 조절·끼어들기·완료 판정).

## §2c 로그인 화면 = 랜딩 (2026-08-07)

로그인 전 화면은 **한 줄 + 시작 버튼**이다. 방문자에게 필요한 것은 "이게 무엇인가"
한 문장과 지금 바로 눌러 볼 곳뿐이고, 나머지는 게임과 규칙 화면이 맡는다.
**PROJECT_CHARTER.md는 내부 개발 문서다 — 그 문구·철학·감정 설계 어휘를 제품 UI에
옮기지 않는다**(2026-08-07 사용자 지시).

- `serverInfo{signupGate, guestPlay}` — 서버가 연결 직후(인증 전) 먼저 보낸다.
  덕분에 가입 코드 칸이 "서버에 설정된 경우 필요"라고 얼버무리지 않고, 게이트가
  켜져 있으면 **필수**로 표시하며 "지금은 초대제"라고 적고 게스트 체험을 가리킨다.
- 서버 인증 오류(`LOGIN_FAILED`·`REGISTER_FAILED`·`SIGNUP_CODE_REQUIRED` 등)는
  3.2초 토스트가 아니라 **폼 안 `auth-error` 자리**에 남는다(미인증 연결일 때만 —
  판정은 live ref `authedRef`).
- `index.html`에 `description`·OpenGraph·Twitter 카드 — 공유 링크가 "Majak" 한
  단어로만 보이지 않게.

## §2d 규칙 · 도움말 화면 (2026-08-07)

`HelpScreen` — **로그인 전 · 홈 · 게임 중** 어디서나 열린다. 내용은 **기능 설명만**이다
(§2c와 같은 이유로 헌장 어휘 금지). 리치마작 기본(목표·
4멘츠1작두·역 필요·리치·도라·후로·후리텐·점수·종국)과 증강 메타(무엇인지·게임 시작과
남장 진입에 총 2개·전원 동시·상대 증강 공개·무페널티) 두 탭. 도감(`.codex` 계열)의
표현을 그대로 재사용한다 — 새 디자인 언어를 만들지 않는다.

**도감·규칙은 오버레이다**(`.screen-overlay`, position:fixed). 예전에는 도감이 홈
라우팅 분기에 있어, 정작 필요한 순간(상대가 방금 공개한 증강이 무엇인지 궁금한
드래프트·대국 중)에 열 수 없었다 — 유일한 단서는 터치에서 아예 안 뜨는 `title=`
툴팁이었다. 지금은 게임판 위에 덮이고 아래 화면은 그대로 살아 있어 **게임 상태도
결정 타이머도 멈추지 않는다**. 진입점: 랜딩 "📘 규칙 · 증강 설명", 홈 "📘 규칙 ·
도움말", 게임 화면 우상단 📖 · 📘.
...
## §5b3 제보 게시판 — 버그 제보 · 증강 아이디어 (2026-08-04)

홈 화면의 **"📮 제보 게시판"** 카드. 종류(버그/아이디어)를 고르고 제목·내용을 써서 제출한다.

- **공개 범위가 곧 쿼리다.** `SiteDb.listFeedback(user)`가 관리자면 전체를, 아니면
  `WHERE user_id = 본인`만 조회한다 — 상위 계층이 필터를 빠뜨려도 남의 글이 샐 수 없다.
  응답의 `mine` 플래그는 관리자 목록에서 내 글을 구분하는 표시용이다.
- 프로토콜: `feedbackSubmit{kind,title,body}` · `feedbackList` · `feedbackUpdate{id,status?,reply?}`
  (**관리자 전용**) · `feedbackDelete{id}` (작성자 본인 또는 관리자). 응답은 항상 갱신된
  `feedbackList{entries, isAdmin}` — 쓰기 성공이 곧 목록 갱신이라 클라가 따로 재조회하지 않는다.
- 상태: `open`(접수) → `reviewing`(검토 중) → `done`(반영 완료) / `rejected`(반려). 관리자만 바꾼다.
  관리자 답변(`reply`)은 작성자와 관리자에게 보인다. 상태만 바꿔도 답변은 지워지지 않는다.
- 제한: 제목 80자·본문 4000자·답변 4000자, **계정당 1시간 10건**(도배 방지). 어기면
  `FEEDBACK_FAILED`로 거부하고 조용히 자르지 않는다. 라우터에도 8000자 입구 방벽이 있다.
- 계정 삭제 시 제보는 `user_id`만 NULL로 끊는다 — 관리자에게는 처리 이력이 남고,
  같은 닉네임으로 재가입한 다른 사람에게는 보이지 않는다.
- 회귀 테스트: `packages/server/test/Feedback.test.ts`(공개 범위·권한·상태/답변·삭제·도배 상한).

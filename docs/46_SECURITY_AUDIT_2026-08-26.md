...
| L-1 | 낮음 | `sendNotFound`(404 응답)에 `Strict-Transport-Security` 헤더 누락 | 전송 계층 | 아니오 — 신규 코드(도입 시부터 누락) | [완료] (#409 → 이번 재적용) |
...
**2026-09-16 갱신** — L-1을 닫은 #409(`0b1b20d`)는 머지 13분 뒤 #412(`a92e3e5`)가
통째로 되돌렸고, 그 뒤로 복구된 적이 없었다(머지 되돌림 스캐너로 확인 — QA 55 §A-8).
운영 404는 그동안 계속 HSTS 없이 나갔다. 같은 패치를 다시 적용했고, 되돌아가면 실제
응답으로 잡히는 `SecurityHeaders.test.ts`도 함께 복원했다. 아래
[L-1 조치](#l-1--404-응답-hsts-누락-완료)에 적었다.
...
> **재적용 이력 (2026-09-16).** 이 절의 변경(#409, 08-27)은 13분 뒤에 들어온 #412가
> `index.ts`·두 테스트·이 문서를 **통째로 되돌리면서** 사라졌고, 되돌린 쪽도 되돌아간
> 쪽도 그 사실을 몰랐다 — `SecurityHeaders.test.ts`가 파일째 지워졌으니 실패할 테스트도
> 함께 없어진 것이다. 2026-09-16 QA(55 §A-8)에서 `git diff 0b1b20d^ 0b1b20d`를 그대로
> 다시 적용했다. 그 사이 `index.ts`가 많이 바뀌었지만(perf·healthz·구간 요청 등) 문서
> 응답 세 자리는 그대로여서 헤더 한 벌의 위치와 값은 #409와 같다.

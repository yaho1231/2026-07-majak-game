...
| L-1 | 낮음 | `sendNotFound`(404 응답)에 `Strict-Transport-Security` 헤더 누락 | 전송 계층 | 아니오 — 신규 코드(도입 시부터 누락) | [열림] |

**2026-08-27 갱신** — H-1·H-2를 코드로 닫았다. 무엇이 어떻게 바뀌었는지는 맨 아래
...
- **L-1**(404 응답 HSTS 누락)은 열려 있다. 헤더 다섯을 공통 헬퍼로 뽑는 리팩터링과 함께
  처리하는 편이 같은 누락의 재발을 막는다.

...
- 봇 4명 반장전 완주·드래프트·토비·우마 정산 검증 완료
...
- Gold 신규(22차): **die_hard(죽기살기 — 게임당 1회 0 미만 피해를 0에서 버팀, 토비 방지)**
- 신규(28차):
...
- `~/majak` (npm workspaces 모노레포 · 2026-08-18에 `~/Documents` 밖으로 옮겼다 — 아래 CLAUDE.md TCC 절)
...
## 2026-08-18 (미래를 보는 자 — 2026-08-15의 "순수 교환"을 되돌린다)

**사용자 지시.** 2026-08-15 PR #269/#270에서 이 증강을 "손패 3장을 **내가 전부 고르고**
바닥에 버리는 패도 없는 순수 교환"으로 바꿨었다. 그 결과 **리스크가 통째로 사라져
누르면 무조건 이득인 버튼**이 됐다 — 손패는 항상 좋아지고 발동마다 +1판이 확정으로 붙어,
억제라고는 3순 쿨다운과 "가져온 3장 공개"뿐이었다. 이 증강의 정체성은 하이리스크
하이리턴이라는 사용자 판단으로 **2026-07-22 사양으로 복귀**했다.

- 무장(`future_arm`) 뒤 손패에서 **무작위 3장**이 뽑히고, 그중 **바닥에 버릴 1장만** 고른다
  (`future_exchange {tileId}` **한 번**으로 끝난다 — 세 번 고르던 다단계는 없어졌다).
- 나머지 2장은 패산 맨 밑으로, 패산 위 3장이 손에. 버린 1장은 **내 후리텐 근거로 남는다**
  (`discardedKinds`에 새긴다 — lastDiscard는 건드리지 않아 남의 론·후로는 열리지 않는다).
- 난수를 다시 쓴다 — `pickThree`가 `state.prngState`에서 결정적으로 뽑아 후보 제시와
  확정이 같은 3장을 본다. 리듀서가 `prngState`를 전진시킨다.
- 3순 쿨다운·스택당 1판·가져온 3장 전원 공개·쿨다운 pill(#283)은 **그대로**.
- 봇 정책도 되돌렸다: 2단계는 손패 문제가 아니라 **안전패 문제**(`safety` 우선, 동률이면
  손에 덜 쓸모 있는 쪽). 1단계는 종전대로 텐파이·1샹텐이면 무장하지 않는다.
- 클라 모달 문구 복귀("버릴 패 선택" / "이 패를 버린다" / "랜덤으로 버리기").
- docs/17·20·27은 애초에 2026-08-15 사양을 반영한 적이 없어 **손댈 것이 없었다**
  (되돌린 지금이 오히려 문서와 맞는다). powerTier 노트만 갱신.
...
- **죽기살기(die_hard, gold)**: 게임당 1회, 정산으로 점수가 0 미만이 될 때
  부족분을 환급받아 정확히 0에서 버틴다(토비는 0 '미만'이므로 생존).
...
- **보완**: 토비 0점 '이하'→'미만'(문서 기준), 영상개화+해저 중복 가산 제거,

...
**메인 로비** — 방 코드로 대국을 열고, 전적·증강별 성적·리플레이를 한자리에서 본다.

![메인 로비](docs/screenshots/lobby.png)

**인게임** — 정통 리치마작 작탁. 좌석 이름표마다 그 사람이 들고 있는 증강이 붙어,
누가 무엇을 쥐고 있는지 판 위에서 그대로 보인다.

![인게임](docs/screenshots/ingame.png)

**증강 선택** — 대국 개시와 남장 진입 때 각각 한 번, 세 장 중 하나를 고른다.
후보는 좌석마다 겹치지 않고, 카드에서 바로 상세 설명을 펼칠 수 있다.

![증강 선택](docs/screenshots/augment-draft.png)
...
별도 라이선스를 두지 않는다 — 저작권은 전부 보유(All rights reserved)한다.
소스는 공개돼 있지만 복제·수정·재배포·2차 저작을 허락하지 않는다.
읽고 배우는 것은 자유이며, 쓰임이 필요하면 먼저 문의해 주기 바란다.

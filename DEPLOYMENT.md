...
- **프로세스 그룹 함정 (`AbandonProcessGroup`)**: launchd는 기본값에서 **작업이 끝나는
  순간 그 프로세스 그룹에 남은 프로세스 전부에 SIGTERM**을 보낸다. 감시자는 서버를
  백그라운드 자식으로 띄우고 곧 끝나므로, 이 키가 없으면 **방금 세운 서버를 감시자가
  스스로 죽인다** → 1분마다 세우고 죽고를 반복한다(2026-08-19 오전, 한 시간 넘게 폭주).
  로그에는 "다시 세움 완료"만 남아 원인이 보이지 않는 게 이 고장의 고약한 점이다.
  플리스트에 `AbandonProcessGroup=true` 를 넣어 고쳤다.
  - **플리스트는 `install` 때만 다시 쓰인다.** 이 키가 생기기 전에 등록해 둔 플리스트가
    남아 있으면 그대로 폭주한다 — `npm run agents` 가 낡은 플리스트를 짚어 주고, 감시자
    자신도 재시작 직전에 로그·알림으로 알린다. 고치는 법은 한 줄: `npm run agents:install`.
  - 증상 구분법: **손으로 켠 서버는 멀쩡히 몇 시간 돌고, 감시자가 세운 서버만 10초 뒤
    SIGTERM으로 죽는다.** `.majak/server.log` 의 시작–SIGTERM 간격을 보면 바로 갈린다.
...
- **서버 재시작 시 진행 중 게임은 자동으로 이어진다** (감사 §2-10, 2026-09-24 보강): 판은
  `live_games` + 리플레이 파일로 남고, 다음 부팅이 **포트를 열기 전에** 같은 방 코드로 되살린다.
  종료 직전 사람들에게는 `gameAborted`가 아니라 `serverRestarting`이 가서 클라이언트가 방 기억을
  지키고, 재연결 → 자동 재로그인 → 자동 `joinRoom`으로 **아무 조작 없이** 판에 돌아온다.
  되살린 판은 사람이 돌아올 때까지 최대 3분(`RESUME_HOLD_MS`) 세워 둔다. 대상이 아닌 판:
  체험(손님)·튜토리얼·증강 테스트 — 기록을 남기지 않는 판이라 재시작하면 무효로 끝난다.

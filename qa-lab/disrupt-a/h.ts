...
export interface Ledger {
  scoreTotal?: number;
  /** 최근 정산에 실린 augPoints 메모 (증강이 "이 점수는 내가 발행했다"고 남긴 것) */
  notes: string[];
  /** 최근 ScoreChanged 이벤트의 reason */
  reasons: string[];
}
...
export function checkState(
  ...
  if (seen.scoreTotal !== undefined && total !== seen.scoreTotal) {
    ...
    // 그런 정산에는 augPoints 메모나 ScoreChanged.reason 이 남는다 — 그게 하나도
    // 없이 총합이 움직이면 그건 회계 구멍이다.
    const why = [...seen.notes, ...seen.reasons].join(", ");
    const kind = why === "" ? "SCORE_DRIFT_UNEXPLAINED" : "SCORE_DRIFT_ATTRIBUTED";
    add(kind, `${seen.scoreTotal} -> ${total} (delta ${total - seen.scoreTotal})${why === "" ? "" : ` — ${why}`}`);
  }
  seen.notes = [];
  seen.reasons = [];

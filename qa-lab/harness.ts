...
export interface Ledger {
  scoreTotal?: number;
  /** 정산에 실린 augPoints 메모 (증강이 "이 점수는 내가 발행했다"고 남긴 것) */
  notes: { label: string; amount: number }[];
  /** ScoreChanged 이벤트의 reason */
  reasons: { label: string; amount: number }[];
  /** 관측된 총합 변동 — 뒤에 도착하는 근거와 금액으로 대조한다 */
  drifts: { delta: number; round: string; from: number; to: number }[];
}
...
export function checkState(
  ...
  if (seen.scoreTotal !== undefined && total !== seen.scoreTotal) {
    // 이 게임에는 **뱅크 발행**(증강이 없는 점수를 만드는 것)이 설계로 존재한다.
    // 근거는 augPoints 메모·ScoreChanged.reason 으로 남지만 **뷰보다 늦게 도착할 수
    // 있어**, 여기서는 기록만 하고 판정은 매치가 끝난 뒤 금액 대조로 한다.
    seen.drifts.push({ delta: total - seen.scoreTotal, round: rk, from: seen.scoreTotal, to: total });
  }
...
export async function runMatch(o: RunOpts): Promise<MatchReport> {
  ...
  const ctrl = new HanchanController(agents, {
    ...
    onRoundStart: (g) => { rounds++; o.onRound?.(g.engine.state, "start"); },
  ...
  // 드리프트 판정 — 근거(augPoints·ScoreChanged.reason)의 **부분합**과 대조한다.
  // 한 건과 1:1로만 맞춰 보면, 한 정산에 뱅크 발행이 둘 이상 겹칠 때(같은 증강을
  // 두 사람이 들면 흔하다) 멀쩡한 발행이 UNEXPLAINED로 뜬다 — 2026-08-20 짝 감사에서
  // 신호 16건이 전부 그 오탐이었다.
  {
    const pool = [...seen.notes, ...seen.reasons];
    const subsetSums = new Map<number, string>([[0, ""]]);
    for (const item of pool) {
      for (const [sum, label] of [...subsetSums]) {
        const next = sum + item.amount;
        if (!subsetSums.has(next) && subsetSums.size < 20000) {
          subsetSums.set(next, label === "" ? item.label : `${label} + ${item.label}`);
        }
      }
    }
    for (const d of seen.drifts) {
      const why = subsetSums.get(d.delta);
      const head = `${d.from} -> ${d.to} (${d.delta >= 0 ? "+" : ""}${d.delta})`;
      if (why !== undefined && why !== "") {
        violations.push({ kind: "SCORE_DRIFT_ATTRIBUTED", round: d.round, detail: `${head} — ${why}` });
      } else {
        violations.push({
          kind: "SCORE_DRIFT_UNEXPLAINED",
          round: d.round,
          detail: `${head} — 어떤 부분합과도 맞지 않음. 근거 후보: ${pool.map((x) => x.label).slice(-8).join(", ")}`,
        });
      }
    }
  }

...
1. **솔로 대국 무효 투표로 성적 세탁** (MEDIUM, 게임 무결성 · **의사결정 필요**) —
   `voteAbort`는 "남아 있는 사람 전원 동의"로 성립한다. 사람이 1명(+봇 3)인 방에서는
   **혼자서 언제든 판을 무효로 만들 수 있고**, 무효 판은 순위·통계·리플레이를 전혀
   남기지 않는다. 지고 있을 때만 무효를 누르면 리더보드에 이긴 판만 쌓인다.
   (창을 닫는 이탈은 봇이 이어 두고 순위가 기록되므로, 무효 쪽이 더 유리하다.)
   선택지: ⑴ 사람 2명 이상일 때만 무효 허용(솔로는 이탈로 종료 = 기록됨),
   ⑵ 무효도 참가 기록은 남기기, ⑶ 솔로 대국은 리더보드에서 분리. **게임 설계 결정이라
   코드로 먼저 정하지 않았다.**
2. **비밀번호 변경·세션 일괄 로그아웃 부재** (MEDIUM, 기능) — 토큰이 샜다고 판단해도
   사용자가 스스로 무효화할 수단이 없다(관리자 계정 삭제만 세션을 끊는다).
   권장: 비밀번호 변경 + 변경 시 전 세션 무효화, "다른 기기 로그아웃".
